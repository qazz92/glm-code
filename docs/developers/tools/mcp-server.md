# MCP servers with GLM Code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with GLM Code.
...
GLM Code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
4. **Sanitizes and validates** tool schemas for compatibility with the GLM API
...
GLM Code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
GLM Code supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **`redirectUri`** (string): Custom redirect URI. **Critical for remote deployments**: Defaults to `http://localhost:7777/oauth/callback`. When running GLM Code on remote/cloud servers, set this to a publicly accessible URL (e.g., `https://your-server.com/oauth/callback`). Can be configured via `glm mcp add --oauth-redirect-uri` or directly in settings.json.
...
When GLM Code starts, it performs MCP server discovery through the following detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet GLM API requirements:
...
Once discovered, MCP tools are available to the GLM model like built-in tools. The model will automatically:
...
When GLM Code receives this response, it will:
...
This enables you to build sophisticated tools that can provide rich, multi-modal context to the GLM model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within GLM Code. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="GLM Code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "GLM Code" reverent
```
...
## Managing MCP Servers with `glm mcp`
...
The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.glm/settings.json` or the project config `.glm/settings.json` file.
...
- `--oauth-redirect-uri`: OAuth redirect URI (e.g., `https://your-server.com/oauth/callback`). Defaults to `http://localhost:7777/oauth/callback` for local setups. **Important for remote deployments**: When running GLM Code on remote/cloud servers, set this to a publicly accessible URL.
...
```bash
# Basic syntax
glm mcp add <name> <command> [args...]

# Example: Adding a local server
glm mcp add my-stdio-server -e API_KEY=123 /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
glm mcp add python-server python server.py --port 8080
```
...
```bash
# Basic syntax
glm mcp add --transport http <name> <url>

# Example: Adding an HTTP server
glm mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
glm mcp add --transport http secure-http https://api.example.com/mcp/ --header "Authorization: Bearer abc123"
```
...
```bash
# Basic syntax
glm mcp add --transport sse <name> <url>

# Example: Adding an SSE server
glm mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
glm mcp add --transport sse secure-sse https://api.example.com/sse/ --header "Authorization: Bearer abc123"
...
### Managing Servers (`glm mcp`)

To view and manage all MCP servers currently configured, use the `manage` command or simply `glm mcp`. This opens an interactive TUI dialog where you can:
