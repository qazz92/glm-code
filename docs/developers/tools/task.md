...
This document describes the `task` tool for GLM Code.
...
You can view available subagents by using the `/agents` command in GLM Code.
...
Subagents are configured through GLM Code's agent configuration system. Use the `/agents` command to:
