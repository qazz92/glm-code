...
This document describes the `todo_write` tool for GLM Code.
...
## How to use `todo_write` with GLM Code
...
The tool stores todo lists in your home directory (`~/.glm/todos/`) with session-specific files, so each coding session maintains its own task list.
