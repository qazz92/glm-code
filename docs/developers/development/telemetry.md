...
Learn how to enable and setup OpenTelemetry for GLM Code.
...
observability framework — GLM Code's observability system provides:
...
All telemetry behavior is controlled through your `.glm/settings.json` file.
These settings can be overridden by environment variables or CLI flags.

| Setting                          | Environment Variable                               | CLI Flag                                                 | Description                                                                                                                          | Values            | Default                 |
| -------------------------------- | -------------------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | ----------------------- |
| `enabled`                        | `GLM_TELEMETRY_ENABLED`                           | `--telemetry` / `--no-telemetry`                         | Enable or disable telemetry                                                                                                          | `true`/`false`    | `false`                 |
| `target`                         | `GLM_TELEMETRY_TARGET`                            | `--telemetry-target <local\|gcp>` _(deprecated)_         | Informational destination label; does not control exporter routing — set `otlpEndpoint` or `outfile` to configure where data is sent | `"gcp"`/`"local"` | `"local"`               |
| `otlpEndpoint`                   | `GLM_TELEMETRY_OTLP_ENDPOINT`                     | `--telemetry-otlp-endpoint <URL>`                        | OTLP collector endpoint                                                                                                              | URL string        | `http://localhost:4317` |
| `otlpProtocol`                   | `GLM_TELEMETRY_OTLP_PROTOCOL`                     | `--telemetry-otlp-protocol <grpc\|http>`                 | OTLP transport protocol                                                                                                              | `"grpc"`/`"http"` | `"grpc"`                |
| `otlpTracesEndpoint`             | `GLM_TELEMETRY_OTLP_TRACES_ENDPOINT`              | -                                                        | Per-signal endpoint override for traces (HTTP only)                                                                                  | URL string        | -                       |
| `otlpLogsEndpoint`               | `GLM_TELEMETRY_OTLP_LOGS_ENDPOINT`                | -                                                        | Per-signal endpoint override for logs (HTTP only)                                                                                    | URL string        | -                       |
| `otlpMetricsEndpoint`            | `GLM_TELEMETRY_OTLP_METRICS_ENDPOINT`             | -                                                        | Per-signal endpoint override for metrics (HTTP only)                                                                                 | URL string        | -                       |
| `outfile`                        | `GLM_TELEMETRY_OUTFILE`                           | `--telemetry-outfile <path>`                             | Save telemetry to file (overrides OTLP export)                                                                                       | file path         | -                       |
| `logPrompts`                     | `GLM_TELEMETRY_LOG_PROMPTS`                       | `--telemetry-log-prompts` / `--no-telemetry-log-prompts` | Include prompts in telemetry logs                                                                                                    | `true`/`false`    | `true`                  |
| `includeSensitiveSpanAttributes` | `GLM_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES` | -                                                        | Include sensitive attributes in log-to-span bridge spans                                                                             | `true`/`false`    | `false`                 |
...
**Sensitive log-to-span attributes:** When GLM Code exports HTTP traces but has
...
of this bridge setting. GLMLogger does not include `response_text`.

**HTTP OTLP signal routing:** When using HTTP protocol (`otlpProtocol: "http"`),
GLM Code automatically appends signal-specific paths (`/v1/traces`, `/v1/logs`,
...
The `GLM_TELEMETRY_OTLP_*` variants take precedence over the `OTEL_*` variants.
...
To view GLM Code telemetry in Alibaba Cloud Managed Service for
OpenTelemetry, configure GLM Code to export to the OTLP endpoint
provided by ARMS.

Setting `"target": "gcp"` alone does not configure the export
destination. If `otlpEndpoint` is not set, GLM Code still defaults to
...
1. Enable telemetry in your `.glm/settings.json` and set the OTLP
   endpoint:
   ...
   **Option B: HTTP protocol with per-signal endpoints** (for backends
   ...
   > per-signal overrides), GLM Code appends standard OTLP paths
...
2. If your Alibaba Cloud endpoint requires authentication, provide OTLP
   ...
   `OTEL_EXPORTER_OTLP_HEADERS` (or the signal-specific variants). GLM
   Code does not currently expose OTLP auth headers directly in
   `.glm/settings.json`.
3. Run GLM Code and send prompts.
...
1. Enable telemetry in your `.glm/settings.json`:

   ```json
   {
     "telemetry": {
       "enabled": true,
       "outfile": ".glm/telemetry.log"
     }
   }
   ```
...
2. Run GLM Code and send prompts.
3. View logs and metrics in the specified file (e.g., `.glm/telemetry.log`).
...
1. Run the automation script:
   ...
   - Save logs/metrics to `~/.glm/tmp/<projectHash>/otel/collector.log`
...
Logs are timestamped records of specific events. The following events are logged for GLM Code:

- `glm-code.config`: This event occurs once at startup with the CLI's configuration.
  - **Attributes**:
    - `model` (string)
...
- `glm-code.user_prompt`: This event occurs when a user submits a prompt.
  - **Attributes**:
    - `prompt_length` (int)
    - `prompt_id` (string)
...
- `glm-code.tool_call`: This event occurs for each function call.
  - **Attributes**:
    - `function_name`
    - `function_args`
    - `duration_ms`
...
- `glm-code.file_operation`: This event occurs for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
- `glm-code.api_request`: This event occurs when making a request to GLM API.
  - **Attributes**:
    - `model`
    - `request_text` (if applicable)

- `glm-code.api_error`: This event occurs if the API request fails.
  - **Attributes**:
    - `model`
    - `error`
    - `error_type`
...
- `glm-code.api_response`: This event occurs upon receiving a response from GLM API.
  - **Attributes**:
    - `model`
    - `status_code`
    - `duration_ms`
...
- `glm-code.tool_output_truncated`: This event occurs when the output of a tool call is too large and gets truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `glm-code.malformed_json_response`: This event occurs when a `generateJson` response from GLM API cannot be parsed as a json.
  - **Attributes**:
    - `model`

- `glm-code.flash_fallback`: This event occurs when GLM Code switches to flash as fallback.
  - **Attributes**:
    - `auth_type`

- `glm-code.slash_command`: This event occurs when a user executes a slash command.
  - **Attributes**:
    - `command` (string)
    - `subcommand` (string, if applicable)

- `glm-code.extension_enable`: This event occurs when an extension is enabled
- `glm-code.extension_install`: This event occurs when an extension is installed
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
    - `extension_source` (string)
    - `status` (string)
- `glm-code.extension_uninstall`: This event occurs when an extension is uninstalled

### Metrics

Metrics are numerical measurements of behavior over time. The following metrics are collected for GLM Code (metric names remain `glm-code.*` for compatibility):

- `glm-code.session.count` (Counter, Int): Incremented once per CLI startup.

- `glm-code.tool.call.count` (Counter, Int): Counts tool calls.
  - **Attributes**:
    - `function_name`
    - `success` (boolean)
...
- `glm-code.tool.call.latency` (Histogram, ms): Measures tool call latency.
  - **Attributes**:
    - `function_name`
    - `decision` (string: "accept", "reject", or "modify", if applicable)

- `glm-code.api.request.count` (Counter, Int): Counts all API requests.
  - **Attributes**:
    - `model`
    - `status_code`
    - `error_type` (if applicable)

- `glm-code.api.request.latency` (Histogram, ms): Measures API request latency.
  - **Attributes**:
    - `model`

- `glm-code.token.usage` (Counter, Int): Counts the number of tokens used.
  - **Attributes**:
    - `model`
    - `type` (string: "input", "output", "thought", or "cache")

- `glm-code.file.operation.count` (Counter, Int): Counts file operations.
  - **Attributes**:
...
- `glm-code.chat_compression` (Counter, Int): Counts chat compression operations
  - **Attributes**:
