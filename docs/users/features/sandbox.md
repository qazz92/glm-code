...
This document explains how to run GLM Code inside a sandbox to reduce risk when tools execute shell commands or modify files.

## Prerequisites

Before using sandboxing, you need to install and set up GLM Code:
...
> [!note]
>
> **Naming note:** Some sandbox-related environment variables may have used the `GEMINI_*` prefix historically. All new environment variables use the `GLM_*` prefix.
...
By default, GLM Code uses a published sandbox image (configured in the CLI package) and will pull it as needed.

The container sandbox mounts your workspace and your `~/.glm` directory into the container so auth and settings persist between runs.
...
glm -s -p "analyze the code structure"

# Or enable sandboxing for your shell session (recommended for CI / scripts)
export GLM_SANDBOX=true   # true auto-picks a provider (see notes below)
glm -p "run the test suite"
...
>
> - On **macOS**, `GLM_SANDBOX=true` typically selects `sandbox-exec` (Seatbelt) if available.
> - On **Linux/Windows**, `GLM_SANDBOX=true` requires `docker` or `podman` to be installed.
> - To force a provider, set `GLM_SANDBOX=docker|podman|sandbox-exec`.

## Configuration

### Enable sandboxing (in order of precedence)

1. **Environment variable**: `GLM_SANDBOX=true|false|docker|podman|sandbox-exec`
...
> [!important]
>
> If `GLM_SANDBOX` is set, it **overrides** the CLI flag and `settings.json`.

### Configure the sandbox image (Docker/Podman)

- **CLI flag**: `--sandbox-image <image>`
- **Environment variable**: `GLM_SANDBOX_IMAGE=<image>`
- **Settings file**: `tools.sandboxImage` in your `settings.json` (e.g., `{"tools": {"sandboxImage": "ghcr.io/qazz92/glm-code:0.14.1"}}`)
...
4. Built-in default image from the CLI package (for example `ghcr.io/qazz92/glm-code:<version>`)

`settings.env.GLM_SANDBOX_IMAGE` also works as a generic env injection mechanism, but `tools.sandboxImage` is the preferred persistent setting.
...
1. Create a file named `.glm/sandbox-macos-<profile_name>.sb` in your project.
...
- Set `GLM_SANDBOX_PROXY_COMMAND=<command>`
...
On Linux, GLM Code defaults to enabling UID/GID mapping so the sandbox runs as your user (and reuses the mounted `~/.glm`). Override with:
...
- Container sandbox: add them via `.glm/sandbox.Dockerfile` or `.glm/sandbox.bashrc`.
...
The official GLM Code Docker image is intentionally minimal to keep the image small, secure, and fast to pull. Different users require different language runtimes (Java, Python, Node.js, etc.), and bundling all environments into a single image is not practical. Therefore, Java is **not included by default** in the Docker sandbox.

If your workflow requires Java, you can extend the base image by creating a `.glm/sandbox.Dockerfile` in your project:

```dockerfile
FROM ghcr.io/qazz92/glm-code:latest
...
```bash
GLM_SANDBOX=docker BUILD_SANDBOX=1 glm -s
```
...
```bash
DEBUG=1 glm -s -p "debug command"
```

**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect the CLI due to automatic exclusion. Use `.glm/.env` files for GLM Code-specific debug settings.

### Inspect sandbox

```bash
# Check environment
glm -s -p "run shell command: env | grep SANDBOX"

# List mounts
glm -s -p "run shell command: mount | grep workspace"
```
