...
> Create, manage, and share Skills to extend GLM Code's capabilities.

This guide shows you how to create, use, and manage Agent Skills in **GLM Code**. Skills are modular capabilities that extend the model's effectiveness through organized folders containing instructions (and optionally scripts/resources).

## Prerequisites

- GLM Code (recent version)
- Basic familiarity with GLM Code ([Quickstart](../quickstart.md))
...
- Extend GLM Code for your workflows
...
Personal Skills are available across all your projects. Store them in `~/.glm/skills/`:

```bash
mkdir -p ~/.glm/skills/my-skill-name
```
...
Project Skills are shared with your team. Store them in `.glm/skills/` within your project:
...
## Instructions
Provide clear, step-by-step guidance for GLM Code.
...
GLM Code currently validates that:
...
GLM Code discovers Skills from:

- Personal Skills: `~/.glm/skills/`
- Project Skills: `.glm/skills/`
...
To see which extensions provide skills, check the extension's `glm-extension.json` file for a `skills` field.

To view available Skills, ask GLM Code directly:
...
cat ~/.glm/skills/my-skill/SKILL.md
```
...
If GLM Code doesn't use your Skill, check these common issues:
...
- Personal Skills: `~/.glm/skills/<skill-name>/SKILL.md`
- Project Skills: `.glm/skills/<skill-name>/SKILL.md`

```bash
# Personal
ls ~/.glm/skills/my-skill/SKILL.md
...
Run GLM Code with debug mode to see Skill loading errors:
...
1. Add the Skill under `.glm/skills/`
2. Commit and push
...
```bash
# Personal Skill
code ~/.glm/skills/my-skill/SKILL.md

# Project Skill
code .glm/skills/my-skill/SKILL.md
```

Changes take effect the next time you start GLM Code. If GLM Code is already running, restart it to load the updates.
