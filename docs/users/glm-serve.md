# Daemon mode (`glm serve`)

Run GLM Code as a local HTTP daemon so multiple clients (IDE plugins, web UIs, CI scripts, custom CLIs) share one agent session over HTTP + Server-Sent Events instead of each spawning their own subprocess.

> **Status:** Stage 1 (experimental). The protocol surface is locked at the §04 routes table from issue [#3803](https://github.com/qazz92/glm-code/issues/3803). Stage 1.5 (`glm --serve` flag — TUI co-hosts the same HTTP server) and Stage 2 (in-process refactor + `mDNS`/OpenAPI/WebSocket/Prometheus polish) are immediately downstream.
>
...
# → glm serve: bearer auth disabled (loopback default). Set GLM_SERVER_TOKEN to enable.
```
...
```bash
export GLM_SERVER_TOKEN="$(openssl rand -hex 32)"
glm serve --hostname 0.0.0.0 --port 4170
# → boot refuses without GLM_SERVER_TOKEN
```

Clients then send `Authorization: Bearer $GLM_SERVER_TOKEN` on every request. `/health` is exempted **only on loopback binds** so k8s/Compose liveness probes inside the pod (where the daemon listens on `127.0.0.1`) don't need credentials. On non-loopback binds (`--hostname 0.0.0.0` etc.) `/health` requires the token like every other route — otherwise an attacker can probe arbitrary addresses to confirm the daemon's existence. Use `/capabilities` to verify your token is correct end-to-end (it always requires auth):

```bash
curl -H "Authorization: Bearer $GLM_SERVER_TOKEN" http://your-host:4170/capabilities
...
| `--token <str>`         | —           | Bearer token. Falls back to `GLM_SERVER_TOKEN` env var (with leading/trailing whitespace stripped — handy for `$(cat token.txt)`).                                                                                                                                                                                                                 |
...
| `--http-bridge`         | `true`      | Stage 1 mode: per-session `glm --acp` child process. Stage 2 native in-process becomes available later.                                                                                                                                                                                                                                            |
...
- **Spawned `glm --acp` child inherits the daemon's environment** with one explicit scrub: `GLM_SERVER_TOKEN` is removed before the child starts (the daemon's own bearer; the agent doesn't need it). Everything else — `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GLM_*` / `DASHSCOPE_API_KEY` / your custom `modelProviders[].envKey` / etc. — passes through, because the agent legitimately needs those to authenticate to the LLM. **This is intentional, not a sandbox.** The agent runs as the same UID with shell-tool access, so anything in `~/.bashrc` / `~/.aws/credentials` / `~/.npmrc` is reachable by prompt injection regardless. The env passthrough is not the security boundary; the user-as-trust-root is. Don't run `glm serve` under an identity that has env-resident credentials you wouldn't trust the agent with.
...
To handle multiple **users** (each with their own quota, audit log, sandbox) or to scale beyond one process's reach (cold-start budget, FD count, RSS), you spawn multiple daemon instances behind an external orchestrator. That orchestrator (multi-tenancy / OIDC / Quota / Audit / k8s) is **out of scope** for the glm-code project — see issue [#3803](https://github.com/qazz92/glm-code/issues/3803) "External Reference Architecture" for the design pointers.
...
Stage 1's contract is sized for prototyping. Per [#3889 chiga0 downstream-consumer review](https://github.com/qazz92/glm-code/pull/3889#issuecomment-4427875644), the following are **not** in Stage 1 — production-grade integrations need Stage 1.5+ before relying on them:
...
The full convergence roadmap is tracked on [#3803](https://github.com/qazz92/glm-code/issues/3803).
...
### Session state is local-mutation-only (per [LaZzyMan review #4270256721](https://github.com/qazz92/glm-code/pull/3889#pullrequestreview-4270256721))
...
#### Mode 1 — headless `glm serve` (this PR)
...
- **Boot-time-frozen** for `approval-mode` / `memory` / `mcp servers` / `agents` / `tools` allowlist / `auth` — all loaded from settings + disk when the daemon's `glm --acp` child starts; immutable for the session's lifetime.
...
#### Mode 2 — Stage 1.5 `glm --serve` co-hosted TUI (not in this PR)

When Stage 1.5 lands `glm --serve` (TUI process co-hosts the same HTTP server), the TUI **does** exist alongside remote clients. A local operator typing `/approval-mode yolo` or `/mcp add-server` mutates session state, and remote clients on HTTP have no event to observe the change.
...
| `glm --acp` child                   | shared      | **60–100 MB** (one child)    |
...
**Peer agents (Cursor / Continue / Claude Code / OpenCode / Gemini CLI) all do single-process multi-session.** glm-code matches them at the agent layer; the Stage 1 bridge in this PR makes the same architecture visible over HTTP.

## What's next

- **Build a client?** See the [DaemonClient TypeScript quickstart](../developers/examples/daemon-client-quickstart.md) and the [HTTP protocol reference](../developers/glm-serve-protocol.md).
- **Reading the source?** Bridge code lives at `packages/cli/src/serve/`; SDK client at `packages/sdk-typescript/src/daemon/`.
- **Tracking the roadmap?** Stage 1.5 / Stage 2 progress is tracked on issue [#3803](https://github.com/qazz92/glm-code/issues/3803).
