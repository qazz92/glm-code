...
- Status: Draft
- Date: 2026-05-15
- Scope: Full Qwen-to-GLM rebrand plus implementation-gap closure for the GLM Code claims currently documented in `README.md`.
- Current convention note: this plan is saved under `.qwen/design/` because the active repository instructions still designate `.qwen/design/` for design artifacts. Phase 3 migrates project artifact paths to `.glm/` as part of the rebrand.

## 1. Requirements Summary

### Primary goals

1. Remove Qwen branding and namespaces from the product, source tree, package metadata, scripts, docs, tests, SDKs, and editor extensions.
...
4. Leave the repo in a releasable state: buildable packages, updated tests, no stale Qwen references except explicitly approved third-party/reference exceptions.
...
- No public `qwen`, `.qwen`, `QWEN_*`, `@qwen-code/*`, `qwen-code`, or `qwencode` compatibility aliases unless the user later changes the policy.
...
- other packages: 300
- `.glm`/`.qwen` agent artifacts: 104
- root/GitHub metadata: 51

Concrete examples:

- Root sandbox image still references Qwen: `package.json:25-27`.
- Root integration scripts still use `QWEN_SANDBOX`: `package.json:46-58`.
- CLI package repository and sandbox metadata still reference Qwen: `packages/cli/package.json:5-8` and `packages/cli/package.json:39-51`.
- Public OSS setup intentionally started with core-only CI: `.qwen/design/public-oss-github-setup.md:17-20`; current CI only runs metadata checks and core build at `.github/workflows/ci.yml:16-67`.
...
3. Generate a machine-readable Qwen inventory.
   - Suggested output: `.glm/rebrand/qwen-inventory.json` or `.glm/rebrand/qwen-inventory.md`.
   - Exclude `.git`, `node_modules`, `coverage`, `repos`, and generated `dist` on the first pass.
   - Re-run after each major phase.
4. Classify each remaining Qwen hit as:
...
1. Root package metadata.
   ...
   - Rename Qwen-specific npm scripts in `package.json:46-58` to GLM env vars and GLM test names.
2. Workspace package metadata.
   - Fix `packages/cli/package.json:7` repository URL from Qwen path to the actual GLM repo.
   - Fix `packages/cli/package.json:40` sandbox image.
   - Rename `@glm-code/qwen-code-core` at `packages/cli/package.json:51` to `@glm-code/core` and update imports/lockfile accordingly.
...
3. TypeScript SDK.
   - Rename docs/metadata from Qwen to GLM.
   - Ensure import examples use `@glm-code/sdk`.
4. Python SDK.
   - Rename package from `qwen-code-sdk` to a GLM name such as `glm-code-sdk`.
   - Rename import module from `qwen_code_sdk` to `glm_code_sdk`.
   - Update tests, docs, build config, pyproject metadata, and examples.
5. Java SDK.
   - Rename Maven artifact and Java package away from `qwencode` / `com.alibaba.qwen`.
   - Suggested target: artifact `glm-code-sdk`, package `ai.glm.code` or the final owner-approved namespace.
6. Editor extensions.
   - VS Code: change publisher/name/display identifiers, commands, settings, views, activation events, and docs from `qwen-code.*` / `qwen.diff.*` to `glm-code.*` / `glm.diff.*`.
...
- `npm install` completes without `@qwen-code/*` workspace package references.
- Package metadata scan returns no Qwen refs outside approved generated/reference exceptions.
...
1. Environment variables.
   - Rename `QWEN_SANDBOX` to `GLM_SANDBOX`.
   - Rename `QWEN_HOME` to `GLM_HOME`.
   - Rename `QWEN_RUNTIME_DIR` to `GLM_RUNTIME_DIR`.
   - Rename any `QWEN_CODE_*`, `QWEN_SERVER_TOKEN`, package archive env vars, or telemetry namespace env vars to `GLM_*`.
2. Filesystem paths.
   - Rename `.qwen` runtime/config references to `.glm`.
   - Rename `.qwenignore` to `.glmignore` if present.
   - Update settings schema, config loading, tests, docs, and integration helpers.
3. CLI binary and release artifacts.
   - Ensure all shims and package archives produce `glm`, not `qwen`.
   - Update standalone package scripts under `scripts/`.
   - Update install scripts under `scripts/installation/` from Qwen-specific filenames and release URLs to GLM-specific ones.
4. Docker/sandbox.
...
- New clean HOME starts with `~/.glm`, not `~/.qwen`.
- Integration helpers create `.glm/settings.json`, not `.qwen/settings.json`.
- `glm --version` or equivalent package command works from built output.
- No root script requires `QWEN_*` env vars.

### Phase 3 — Documentation, repo artifacts, and agent artifacts

1. Migrate project artifact convention.
   - Update `AGENTS.md` project directories from `.qwen/*` to `.glm/*`.
   - Move current planning/design artifacts to `.glm/design/` after repo instruction update.
   - Update any command/skill docs that create `.qwen/issues`, `.qwen/e2e-tests`, `.qwen/pr-drafts`, etc.
2. User docs.
   - Update `docs/users/**` for GLM install, config, auth, memory, headless, extension, GitHub Action, and quickstart.
   - Rename or remove `docs/users/qwen-serve.md` depending on final server command naming.
3. Developer docs.
   - Update `docs/developers/**` for GLM package names, release process, telemetry, SDK docs, and roadmap.
   - Change `docs/developers/roadmap.md:1` from Qwen Code roadmap to GLM Code roadmap.
4. Internal `.glm` agent/skill artifacts.
   - Rebrand `.glm/agents/test-engineer.md`, `.glm/skills/e2e-testing`, `.glm/commands/qc/*`, and remove/rename `.glm/skills/qwen-code-claw`.
...
- Docs scan has no Qwen references except an explicitly approved changelog/history section, if any.
- `AGENTS.md` no longer tells agents to use Qwen Code, global `qwen`, or `.qwen` artifacts.
...
4. Security/community files.
   - Re-scan issue templates, security policy, support docs, and CodeQL for Qwen refs.

Acceptance criteria:

- CI blocks regressions in package metadata and core/CLI build.
- Release process cannot publish Qwen-named artifacts.
...
3. Re-run Qwen inventory including generated outputs.
4. Regenerate snapshots/tests intentionally.
5. Review final diff for accidental unrelated changes.

Acceptance criteria:

- Final Qwen scan is zero outside approved exceptions.
...
```bash
rg -n -i \
  "qwen|qwencode|qwen-code|@qwen-code|QWEN_|\\.qwen|qwenlm|QwenLM" \
  --glob '!node_modules/**' \
  --glob '!.git/**' \
  --glob '!coverage/**' \
  --glob '!repos/**'
...
| Complete removal of Qwen aliases breaks existing users | High | This plan intentionally chooses clean break. If compatibility becomes required, revise policy before Phase 2. |
...
| Generated `dist/` contains stale Qwen strings | Medium | Do not hand-edit first; rebuild from source after source migration. |
| Tests/snapshots encode Qwen names | Medium | Update tests alongside behavior; regenerate snapshots intentionally. |
...
1. `rg -i "qwen|qwencode|qwen-code|@qwen-code|QWEN_|\.qwen|qwenlm|QwenLM"` returns no unapproved matches outside ignored reference/vendor areas.
