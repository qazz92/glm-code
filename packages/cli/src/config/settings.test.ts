...
import { GLM_DIR } from '@glm-code/core';
...
// must keep going through `getUserSettingsPath()` to pick up `GLM_HOME`
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    it('should add version field to V2 settings without version and write to disk', () => {
      ...
      expect(writtenContent.model?.name).toBe('glm-coder');
    ...
    it('should correctly handle partially migrated settings without version field', () => {
      ...
      expect(writtenContent.model).toEqual({ name: 'glm-coder' });
    ...
    describe('when GLM_CODE_SYSTEM_SETTINGS_PATH is set', () => {
      const MOCK_ENV_SYSTEM_SETTINGS_PATH = '/mock/env/system/settings.json';

      beforeEach(() => {
        process.env['GLM_CODE_SYSTEM_SETTINGS_PATH'] =
          MOCK_ENV_SYSTEM_SETTINGS_PATH;
      });

      afterEach(() => {
        delete process.env['GLM_CODE_SYSTEM_SETTINGS_PATH'];
      });
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      const geminiEnvPath = path.resolve(path.join(GLM_DIR, '.env'));
    ...
    describe('settings.env field', () => {
      ...
      it('should never set GLM_HOME or GLM_RUNTIME_DIR from settings.env', () => {
        ...
        const userSettingsContent: Settings = {
          env: {
            GLM_HOME: '/redirected/by/settings',
            GLM_RUNTIME_DIR: '/redirected/runtime',
            HARMLESS_VAR: 'ok',
          },
        };
        ...
        expect(process.env['GLM_HOME']).toBeUndefined();
    ...
    describe('GLM_HOME custom directory', () => {
      const originalGLMHome = process.env['GLM_HOME'];

      beforeEach(() => {
        delete process.env['DEBUG'];
        delete process.env['DEBUG_MODE'];
        delete process.env['GLM_HOME_TEST_VAR'];
      });

      afterEach(() => {
        if (originalGLMHome === undefined) {
          delete process.env['GLM_HOME'];
        } else {
          process.env['GLM_HOME'] = originalGLMHome;
        }
        delete process.env['DEBUG'];
        delete process.env['DEBUG_MODE'];
        delete process.env['GLM_HOME_TEST_VAR'];
      ...
      it('does not exclude DEBUG/DEBUG_MODE from .env in a GLM_HOME dir not named .glm', () => {
        const customHome = '/tmp/glm-home-custom';
        process.env['GLM_HOME'] = customHome;
        ...
        // because the .env lives inside the user-level GLM_HOME directory.
        expect(process.env['DEBUG']).toEqual('true');
        expect(process.env['DEBUG_MODE']).toEqual('1');
        expect(process.env['GLM_HOME_TEST_VAR']).toEqual('hello');
      });

      it('ignores GLM_HOME and GLM_RUNTIME_DIR set in a project .env', () => {
        delete process.env['GLM_HOME'];
        delete process.env['GLM_RUNTIME_DIR'];
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            ...
            if (p === projectEnvPath)
              return [
                'GLM_HOME=/tmp/hijack',
                'GLM_RUNTIME_DIR=/tmp/hijack-runtime',
                'OTHER_VAR=ok',
              ].join('\n');
            return '{}';
          },
      ...
      it('still honors GLM_HOME from a user-level .env (~/.glm/.env)', () => {
        delete process.env['GLM_HOME'];

        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue('/mock/home/user');
        const userGLMEnvPath = path.join('/mock/home/user', GLM_DIR, '.env');
        ...
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
          [USER_SETTINGS_PATH, userGLMEnvPath].includes(p.toString()),
        );
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === userGLMEnvPath) return 'GLM_HOME=/tmp/from-user-env';
            return '{}';
          },
        );
        ...
        expect(process.env['GLM_HOME']).toEqual('/tmp/from-user-env');
        cwdSpy.mockRestore();
      });

      it('does not exclude DEBUG/DEBUG_MODE from a workspace .glm/.env', () => {
        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue(MOCK_WORKSPACE_DIR);
        const workspaceGLMEnvPath = path.join(
          MOCK_WORKSPACE_DIR,
          GLM_DIR,
          '.env',
        );
        ...
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
          [USER_SETTINGS_PATH, workspaceGLMEnvPath].includes(p.toString()),
        );
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === workspaceGLMEnvPath)
              return 'DEBUG=true\nDEBUG_MODE=1\nGLM_HOME_TEST_VAR=hello';
            return '{}';
          },
        );
        ...
        // Per docs, `.glm/.env` files are never filtered by excludedEnvVars,
      ...
      it('still blocks GLM_HOME from a workspace .glm/.env', () => {
        delete process.env['GLM_HOME'];
        delete process.env['GLM_RUNTIME_DIR'];
        ...
        // A workspace `.glm/.env` is exempt from `excludedEnvVars` but must
      ...
      it('redirects user settings path when GLM_HOME is set in ~/.glm/.env', () => {
        delete process.env['GLM_HOME'];
        ...
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
          [userGLMEnvPath, customSettingsPath].includes(p.toString()),
        );
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === userGLMEnvPath) return 'GLM_HOME=/tmp/from-user-env';
        ...
        // The pre-pass propagates GLM_HOME from ~/.glm/.env into
      ...
      it('warns when GLM_HOME redirects but the legacy ~/.glm still has settings', () => {
        const customHome = '/tmp/glm-home-fresh';
        process.env['GLM_HOME'] = customHome;
        ...
        // Active GLM_HOME has nothing yet; legacy ~/.glm has settings.json.
        ...
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) => {
          const s = p.toString();
          // Legacy settings exist, active GLM_HOME settings do not.
        ...
        const warningMatch = loaded.migrationWarnings.find((w) =>
          w.includes('GLM_HOME points to'),
        );
        expect(warningMatch).toBeDefined();
        expect(warningMatch).toContain(customHome);
        expect(warningMatch).toContain(path.join('/mock/home/user', GLM_DIR));
      });

      it('does not warn when GLM_HOME points to a directory with settings.json', () => {
        const customHome = '/tmp/glm-home-migrated';
        process.env['GLM_HOME'] = customHome;
        const customSettingsPath = path.join(customHome, 'settings.json');
      ...
      it('does not warn when GLM_HOME is unset (default ~/.glm)', () => {
        delete process.env['GLM_HOME'];
        const legacySettings = path.join(
          '/mock/home/user',
          GLM_DIR,
      ...
      it('prefers GLM_HOME/.env over ~/.env at the home-dir step', () => {
        const customHome = '/tmp/glm-home-custom';
        process.env['GLM_HOME'] = customHome;
        const customGlobalEnvPath = path.join(customHome, '.env');
        ...
        (fs.readFileSync as Mock).mockImplementation(
          (p: fs.PathOrFileDescriptor) => {
            if (p === USER_SETTINGS_PATH) return JSON.stringify({});
            if (p === customGlobalEnvPath)
              return 'GLM_HOME_TEST_VAR=fromGLMHome';
            if (p === homeEnvPath) return 'GLM_HOME_TEST_VAR=fromHomeDir';
            return '{}';
          },
        );
        ...
        // GLM_HOME/.env must win — without the precedence fix, ~/.env would
        // be returned by the walk-up before the GLM_HOME fallback was ever
        // consulted.
        expect(process.env['GLM_HOME_TEST_VAR']).toEqual('fromGLMHome');
      });

      it('falls back to legacy ~/.glm/.env for non-routing keys when <GLM_HOME>/.env is absent', () => {
        // User keeps OPENAI_API_KEY in ~/.glm/.env and adds GLM_HOME to the
        ...
        const cwdSpy = vi
          .spyOn(process, 'cwd')
          .mockReturnValue('/mock/home/user');
        const customHome = '/tmp/glm-home-fresh-fallback';
        const userGLMEnvPath = path.join('/mock/home/user', GLM_DIR, '.env');
        const customSettingsPath = path.join(customHome, 'settings.json');
        ...
        // Only the legacy ~/.glm/.env exists; <GLM_HOME>/.env, the active
        // settings.json under <GLM_HOME>, and ~/.env all do not.
        (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
          [USER_SETTINGS_PATH, customSettingsPath, userGLMEnvPath].includes(
            p.toString(),
          ),
        );
        (fs.readFileSync as Mock).mockImplementation(
        ...
        expect(process.env['GLM_HOME']).toEqual(customHome);
