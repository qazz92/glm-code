...
export async function parseArguments(): Promise<CliArgs> {
  ...
  if (
    rawArgv.length > 0 &&
    (rawArgv[0].endsWith('/dist/glm-cli/cli.js') ||
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('glm')
    .usage(
      'Usage: glm [options] [command]\n\nGLM Code - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description: 'Proxy for GLM Code, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch GLM Code CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('disabled-slash-commands', {
          ...
          description:
            ...
            'and GLM_DISABLED_SLASH_COMMANDS. Matched case-insensitively ' +
        ...
        .check((argv: { [x: string]: unknown }) => {
          ...
          if (argv['jsonSchema']) {
            ...
            const hasPositionalQuery = Array.isArray(query)
              ? query.length > 0
              : !!query;
            // Allow stdin piping (`echo "..." | glm --json-schema ...`):
    ...
    // Register Plugin subcommands (alias for extensions)
    ...
    // Register `glm serve` (Stage 1 daemon — see issue #3803)
    .command(serveCommand)
    // Register `glm workflow` subcommands (autopilot, ralph, ultrawork, etc.)
    .command(workflowCommand)
    // Register `glm export` (session export to markdown/json)
    .command(exportCommand)
    // Register `glm gc` (garbage collect old sessions and temp files)
    .command(gcCommand)
    // Register `glm doctor` (environment diagnostics)
  ...
  if (
    ...
    // forever (after the listener is up); SIGINT/SIGTERM in runGLMServe
...
export async function loadCliConfig(
  ...
  // Set runtime output directory from settings (env var GLM_RUNTIME_DIR
  // is auto-detected inside getRuntimeBaseDir() at each call site).
  // Pass cwd so that relative paths like ".glm" resolve per-project.
  ...
  for (const name of (process.env['GLM_DISABLED_SLASH_COMMANDS'] ?? '').split(
    ',',
  )) {
    addDisabled(name);
  }
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`glm --resume\` without an ID to choose from existing sessions.`;
