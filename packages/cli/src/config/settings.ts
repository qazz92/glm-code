...
import {
  readClaudeSettings,
  mergeClaudeCompat,
} from './claude-settings-compat.js';
...
export const SETTINGS_DIRECTORY_NAME = GLM_DIR;

// Lazy getters: must NOT be top-level consts. `GLM_HOME` may be resolved
// from `~/.env` or `~/.glm/.env` by `preResolveHomeEnvOverrides()` in
...
// GLM_HOME and GLM_RUNTIME_DIR control where global state (settings, OAuth
...
const PROJECT_ENV_HARDCODED_EXCLUSIONS = ['GLM_HOME', 'GLM_RUNTIME_DIR'];
...
export function getSystemSettingsPath(): string {
  if (process.env['GLM_CODE_SYSTEM_SETTINGS_PATH']) {
    return process.env['GLM_CODE_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/GLMCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\glm-code\\settings.json';
  } else {
    return '/etc/glm-code/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['GLM_CODE_SYSTEM_DEFAULTS_PATH']) {
    return process.env['GLM_CODE_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
 ...
 *
 * User-level paths cover the home `.env` and the global GLM config dir
 * `.env` (which respects `GLM_HOME`). When `GLM_HOME` redirects elsewhere,
 * the legacy `<homedir>/.glm/.env` is also included so credentials users
...
function getUserLevelEnvPaths(): Set<string> {
  ...
  const paths = new Set([
    ...
    path.normalize(path.join(globalGLMDir, '.env')),
  ]);
  const legacyGLMEnv = path.normalize(path.join(homeDir, GLM_DIR, '.env'));
  paths.add(legacyGLMEnv);
  return paths;
}

/**
 * Pre-resolves GLM_HOME and GLM_RUNTIME_DIR from user-level `.env` files
 * before any settings or storage paths are read. Required because
 * module-load `Storage.getGlobalGLMDir()` would otherwise snapshot legacy
 ...
 * global state between `~/.glm/...` and `<GLM_HOME>/...`.
 *
...
export function preResolveHomeEnvOverrides(): void {
  ...
  // Storage.getGlobalGLMDir() shares the same homedir resolution as the
  // rest of the storage layer; when GLM_HOME is unset it equals
  // `<homedir>/.glm`, so path.dirname() recovers `<homedir>`.
  const initialGLMHome = process.env['GLM_HOME'];
  const initialGLMDir = Storage.getGlobalGLMDir();
  const candidates: string[] = [path.join(initialGLMDir, '.env')];
  if (!initialGLMHome) {
    candidates.push(path.join(path.dirname(initialGLMDir), '.env'));
  }
  ...
  if (discoveredGLMHome && discoveredGLMHome !== initialGLMHome) {
    const discoveredDir = Storage.getGlobalGLMDir();
...
/**
 * Surfaces a one-shot warning when GLM_HOME has been redirected but the
 ...
 */
function detectGLMHomeRedirectWithoutMigration(
  activeUserSettingsPath: string,
): string | null {
  if (!process.env['GLM_HOME']) {
    return null;
  }
  // Compute the legacy path by briefly unsetting GLM_HOME so Storage uses
  ...
  const activeGLMDir = Storage.getGlobalGLMDir();
  const savedGLMHome = process.env['GLM_HOME'];
  delete process.env['GLM_HOME'];
  let legacyGLMDir: string;
  try {
    legacyGLMDir = Storage.getGlobalGLMDir();
  } finally {
    process.env['GLM_HOME'] = savedGLMHome;
  }
  if (path.resolve(activeGLMDir) === path.resolve(legacyGLMDir)) {
    return null;
  }
  if (fs.existsSync(activeUserSettingsPath)) {
    return null;
  }
  const legacyUserSettings = path.join(legacyGLMDir, 'settings.json');
  if (!fs.existsSync(legacyUserSettings)) {
    return null;
  }
  return (
    `GLM_HOME points to "${activeGLMDir}" but no settings.json was found there. ` +
    `Existing config remains at "${legacyGLMDir}" — OAuth tokens, settings, memory, ` +
...
function findEnvFile(
  ...
  const globalGLMDir = Storage.getGlobalGLMDir();
  const legacyGLMDir = path.normalize(path.join(homeDir, GLM_DIR));
  const hasCustomConfigDir = path.normalize(globalGLMDir) !== legacyGLMDir;

  const canUseEnvFile = (filePath: string): boolean =>
    isTrusted !== false || userLevelPaths.has(path.normalize(filePath));

  // Home-dir candidates in priority order: globalGLMDir/.env, then legacy
  // ~/.glm/.env (only when GLM_HOME redirects), then ~/.env.
  // Users who add `GLM_HOME=` to an existing global env file shouldn't lose
  ...
  const findHomeCandidate = (): string | null => {
    const candidates = [path.join(globalGLMDir, '.env')];
    if (hasCustomConfigDir) {
      candidates.push(path.join(legacyGLMDir, '.env'));
    }
  ...
  while (true) {
    if (currentDir === homeDir) {
      ...
      // Workspace step: prefer .glm/.env, then plain .env.
      const geminiEnvPath = path.join(currentDir, GLM_DIR, '.env');
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    try {
      ...
      // homeScoped: `.env` lives under the user's home GLM dir or `~/.env` —
      //   only these may set GLM_HOME / GLM_RUNTIME_DIR.
      // glmScoped: any `.env` whose immediate parent is `.glm` (including
      //   `<repo>/.glm/.env`) — exempt from the user `excludedEnvVars` list.
      const isHomeScopedEnvFile = userLevelPaths.has(normalizedEnvFilePath);
      const isGLMScopedEnvFile =
        isHomeScopedEnvFile ||
        path.basename(path.dirname(normalizedEnvFilePath)) === GLM_DIR;
      ...
      for (const key in parsedEnv) {
        if (Object.hasOwn(parsedEnv, key)) {
          ...
          if (!isGLMScopedEnvFile && excludedVars.includes(key)) {
            continue;
          }
...
export function loadSettings(
  ...
  // Apply any GLM_HOME / GLM_RUNTIME_DIR set in user-level `.env` files
  // BEFORE any code reads a path derived from them. After this call, the
  // lazy `getUserSettingsPath()` / `Storage.getGlobalGLMDir()` getters
  // return the post-bootstrap value.
  preResolveHomeEnvOverrides();
  const userSettingsPath = getUserSettingsPath();
  const glmHomeRedirectWarning =
    detectGLMHomeRedirectWithoutMigration(userSettingsPath);
  ...
  // Read .claude/settings.json, .mcp.json, ~/.claude.json etc. and
  // deep-merge their mcpServers/hooks/permissions as lower-priority
  // fallbacks under the GLM settings.
  try {
    const claudeCompat = readClaudeSettings(resolvedWorkspaceDir);
    if (claudeCompat.sources.length > 0) {
      // Global Claude settings → user scope (lower priority than .glm)
      mergeClaudeCompat(
        userSettings as Record<string, unknown>,
        claudeCompat.settings,
      );
      // Project Claude settings → workspace scope (lower priority than .glm)
      mergeClaudeCompat(
    ...
    debugLogger.warn(`Claude compat layer failed: ${e}`);
  }
  ...
  const allMigrationWarnings: string[] = [
    ...(glmHomeRedirectWarning ? [glmHomeRedirectWarning] : []),
...
// ---------------------------------------------------------------------------
// Settings file watcher for MCP server hot-reload
// ---------------------------------------------------------------------------

let settingsWatcher: fs.FSWatcher | null = null;

/**
 * Watch the global settings file for changes.
 * On change, invokes the provided callback (e.g. MCP server reload).
...
export function watchSettingsFile(
  ...
  try {
    ...
    // Watch the directory (not the file) — file watches can miss
    // atomic saves that do write+rename.
    settingsWatcher = fs.watch(settingsDir, (_event, filename) => {
      if (filename === path.basename(settingsPath)) {
        debugLogger.debug(`Settings file changed, triggering callback`);
        onChange();
      }
    });
    ...
    settingsWatcher.on('error', (err) => {
      debugLogger.warn(
        `Settings watcher error: ${err instanceof Error ? err.message : String(err)}`,
      );
    });
  } catch (err) {
    debugLogger.warn(
      `Failed to start settings watcher: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
 ...
 */
export function unwatchSettingsFile(): void {
  if (settingsWatcher) {
