...
import { AuthType, OutputFormat } from '@glm-code/core';
...
function createMockConfig(overrides?: Partial<Config>): Config {
  const baseModelsConfig = {
    ...
    getCurrentAuthType: vi.fn().mockReturnValue(AuthType.GLM_OAUTH),
...
describe('validateNonInterActiveAuth', () => {
  ...
  let originalEnvGLMOauth: string | undefined;
  ...
  beforeEach(() => {
    ...
    originalEnvGLMOauth = process.env['GLM_OAUTH'];
  ...
  afterEach(() => {
    ...
    if (originalEnvGLMOauth !== undefined) {
      process.env['GLM_OAUTH'] = originalEnvGLMOauth;
    } else {
      delete process.env['GLM_OAUTH'];
    }
  ...
  it('exits with error for GLM_OAUTH (free tier discontinued)', async () => {
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
        getModel: vi.fn().mockReturnValue('default-model'),
  ...
  it('skips validation if useExternalAuth is true', async () => {
    ...
    expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.GLM_OAUTH);
  });
  ...
  it('exits if currentAuthType does not match enforcedAuthType', async () => {
    mockSettings.merged.security!.auth!.enforcedType = AuthType.GLM_OAUTH;
    process.env['OPENAI_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createMockConfig({
      refreshAuth: refreshAuthMock,
      getModelsConfig: vi.fn().mockReturnValue({
    ...
    expect(mockWriteStderrLine).toHaveBeenCalledWith(
      'The configured auth type is glm-oauth, but the current auth type is openai. Please re-authenticate with the correct type.',
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
    ...
    it('emits error result and exits when enforced auth mismatches current auth', async () => {
      ...
      expect(emitResultMock).toHaveBeenCalledWith({
        ...
        errorMessage: expect.stringContaining(
          'The configured auth type is glm-oauth, but the current auth type is openai.',
        ),
        durationMs: 0,
        apiDurationMs: 0,
        numTurns: 0,
