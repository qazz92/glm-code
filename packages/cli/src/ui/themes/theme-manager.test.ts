...
describe('ThemeManager', () => {
  ...
  describe('auto theme detection', () => {
    it('should select GLM Dark when terminal is detected as dark', () => {
      ...
      expect(themeManager.getActiveTheme().name).toBe('GLM Dark');
    });

    it('should select GLM Light when terminal is detected as light', () => {
      vi.mocked(detectModule.detectTerminalTheme).mockReturnValue('light');
      const result = themeManager.setActiveTheme(AUTO_THEME_NAME);
      expect(result).toBe(true);
      expect(themeManager.getActiveTheme().name).toBe('GLM Light');
    });
    ...
    it('should resolve async auto theme with GLM Light for light', async () => {
    ...
    it('should resolve async auto theme with GLM Dark for dark', async () => {
      vi.mocked(detectModule.detectTerminalThemeAsync).mockResolvedValue(
        'dark',
      );
      await themeManager.resolveAutoThemeAsync();
