...
import { GLMLight } from './glm-light.js';
import { GLMDark } from './glm-dark.js';
...
export const DEFAULT_THEME: Theme = GLMDark;
...
class ThemeManager {
   ...
   *   GLM Dark or GLM Light accordingly.
  ...
  private resolveAutoTheme(): Theme {
    ...
    return detected === 'light' ? GLMLight : GLMDark;
  }
  ...
  async resolveAutoThemeAsync(): Promise<void> {
    ...
    this.activeTheme = detected === 'light' ? GLMLight : GLMDark;
  ...
  getAvailableThemes(): ThemeDisplay[] {
    ...
    // Separate GLM themes
    const glmThemes = builtInThemes.filter(
      (theme) => theme.name === GLMLight.name || theme.name === GLMDark.name,
    );
    ...
    // Combine GLM themes first, then sorted others
    return [...glmThemes, ...sortedOtherThemes];
  }
