...
export const AppContainer = (props: AppContainerProps) => {
   ...
   *
   * In legacy blocking mode (`GLM_CODE_LEGACY_MCP_BLOCKING=1`) MCP
  ...
  useEffect(() => {
    ...
    // `debugLogger.warn` so it shows up under `GLM_CODE_DEBUG=1` and in
  ...
  const performMemoryRefresh = useCallback(async () => {
    historyManager.addItem(
      {
        type: MessageType.INFO,
        text: 'Refreshing hierarchical memory (GLM.md or other context files)...',
      },
      Date.now(),
    );
    try {
  ...
  // Update terminal title with GLM Code status and thoughts
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because GLM Code is already doing that elsewhere
  }, [
    streamingState,
    thought,
    settings.merged.ui?.showStatusInTitle,
