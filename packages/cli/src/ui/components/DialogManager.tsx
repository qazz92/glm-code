...
export const DialogManager = ({
  ...
  if (uiState.auth.isAuthenticating) {
    ...
    // GLM OAuth remains as a separate flow
    if (uiState.auth.pendingAuthType === AuthType.GLM_OAUTH) {
      return (
        <GLMOAuthProgress
          deviceAuth={uiState.auth.glmAuthState.deviceAuth || undefined}
          authStatus={uiState.auth.glmAuthState.authStatus}
          authMessage={uiState.auth.glmAuthState.authMessage}
          onTimeout={() => {
            uiActions.auth.onAuthError('GLM OAuth authentication timed out.');
            uiActions.auth.cancelAuthentication();
            uiActions.auth.setAuthState(AuthState.Updating);
          }}
          onCancel={() => {
