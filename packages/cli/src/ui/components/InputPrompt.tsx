...
import {
  formatFileSize,
  isImageFilePath,
} from '../hooks/useImageAttachments.js';
...
export const InputPrompt: React.FC<InputPromptProps> = ({
    ...
    try {
      ...
      if (hasImage) {
        ...
        if (imagePath) {
          ...
          // Read file size for display
          let sizeInBytes = 0;
          try {
            const stat = await fs.stat(imagePath);
            sizeInBytes = stat.size;
          } catch {
            // Size unavailable — show 0B
          }
  ...
  const handleImageFileDrop = useCallback(
    async (filePath: string) => {
      try {
        const resolvedPath = filePath.replace(/^file:\/\//, '');
        ...
        try {
          ...
          // File doesn't exist, use original destPath
        }
        ...
        debugLogger.error('Error handling image file drop:', error);
      }
    },
    [],
  );
  ...
  const handleInput = useCallback(
    (key: Key): boolean => {
      ...
      if (key.paste) {
        ...
        // Check if the pasted content is a single-line image file path (drag-and-drop)
        const trimmedPaste = pasted.trim();
        const unquotedPaste = trimmedPaste.match(/^'(.*)'$/)
          ? trimmedPaste.match(/^'(.*)'$/)![1]
          : trimmedPaste;
        const potentialPath = unquotedPaste.trim();
        if (
          charCount <= LARGE_PASTE_CHAR_THRESHOLD &&
          lineCount === 1 &&
          potentialPath.length >= 3 &&
          isImageFilePath(potentialPath)
        ) {
          // Image file drag-and-drop — attach instead of inserting @ref
          handleImageFileDrop(potentialPath);
          return true;
        }
      ...
      // Toggle dashboard overlay with Ctrl+G
      if (key.ctrl && key.name === 'g' && onToggleDashboard) {
        onToggleDashboard();
        return true;
      }
      ...
      // Hide dashboard on any key press (except Ctrl+G which is handled above)
      if (showDashboard && onToggleDashboard && !(key.ctrl && key.name === 'g')) {
        onToggleDashboard();
      }
      ...
      {attachments.length > 0 && (
        <Box marginLeft={2} marginBottom={0}>
          ...
          {attachments.map((att, idx) => (
            ...
            >
              [{idx + 1}] {att.filename} ({formatFileSize(att.sizeInBytes)})
