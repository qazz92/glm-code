...
const MODEL_PROVIDERS_DOCUMENTATION_URL =
  'https://docs.z.ai/glm-code-docs/en/users/configuration/settings/#modelproviders';
...
export function ArenaStartDialog({
  ...
  const modelItems = useMemo(() => {
    ...
    return selectableModels.map((model) => {
      ...
      const isGLMOauth = model.authType === AuthType.GLM_OAUTH;
      return {
        key: token,
        value: token,
  ...
  const hasDisabledGLMOauth = modelItems.some((item) => item.disabled);
  ...
  return (
    ...
    >
      ...
      {(hasDisabledGLMOauth || needsMoreModels) && (
        <Box marginTop={1} flexDirection="column">
          {hasDisabledGLMOauth && (
            <Text color={theme.status.warning}>
              {t('Note: glm-oauth models are not supported in Arena.')}
            </Text>
          )}
          {needsMoreModels && (
            <>
