...
const defaultProps = {
  version: '1.0.0',
  authDisplayType: AuthDisplayType.GLM_OAUTH,
  model: 'glm-coder-plus',
...
describe('<Header />', () => {
  ...
  it('displays auth type and model', () => {
    const { lastFrame } = render(<Header {...defaultProps} />);
    expect(lastFrame()).toContain('GLM OAuth');
    expect(lastFrame()).toContain('glm-coder-plus');
  });
  ...
  it('renders the custom subtitle in place of the blank spacer row', () => {
    ...
    const titleIdx = frame.indexOf('>_ GLM Code');
    const subtitleIdx = frame.indexOf('Built-in DataWorks Official Skills');
    const authIdx = frame.indexOf('GLM OAuth');
  ...
  it('keeps the blank spacer row when no subtitle is set (back-compat)', () => {
    ...
    expect(frame).toContain('>_ GLM Code');
    expect(frame).toContain('GLM OAuth');
  });
  ...
  it('hides the logo column when neither custom tier fits — does NOT fall back to the default GLM logo (preserves white-label intent)', () => {
    const { lastFrame } = render(
      <Header
        {...defaultProps}
  ...
  it('falls back to the default GLM logo when no custom art was provided at all', () => {
    useTerminalSizeMock.mockReturnValue({ columns: 60, rows: 24 });
    const { lastFrame } = render(<Header {...defaultProps} />);
    // With no customAsciiArt, narrow widths still hide the GLM logo, but a
