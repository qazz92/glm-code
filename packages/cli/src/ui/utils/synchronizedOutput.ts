...
export function terminalSupportsSynchronizedOutput(
  ...
  if (
    env['GLM_CODE_DISABLE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['GLM_CODE_SYNCHRONIZED_OUTPUT'] === '0'
  ) {
    return false;
  }

  if (
    env['GLM_CODE_FORCE_SYNCHRONIZED_OUTPUT'] === '1' ||
    env['GLM_CODE_SYNCHRONIZED_OUTPUT'] === '1'
  ) {
    return true;
  }
