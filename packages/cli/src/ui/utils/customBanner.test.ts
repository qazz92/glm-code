...
function makeSettings(opts: {
  ...
  return {
    ...
    user: opts.userUi
      ? file(
          { ui: opts.userUi },
          opts.userPath ?? '/home/u/.glm/settings.json',
        )
      : empty,
    workspace: opts.workspaceUi
      ? file(
          { ui: opts.workspaceUi },
          opts.workspacePath ?? '/repo/.glm/settings.json',
        )
      : empty,
...
describe('resolveCustomBanner', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'glm-banner-'));
  });
  ...
  it('rejects {path} mixed with tier keys (mutually exclusive object branches)', () => {
    const out = resolveCustomBanner(
      makeSettings({
        ...
        userPath: '/home/u/.glm/settings.json',
      }),
    );
