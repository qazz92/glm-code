...
describe('AuthDialog', { timeout: 15000 }, () => {
  ...
  beforeEach(() => {
    ...
    process.env['GLM_DEFAULT_AUTH_TYPE'] = '';
    vi.clearAllMocks();
  });
  ...
  describe('GEMINI_API_KEY environment variable', () => {
    ...
    it('should not show the GEMINI_API_KEY message if GLM_DEFAULT_AUTH_TYPE is set to something else', () => {
      process.env['GEMINI_API_KEY'] = 'foobar';
      process.env['GLM_DEFAULT_AUTH_TYPE'] = AuthType.USE_OPENAI;
    ...
    it('should show the GEMINI_API_KEY message if GLM_DEFAULT_AUTH_TYPE is set to use api key', () => {
      process.env['GEMINI_API_KEY'] = 'foobar';
      process.env['GLM_DEFAULT_AUTH_TYPE'] = AuthType.USE_OPENAI;
  ...
  describe('GLM_DEFAULT_AUTH_TYPE environment variable', () => {
    it('should select the auth type specified by GLM_DEFAULT_AUTH_TYPE', () => {
      // GLM_OAUTH is the only valid AuthType that can be selected via env var
      // API-KEY is not an AuthType enum value, so it cannot be selected this way
      process.env['GLM_DEFAULT_AUTH_TYPE'] = AuthType.GLM_OAUTH;
      ...
      // GLM_OAUTH maps to the OAuth entry in the four-flow main menu
      expect(lastFrame()).toContain('OAuth');
    });

    it('should fall back to default if GLM_DEFAULT_AUTH_TYPE is not set', () => {
      const settings: LoadedSettings = new LoadedSettings(
        {
          settings: {
            security: { auth: { selectedType: undefined } },
      ...
      // Default is Alibaba ModelStudio (first option); GLM OAuth is under OAuth.
      expect(lastFrame()).toContain('Alibaba ModelStudio');
    });

    it('should show an error and fall back to default if GLM_DEFAULT_AUTH_TYPE is invalid', () => {
      process.env['GLM_DEFAULT_AUTH_TYPE'] = 'invalid-auth-type';
      ...
      // Since the auth dialog doesn't show GLM_DEFAULT_AUTH_TYPE errors anymore,
    ...
    async () => {
      ...
      await vi.waitFor(
        () => {
          ...
          expect(frame).toContain('GLM_CUSTOM_API_KEY_');
          expect(frame).toContain('glm/glm3-coder');
