...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const glmIgnored =
      respectFileIgnore.respectGLMIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectGLMIgnore: true,
      });

    if (gitIgnored || glmIgnored) {
      const reason =
        gitIgnored && glmIgnored ? 'both' : gitIgnored ? 'git' : 'glm';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and glm'
          : reason === 'git'
            ? 'git-ignored'
            : 'glm-ignored';
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['glm'].length) {
      messages.push(`GLM-ignored: ${ignoredByReason['glm'].join(', ')}`);
    }
