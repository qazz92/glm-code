...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectGLMIgnore: () => true,
  ...
  describe('glm-ignore filtering', () => {
    it('should skip glm-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.glmignore'),
        'build/output.js',
      );
      const glmIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${glmIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${glmIgnoredFile} is glm-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nGLM-ignored: ${glmIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .glmignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.glmignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed glm-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.glmignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${glmIgnoredFile}`;
