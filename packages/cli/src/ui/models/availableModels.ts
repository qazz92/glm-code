...
const CACHED_GLM_OAUTH_MODELS: AvailableModel[] = GLM_OAUTH_MODELS.map(
  (model) => ({
    id: model.id,
...
function getGLMOAuthModels(): readonly AvailableModel[] {
  return CACHED_GLM_OAUTH_MODELS;
}
 ...
 */
export function getFilteredGLMModels(): AvailableModel[] {
  return [...getGLMOAuthModels()];
}
