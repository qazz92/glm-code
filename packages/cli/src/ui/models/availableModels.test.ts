...
import {
  ...
  getFilteredGLMModels,
  getOpenAIAvailableModelFromEnv,
} from './availableModels.js';
import { AuthType, type Config } from '@glm-code/core';
...
describe('availableModels', () => {
  ...
  describe('getAvailableModelsForAuthType', () => {
    ...
    it('should return hard-coded glm models for glm-oauth', () => {
      const models = getAvailableModelsForAuthType(AuthType.GLM_OAUTH);
      expect(models.length).toBe(1);
      expect(models[0].id).toBe('coder-model');
      expect(models[0].isVision).toBe(true);
    });

    it('should use config models for glm-oauth when config is provided', () => {
      const mockConfig = {
