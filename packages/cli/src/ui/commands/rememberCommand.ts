...
import { getAutoMemoryRoot } from '@glm-code/core';
...
export const rememberCommand: SlashCommand = {
  ...
  action: (context: CommandContext, args): SlashCommandActionReturn | void => {
    ...
    // Managed auto-memory is disabled: ask the agent to save to GLM.md
    // using its native file tools. We do not call save_memory because that
    // tool was removed.
    return {
      type: 'submit_prompt',
      content: `Please save the following fact to memory (e.g. append to GLM.md in the project root):\n\n${fact}`,
    };
  },
};
