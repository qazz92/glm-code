...
vi.mock('@glm-code/core', async (importOriginal) => {
  const original =
    (await importOriginal()) as typeof import('@glm-code/core');
  return {
    ...original,
...
describe('renameCommand', () => {
  ...
  describe('bare /rename model selection', () => {
    ...
    it('uses fastModel when configured', async () => {
      ...
      expect(generateText.mock.calls[0][0].model).toBe('glm-turbo');
    });
  ...
  describe('--auto flag', () => {
    ...
    it('refuses --auto combined with a positional name', async () => {
      const mockConfig = {
        ...
        getFastModel: vi.fn().mockReturnValue('glm-turbo'),
      };
      mockContext = createMockCommandContext({
        services: { config: mockConfig as never },
      });
