...
export const GITHUB_WORKFLOW_PATHS = [
  'glm-dispatch/glm-dispatch.yml',
  'glm-assistant/glm-invoke.yml',
  'issue-triage/glm-triage.yml',
  'issue-triage/glm-scheduled-triage.yml',
  'pr-review/glm-review.yml',
];
...
// Add GLM Code specific entries to .gitignore file
export async function updateGitignore(gitRepoRoot: string): Promise<void> {
  const gitignoreEntries = ['.glm/', 'gha-creds-*.json'];
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/qazz92/glm-code-action/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/qazz92/glm-code-action/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with GLM.',
        command,
        is_background: false,
      },
    };
