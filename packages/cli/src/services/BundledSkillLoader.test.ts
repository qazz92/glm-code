...
import type { Config, SkillConfig } from '@glm-code/core';
...
describe('BundledSkillLoader', () => {
  ...
  it('should resolve {{model}} template variable in skill body', async () => {
    const skill = makeSkill({
      body: 'Review by {{model}} via GLM Code',
    });
    mockSkillManager.listSkills.mockResolvedValue([skill]);
    ...
    expect(result).toEqual({
      ...
      content: [
        {
          text: 'YOUR_MODEL_ID="glm3-coder"\n\nReview by glm3-coder via GLM Code',
        },
      ],
    });
  });
