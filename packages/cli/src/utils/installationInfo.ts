...
import { createDebugLogger, isGitRepository } from '@glm-code/core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        childProcess.execSync('brew list -1 | grep -q "^glm-code$"', {
          stdio: 'ignore',
        });
        return {
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @glm-code/glm-code@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @glm-code/glm-code@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @glm-code/glm-code@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @glm-code/glm-code@latest';
    return {
