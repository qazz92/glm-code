...
const DEFAULT_BASE_URLS: Record<string, string> = {
  ...
  'glm-oauth': 'https://coding.dashscope.aliyuncs.com',
 ...
 *
 * @param authType - Authentication type (openai, glm-oauth, anthropic, etc.)
...
export function preconnectApi(
  ...
  // Check if disabled
  if (process.env['GLM_CODE_DISABLE_PRECONNECT'] === '1') {
  ...
  try {
    ...
    fetch(targetUrl, {
      ...
      headers: {
        'User-Agent': 'GLMCode-Preconnect/1.0',
      },
      dispatcher,
    } as RequestInit)
      .then(() => {
