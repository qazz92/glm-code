 ...
 *
 * Activated by setting GLM_CODE_PROFILE_STARTUP=1. When enabled, collects
 * high-resolution timestamps at key phases of CLI startup and writes a JSON
 * report to ~/.glm/startup-perf/ on finalization.
 *
 ...
 * reports. Set GLM_CODE_PROFILE_STARTUP_OUTER=1 to also profile the outer
...
import type { StartupEventAttrs } from '@glm-code/core';
...
export function initStartupProfiler(): void {
  ...
  if (process.env['GLM_CODE_PROFILE_STARTUP'] !== '1') {
    return;
  }

  const inSandboxChild = !!process.env['SANDBOX'];
  const outerOptIn = process.env['GLM_CODE_PROFILE_STARTUP_OUTER'] === '1';
  ...
  // Disable with GLM_CODE_PROFILE_STARTUP_NO_HEAP=1 when measuring the
  // Heisenberg overhead of the heap call itself.
  captureHeap = process.env['GLM_CODE_PROFILE_STARTUP_NO_HEAP'] !== '1';
  finalized = false;
...
export function finalizeStartupProfile(sessionId?: string): void {
  ...
  try {
    const dir = path.join(os.homedir(), '.glm', 'startup-perf');
