...
import { IdeClient, AuthType } from '@glm-code/core';
 ...
 * For bug reports, removes 'glm-' or 'glm-code-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'glm-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false): string {
  ...
  // For bug reports, remove glm- prefix
  if (stripPrefix) {
    return sandbox.replace(/^glm-(?:code-)?/, '');
  }
