...
import type { Config, SandboxConfig } from '@glm-code/core';
import {
  FatalSandboxError,
  Storage,
  isSubpath,
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'glm-code-sandbox';
const SANDBOX_NETWORK_NAME = 'glm-code-sandbox';
const SANDBOX_PROXY_NAME = 'glm-code-sandbox-proxy';
...
function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : process.env['DEBUG']
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which glm)`
        : 'glm';
...
export async function start_sandbox(
  ...
  if (config.command === 'sandbox-exec') {
    ...
    // on missing dirs and a custom GLM_HOME / GLM_RUNTIME_DIR may not exist
    // yet on first run.
    const glmDir = Storage.getGlobalGLMDir();
    ...
    const args = [
      ...
      '-D',
      `GLM_DIR=${fs.realpathSync(glmDir)}`,
      '-D',
    ...
    // start and set up proxy if GLM_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env['GLM_SANDBOX_PROXY_COMMAND'];
  ...
  // determine full path for glm-code to distinguish linked vs installed setting
  ...
  // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under glm-code repo
  //
  // note this can only be done with binary linked from glm-code repo
  if (process.env['BUILD_SANDBOX']) {
    if (!gcPath.includes('glm-code/packages/')) {
      throw new FatalSandboxError(
        'Cannot build sandbox using installed GLM Code binary; ' +
          'run `npm link ./packages/cli` under GLMCode-cli repo to switch to linked binary.',
      );
    } else {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            GLM_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  if (!(await ensureSandboxImageIsPresent(config.command, image))) {
    const remedy =
      image === LOCAL_DEV_SANDBOX_IMAGE_NAME
        ? 'Try running `npm run build:all` or `npm run build:sandbox` under the glm-code repo to build it locally, or check the image name and your network connection.'
        : 'Please check the image name, your network connection, or notify glm-code-dev@service.alibaba.com if the issue persists.';
    throw new FatalSandboxError(
  ...
  // Mount user settings at /home/node/.glm and at the canonical host path
  // used by GLM_HOME, unless that host path is already covered by a broader
  ...
  // Pass GLM_HOME so the sandboxed CLI resolves the global glm dir to the
  // same path the host did, instead of relying on the /home/node/.glm mount
  // being the default fallback.
  args.push('--env', `GLM_HOME=${userSettingsDirContainerPath}`);

  // Mount the runtime base dir and pass GLM_RUNTIME_DIR when it diverges
  // from the global glm dir; otherwise the existing user-settings mount
  // already covers it.
  ...
  if (!runtimeSameAsUserSettings) {
    args.push('--env', `GLM_RUNTIME_DIR=${runtimeBaseDirContainerPath}`);
  }
  ...
  // GLM_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env['GLM_SANDBOX_PROXY_COMMAND'];
  ...
  if (isIntegrationTest) {
    containerName = `glm-code-integration-test-${randomBytes(4).toString(
      'hex',
    )}`;
  ...
  // copy GLM_CODE_TEST_VAR for integration tests
  if (process.env['GLM_CODE_TEST_VAR']) {
    args.push(
      '--env',
      `GLM_CODE_TEST_VAR=${process.env['GLM_CODE_TEST_VAR']}`,
    );
  }
  ...
  // copy OPENAI_API_KEY and related env vars for GLM
  ...
  // tests that need to access host's ~/.glm (e.g., --resume functionality)
  ...
  if (useCurrentUser) {
    ...
    // allowing tests that need to access host's ~/.glm (e.g., --resume) to work.
    ...
    // finally switch to that user to run the glm process. This is
