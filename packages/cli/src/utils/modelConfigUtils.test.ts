...
describe('modelConfigUtils', () => {
  describe('getAuthTypeFromEnv', () => {
    ...
    it('should return GLM_OAUTH when GLM_OAUTH is set', () => {
      process.env['GLM_OAUTH'] = 'true';

      expect(getAuthTypeFromEnv()).toBe(AuthType.GLM_OAUTH);
    });
    ...
    it('should prioritize GLM_OAUTH over other auth types when explicitly set', () => {
      process.env['GLM_OAUTH'] = 'true';
      process.env['OPENAI_API_KEY'] = 'test-key';
      process.env['OPENAI_MODEL'] = 'gpt-4';
      process.env['OPENAI_BASE_URL'] = 'https://api.openai.com';

      // GLM_OAUTH is checked first, so it should be returned even when other auth vars are set
      expect(getAuthTypeFromEnv()).toBe(AuthType.GLM_OAUTH);
    });
  ...
  describe('resolveCliGenerationConfig', () => {
    ...
    it('should warn when top-level generationConfig fields are ignored for a provider model', () => {
      ...
      expect(result.warnings[0]).toContain(
        'provider model "glm3.6-27b" from modelProviders.openai',
      );
      expect(result.warnings[0]).toContain(
    ...
    // Edge Case 2: GLM_MODEL is used as final fallback when OPENAI_MODEL is not set
    it('Edge Case 2: GLM_MODEL should be used as fallback when OPENAI_MODEL is not set', () => {
      const argv = {};
      const glmProvider: ProviderModelConfig = {
        id: 'glm-env-model',
        name: 'GLM Env Model',
      };
      const settings = makeMockSettings({
      ...
      vi.mocked(resolveModelConfig).mockReturnValue({
        config: { model: 'glm-env-model', apiKey: '', baseUrl: '' },
        sources: {},
        warnings: [],
      });
      ...
      resolveCliGenerationConfig({
        ...
        settings,
        selectedAuthType,
        env: { GLM_MODEL: 'glm-env-model' },
      });

      expect(vi.mocked(resolveModelConfig)).toHaveBeenCalledWith(
        expect.objectContaining({
          modelProvider: glmProvider,
        }),
      );
    });

    // Edge Case 3: OPENAI_MODEL over GLM_MODEL when both are set and settings.model.name is not set
    it('Edge Case 3: OPENAI_MODEL should win over GLM_MODEL when both set', () => {
      const argv = {};
    ...
    describe('[Regression] model precedence', () => {
      ...
      it('[Regression] GLM_MODEL as fallback when OPENAI_MODEL not set', () => {
        const settings = makeMockSettings({ model: { name: undefined } });
        const selectedAuthType = AuthType.USE_OPENAI;
        const env = { GLM_MODEL: 'glm-model', OPENAI_API_KEY: 'key' };

        vi.mocked(resolveModelConfig).mockImplementation(() => ({
          config: { model: 'glm-model', apiKey: 'key', baseUrl: '' },
          sources: { model: { kind: 'env' as const, envKey: 'GLM_MODEL' } },
          warnings: [],
        }));
        ...
        // GLM_MODEL should be passed to resolveModelConfig
        const callArgs = vi.mocked(resolveModelConfig).mock.calls[0][0];
        expect(callArgs.env?.['GLM_MODEL']).toBe('glm-model');
      });
