...
describe('startupProfiler', () => {
  ...
  beforeEach(() => {
    ...
    vi.restoreAllMocks();
    saveEnv(
      'GLM_CODE_PROFILE_STARTUP',
      'GLM_CODE_PROFILE_STARTUP_OUTER',
      'GLM_CODE_PROFILE_STARTUP_NO_HEAP',
      'SANDBOX',
    );
    delete process.env['GLM_CODE_PROFILE_STARTUP'];
    delete process.env['GLM_CODE_PROFILE_STARTUP_OUTER'];
    delete process.env['GLM_CODE_PROFILE_STARTUP_NO_HEAP'];
  ...
  function enableProfiler() {
    process.env['GLM_CODE_PROFILE_STARTUP'] = '1';
  ...
  describe('when outside sandbox (SANDBOX not set)', () => {
    it('should not enable profiler even with GLM_CODE_PROFILE_STARTUP=1', () => {
      process.env['GLM_CODE_PROFILE_STARTUP'] = '1';
      delete process.env['SANDBOX'];
  ...
  describe('when enabled (GLM_CODE_PROFILE_STARTUP=1 + SANDBOX)', () => {
    beforeEach(() => {
      enableProfiler();
    });
    ...
    it('omits heap snapshots when GLM_CODE_PROFILE_STARTUP_NO_HEAP=1', () => {
      process.env['GLM_CODE_PROFILE_STARTUP_NO_HEAP'] = '1';
      initStartupProfiler();
      profileCheckpoint('phase_a');
      const report = getStartupReport()!;
  ...
  describe('outer-process opt-in (GLM_CODE_PROFILE_STARTUP_OUTER=1)', () => {
    ...
    it('collects outside sandbox when OUTER=1 and writes outer-prefixed file', () => {
      process.env['GLM_CODE_PROFILE_STARTUP'] = '1';
      process.env['GLM_CODE_PROFILE_STARTUP_OUTER'] = '1';
      delete process.env['SANDBOX'];
