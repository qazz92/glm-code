...
// CI / build status, existing GLM Code comment classification, and the
...
async function runPresubmit(args: PresubmitArgs): Promise<void> {
  ...
  const glmComments = allComments.filter((c) =>
    /via GLM Code \/review/.test(c.body ?? ''),
  );
