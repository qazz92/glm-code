...
// Centralised path constants and helpers for the `glm review` subcommands.
...
export const REVIEW_TMP_DIR = join('.glm', 'tmp');
export const REVIEWS_DIR = join('.glm', 'reviews');
export const REVIEW_CACHE_DIR = join('.glm', 'review-cache');
...
export function reviewBranch(prNumber: string | number): string {
  return `glm-review/pr-${prNumber}`;
}

/**
 * Per-target side-file path (review JSON, PR context, presubmit report).
 *
 * Files live under `.glm/tmp/` rather than the OS temp dir so the path is
