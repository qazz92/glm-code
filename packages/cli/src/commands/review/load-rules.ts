...
// `glm review load-rules`: read project-specific code-review rules from
...
// PR cannot inject `.glm/review-rules.md` content that bypasses scrutiny.
// Sources, in order:
//
//   1. `.glm/review-rules.md`
...
//   4. `GLM.md`   — only the `## Code Review` section
//
...
function loadCombined(baseRef: string): {
  ...
  // 1. GLM-native rules.
  const glmRules = showFile(baseRef, '.glm/review-rules.md');
  if (glmRules) {
    sections.push(`### From .glm/review-rules.md\n\n${glmRules.trim()}`);
    loaded.push('.glm/review-rules.md');
  }
  ...
  // 4. GLM.md — extract Code Review section only.
  const glmMd = showFile(baseRef, 'GLM.md');
  if (glmMd) {
    const section = extractCodeReviewSection(glmMd);
    if (section) {
      sections.push(`### From GLM.md\n\n${section}`);
      loaded.push('GLM.md');
    }
  }
...
export const loadRulesCommand: CommandModule = {
  ...
  describe:
    'Read project review rules from the base branch (.glm/review-rules.md, .github/copilot-instructions.md, AGENTS.md, GLM.md) and write a combined Markdown file',
  builder: (yargs) =>
    yargs
      .positional('base_ref', {
        type: 'string',
