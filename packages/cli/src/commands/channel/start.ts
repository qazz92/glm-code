...
import { normalizeProxyUrl, Storage } from '@glm-code/core';
import { loadSettings } from '../../config/settings.js';
import { writeStderrLine, writeStdoutLine } from '../../utils/stdioHelpers.js';
import { AcpBridge, SessionRouter } from '@glm-code/channel-base';
import type {
  ChannelBase,
  ChannelPlugin,
  ToolCallEvent,
...
function sessionsPath(): string {
  return path.join(Storage.getGlobalGLMDir(), 'channels', 'sessions.json');
}
 ...
 * Extensions declare channels in their glm-extension.json manifest.
 */
...
function checkDuplicateInstance(): void {
  ...
  if (existing) {
    ...
    writeStderrLine('Use "glm channel stop" to stop it first.');
    process.exit(1);
  }
}
