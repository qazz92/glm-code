...
export const buildRemovalNotice = (): string =>
  [
    '',
    yellow(t('⚠  glm auth has been removed.')),
    '',
    `  ${cyan(t('Interactive'))}   →  ${t('run glm and use /auth to configure providers')}`,
    ...
    `  ${cyan(t('GLM OAuth'))}    →  ${t('run glm interactively and use /auth; OAuth cannot be configured with env vars alone')}`,
    `  ${cyan(t('Scripted'))}      →  ${t('edit ~/.glm/settings.json, or run glm interactively once')}`,
    '',
