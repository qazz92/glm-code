...
function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  if (process.env['GLM_CODE_NO_RELAUNCH']) {
    return [];
  }
...
export async function main() {
  ...
  // `isStartupProfilerEnabled()` so that when GLM_CODE_PROFILE_STARTUP is
  ...
  if (process.argv.includes('--bare')) {
    process.env[GLM_CODE_SIMPLE_ENV_VAR] = '1';
  }
  ...
  // to run GLM Code. It is now safe to perform expensive initialization that
...

  {
    ...
    const startupWarnings = [
      ...new Set([
        ...
        AuthType.GLM_OAUTH
          ? [
              'GLM OAuth free tier was discontinued on 2026-04-15. Run /auth to switch to Coding Plan or another provider.',
            ]
          : []),
      ]),
    ];
    ...
    if (config.isInteractive()) {
      ...
      // can't catch this case (`glm --json-schema '...'` on a TTY with
      ...
      if (failedMcpServers.length > 0) {
        writeStderrLine(
          `Warning: MCP server(s) failed to start: ${failedMcpServers.join(', ')}. ` +
            ...
            `Re-run with GLM_CODE_DEBUG=1 to see per-server reasons.`,
        );
      }
