...
const MOCK_DATA: InsightData = {
  ...
  qualitative: {
    impressiveWorkflows: {
      intro:
        "You're a highly active GLM Code user with 314 sessions over 67 days, achieving strong results in test creation, PR workflows, and debugging with a 68% success rate on clear tasks.",
      impressive_workflows: [
        ...
        },
        {
          ...
          description:
            "You've mastered the PR creation workflow, successfully creating pull requests using existing templates multiple times. You know how to leverage GLM to format PRs properly in English and complete this task efficiently, often in single responses with multiple coordinated tool calls.",
        },
        {
          title: 'Systematic Debugging and Refactoring',
          description:
            "You effectively use GLM for debugging failing tests, successfully identifying issues like missing KeypressProvider context and ink mock setup problems. You also completed clean refactoring tasks like migrating openFileInBrowser to use the 'open' package, with all checks passing on the first attempt.",
        },
      ],
    },
    projectAreas: {
    ...
    frictionPoints: {
      ...
      categories: [
        ...
        },
        {
          ...
          description:
            "You encountered buggy code generation in 5 instances where GLM's initial implementations were incorrect, forcing you to spend additional time iterating on fixes before achieving working solutions.",
          examples: [
        ...
        },
        {
          ...
          description:
            "You experienced situations where GLM's tools or skills failed to execute properly, forcing manual workarounds and excessive exploration that extended task completion time.",
          examples: [
    ...
    memorableMoment: {
      headline:
        "User paused mid-work to verify GLM's corporate identity—\"Are you Alibaba's model or Zhipu's GLM?\"",
      detail:
        "During what appears to be routine development work, a user stopped everything to ask GLM to confirm its corporate origins. GLM correctly identified itself as Alibaba's model, not Zhipu's GLM. A rare moment of AI identity verification in the wild.",
    },
    improvements: {
      GLM_md_additions: [
        {
          ...
          prompt_scaffold:
            'Create new ## Code Review Standards section in GLM.md',
        },
        {
          addition:
          ...
          prompt_scaffold:
            'Create new ## Testing Conventions section in GLM.md',
        },
        {
          addition:
          ...
          prompt_scaffold: 'Create new ## PR Workflow section in GLM.md',
        },
      ],
      features_to_try: [
        {
        ...
        },
        {
          ...
          example_code:
            'Ask GLM: "Use an agent to explore the test infrastructure and identify all test utilities available"',
        },
      ],
      usage_patterns: [
        {
        ...
        },
        {
          ...
          copyable_prompt:
            'Create .glm/skills/pr/SKILL.md with your PR template requirements, then just type /pr',
        },
        {
        ...
        },
        {
          ...
          suggestion:
            'Add testing conventions to GLM.md to avoid iterative mock fixes',
          detail:
            '5 buggy_code friction points mostly came from test mock implementation issues. Documenting your testing patterns in GLM.md would help GLM generate correct tests on the first attempt.',
          copyable_prompt:
            "Add to GLM.md: 'When generating tests, use vi.mock() before vi.mocked() and always include required providers'",
        },
      ],
    },
    interactionStyle: {
      narrative:
        'Your interaction pattern shows **high-frequency, low-intensity engagement** with GLM Code. With 314 sessions over just 68 days (averaging 4-5 sessions daily) and 58% of analyzed sessions being "warmup_minimal," you treat GLM as a readily-available assistant you check in with frequently but often don\'t have specific tasks ready. You say "hi" or "nihao" and wait to see what happens, suggesting you keep GLM accessible as a background tool rather than planning extensive work sessions.\n\nWhen you do have actual work, you\'re **iterative and feedback-driven rather than specification-heavy**. Examples: when creating PRs, you accepted an initial implementation that created empty content, then reported the issue for GLM to fix; when implementing ESC cancellation support, you tested the code, discovered the signal wasn\'t being passed through, and reported the specific bug for correction; when requesting tests for DataProcessor, you worked through multiple rounds of fixing mock implementation issues. You don\'t provide exhaustive upfront requirements—you point GLM at a problem, see what it produces, and course-correct.\n\n**You trust GLM to explore autonomously but intervene when things go wrong.** Your tool usage (209 read_file calls, 166 shell commands, 98 edits) shows you let GLM investigate and modify freely. The friction data reveals low rejection rates (0 user_rejected_action, 0 excessive_changes) and zero misunderstood requests, indicating you give GLM space to work. However, 5 instances of buggy code required your feedback to fix, and you clearly communicate specific issues when they arise ("abort() was called but execution didn\'t stop"). You\'re a collaborative debugger who provides precise problem descriptions rather than vague complaints.',
      key_pattern:
        'You use GLM Code as a high-frequency, low-commitment tool with iterative feedback loops—many brief check-ins with "hi" or minimal interaction, and when real work happens, you prefer to test implementations and report specific issues for correction rather than providing comprehensive upfront specifications.',
    },
    atAGlance: {
      whats_working:
