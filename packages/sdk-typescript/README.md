...
A minimum experimental TypeScript SDK for programmatic access to GLM Code.
...
```typescript
import { query } from '@glm-code/sdk';
...
Creates a new query session with the GLM Code.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'glm-4.5'`, `'glm-4.5-air'`, `'glm-flash'`). Takes precedence over `OPENAI_MODEL` and `GLM_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                                 |
| `pathToGLMExecutable`   | `string`                                       | Auto-detected    | Path to the GLM Code executable. Supports multiple formats: `'glm'` (native binary from PATH), `'/path/to/glm'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `GLM_CODE_CLI_PATH` env var, `~/.volta/bin/glm`, `~/.npm-global/bin/glm`, `/usr/local/bin/glm`, `~/.local/bin/glm`, `~/node_modules/.bin/glm`, `~/.yarn/bin/glm`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the GLM Code process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                  |
| `systemPrompt`           | `string \| QuerySystemPromptPreset`            | -                | System prompt configuration for the main session. Use a string to fully override the built-in GLM Code system prompt, or a preset object to keep the built-in prompt and append extra instructions.                                                                                                                                                                                                                                                                                  |
...
| `authType`               | `'openai' \| 'glm-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'glm-oauth'` in SDK is not recommended as credentials are stored in `~/.glm` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                          |
...
```typescript
import { query, type SDKUserMessage } from '@glm-code/sdk';
...
```typescript
import { query, type CanUseTool } from '@glm-code/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@glm-code/sdk';
...
```typescript
import { query, isAbortError } from '@glm-code/sdk';
...
```typescript
import { AbortError, isAbortError } from '@glm-code/sdk';
...
#### GLM Code Installation Required

Version 0.1.0 requires [GLM Code](https://github.com/qazz92/glm-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install GLM Code globally
npm install -g glm-code@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate GLM Code installation is needed.
