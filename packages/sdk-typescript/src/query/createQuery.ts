...
export function query({
  ...
  /**
   * The prompt to send to the GLM Code CLI process.
  ...
  const pathToGLMExecutable = options.pathToGLMExecutable;
...
function validateOptions(options: QueryOptions): SpawnInfo | undefined {
  ...
  try {
    return prepareSpawnInfo(options.pathToGLMExecutable);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Invalid pathToGLMExecutable: ${errorMessage}`);
  }
}
