 ...
 * - WebSocket: tcp
 *
 * This interface aligns with MCPServerConfig in @glm-code/core.
 */
...
/**
 * Configuration options for creating a query session with the GLM CLI.
 */
export interface QueryOptions {
   ...
   * This takes precedence over the environment variables `OPENAI_MODEL` and `GLM_MODEL`
   * @example 'glm-max', 'glm-plus', 'glm-turbo'
   */
  model?: string;
   ...
   * - Command name (no path separators): `'glm'` -> executes from PATH
   ...
   * - Native binary: `'/path/to/glm'` -> executes directly
   *
   * Runtime detection:
  ...
  /**
   * Environment variables to pass to the GLM CLI process.
   * These variables will be merged with the current process environment.
   */
  env?: Record<string, string>;

  /**
   * System prompt configuration for the GLM CLI session.
   *
   * - `string`: fully overrides the main session system prompt
   * - `{ type: 'preset', preset: 'glm_code', append?: string }`:
   *   uses GLM Code's built-in prompt as the base and optionally appends extra
  ...
  /**
   * Custom handler for stderr output from the GLM CLI process.
   ...
   * - 'glm-oauth': Use GLM OAuth authentication
   *
   * Though we support 'glm-oauth', it's not recommended to use it in the SDK.
   * Because the credentials are stored in `~/.glm` and may need to refresh periodically.
   */
  authType?: AuthType;
   ...
   * This is equivalent to using the `--resume` flag in the GLM CLI.
