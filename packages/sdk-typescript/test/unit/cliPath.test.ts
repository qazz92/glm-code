...
describe('CLI Path Utilities', () => {
  ...
  describe('findBundledCliPath', () => {
    ...
    it('should throw descriptive error when bundled CLI not found', () => {
      ...
      expect(() => findBundledCliPath()).toThrow('Bundled glm CLI not found');
  ...
  describe('prepareSpawnInfo', () => {
    describe('auto-detection (no spec provided)', () => {
      ...
      it('should throw when bundled CLI not found', () => {
        ...
        expect(() => prepareSpawnInfo()).toThrow('Bundled glm CLI not found');
      });
    });

    describe('command name detection', () => {
      it('should detect command names without path separators', () => {
        const result = prepareSpawnInfo('glm');
      ...
      it('should detect command names on Windows', () => {
        const result = prepareSpawnInfo('glm.exe');

        expect(result).toEqual({
          command: 'glm.exe',
          args: [],
          type: 'native',
          originalInput: 'glm.exe',
        });
      });

      it('should reject invalid command name characters', () => {
        expect(() => prepareSpawnInfo('glm@invalid')).toThrow(
          "Invalid command name 'glm@invalid'. Command names should only contain letters, numbers, dots, hyphens, and underscores.",
        );
      });

      it('should accept valid command names', () => {
        expect(() => prepareSpawnInfo('glm')).not.toThrow();
        expect(() => prepareSpawnInfo('glm-code')).not.toThrow();
        expect(() => prepareSpawnInfo('glm_code')).not.toThrow();
        expect(() => prepareSpawnInfo('glm.exe')).not.toThrow();
        expect(() => prepareSpawnInfo('glm123')).not.toThrow();
      });
    });
    ...
    describe('native executables', () => {
      ...
      it('should prepare spawn info for native binary path', () => {
        const result = prepareSpawnInfo('/usr/local/bin/glm');

        expect(result).toEqual({
          command: path.resolve('/usr/local/bin/glm'),
          args: [],
          type: 'native',
          originalInput: '/usr/local/bin/glm',
        });
      });
    });
    ...
    describe('file path resolution', () => {
      it('should resolve absolute file paths', () => {
        ...
        const result = prepareSpawnInfo('/absolute/path/to/glm');

        expect(result.command).toBe(path.resolve('/absolute/path/to/glm'));
  ...
  describe('Windows path handling', () => {
    ...
    it('should handle Windows native executables', () => {
      const windowsPath = 'C:\\Program Files\\glm\\glm.exe';
      const result = prepareSpawnInfo(windowsPath);
