...
const GLM_LOCK_FILENAME = 'oauth_creds.lock';
...
interface MemoryCache {
  credentials: GLMCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid GLMCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): GLMCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<GLMCredentials>;
  ...
  return creds as GLMCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<GLMCredentials> | null = null;
   ...
   *
   * @param glmClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    try {
      ...
      await this.checkAndReloadIfNeeded(glmClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(glmClient, now);
  ...
  private async performFileCheck(
    glmClient: IGLMOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(glmClient);
   ...
   */
  private async forceFileCheck(glmClient?: IGLMOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with glmClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with glmClient atomically - rollback on failure
      try {
        if (glmClient) {
          glmClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = glmClient.getCredentials();
      ...
      await this.forceFileCheck(glmClient);
      ...
      if (
        ...
        // No need to call glmClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await glmClient.refreshAccessToken();
   ...
   */
  private isTokenValid(credentials: GLMCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(Storage.getGlobalGLMDir(), GLM_LOCK_FILENAME);
  }
