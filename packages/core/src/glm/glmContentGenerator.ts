...
/**
 * GLM Content Generator that uses GLM OAuth tokens with automatic refresh
 */
export class GLMContentGenerator extends OpenAIContentGenerator {
  private readonly debugLogger = createDebugLogger('GLM');
  private glmClient: IGLMOAuth2Client;
  ...
  constructor(
    ...
    // Create DashScope provider for GLM
  ...
  private async executeWithCredentialManagement<T>(
    ...
    try {
      ...
      if (this.isAuthError(error)) {
        ...
        await this.sharedManager.getValidCredentials(this.glmClient, true);
        return await attemptOperation();
      }
      throw error;
    }
