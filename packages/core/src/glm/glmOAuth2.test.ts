...
interface MockSharedTokenManager {
  getValidCredentials(glmClient: GLMOAuth2Client): Promise<GLMCredentials>;
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    async getValidCredentials(
      glmClient: GLMOAuth2Client,
    ): Promise<GLMCredentials> {
      // Try to get credentials from the client first
      const clientCredentials = glmClient.getCredentials();
  ...
  describe('isErrorResponse', () => {
    ...
    it('should return false for successful responses', () => {
      const successResponse: DeviceAuthorizationResponse = {
        device_code: 'test-device-code',
        user_code: 'TEST123',
        verification_uri: 'https://chat.glm.ai/device',
        verification_uri_complete: 'https://chat.glm.ai/device?code=TEST123',
        expires_in: 1800,
      };
...
describe('GLMOAuth2Client', () => {
  let client: GLMOAuth2Client;
  let originalFetch: typeof global.fetch;

  beforeEach(() => {
    // Create client instance
    client = new GLMOAuth2Client();
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<GLMCredentials>;
          };
        }
      ).sharedManager = {
  ...
  describe('refreshAccessToken error handling', () => {
    ...
    it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
      const { CredentialsClearRequiredError } = await import('./glmOAuth2.js');
      ...
      await expect(client.refreshAccessToken()).rejects.toThrow(
        'GLM OAuth refresh returned invalid JSON:',
      );
    });
...
describe('getGLMOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./glmOAuth2.js').then((module) =>
      module.getGLMOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    await expect(
      import('./glmOAuth2.js').then((module) =>
        module.getGLMOAuthClient(mockConfig),
      ),
  ...
  it('should include troubleshooting hints when device auth fetch fails', async () => {
    ...
    const emitSpy = vi.spyOn(glmOAuth2Events, 'emit');

    let thrownError: unknown;
    try {
      const { getGLMOAuthClient } = await import('./glmOAuth2.js');
      await getGLMOAuthClient(mockConfig);
    } catch (error: unknown) {
      thrownError = error;
    }
...
describe('clearGLMCredentials', () => {
  it('should successfully clear credentials file', async () => {
    const { promises: fs } = await import('node:fs');
    const { clearGLMCredentials } = await import('./glmOAuth2.js');

    vi.mocked(fs.unlink).mockResolvedValue(undefined);

    await expect(clearGLMCredentials()).resolves.not.toThrow();
...
describe('GLMOAuth2Client - Additional Error Scenarios', () => {
  let client: GLMOAuth2Client;
  let originalFetch: typeof global.fetch;
...
describe('getGLMOAuthClient - Enhanced Error Scenarios', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('authWithGLMDeviceFlow - Comprehensive Testing', () => {
  let mockConfig: Config;
  let originalFetch: typeof global.fetch;
...
describe('Event Emitter Integration', () => {
  it('should export glmOAuth2Events as EventEmitter', async () => {
    const { glmOAuth2Events } = await import('./glmOAuth2.js');
    expect(glmOAuth2Events).toBeInstanceOf(EventEmitter);
  });

  it('should define correct event enum values', async () => {
    const { GLMOAuth2Event } = await import('./glmOAuth2.js');
    expect(GLMOAuth2Event.AuthUri).toBe('auth-uri');
    expect(GLMOAuth2Event.AuthProgress).toBe('auth-progress');
    expect(GLMOAuth2Event.AuthCancel).toBe('auth-cancel');
  });
});
...
describe('Utility Functions', () => {
  ...
  describe('getGLMCachedCredentialPath', () => {
    it('should return correct path to cached credentials', async () => {
      const os = await import('os');
      const path = await import('path');

      const expectedPath = path.join(os.homedir(), '.glm', 'oauth_creds.json');

      // Since this is a private function, we test it indirectly through clearGLMCredentials
      const { promises: fs } = await import('node:fs');
      const { clearGLMCredentials } = await import('./glmOAuth2.js');
...
describe('Credential Caching Functions', () => {
  describe('cacheGLMCredentials', () => {
    it('should create directory and write credentials to file', async () => {
      // Mock the internal cacheGLMCredentials function by creating client and calling refresh
...
describe('Enhanced Error Handling and Edge Cases', () => {
  ...
  describe('GLMOAuth2Client getAccessToken enhanced scenarios', () => {
  ...
  describe('Enhanced refreshAccessToken scenarios', () => {
    it('should call clearGLMCredentials on 400 error', async () => {
      client.setCredentials({
...
describe('SharedTokenManager Integration in GLMOAuth2Client', () => {
  let client: GLMOAuth2Client;
  ...
  it('should handle TokenManagerError types correctly in getGLMOAuthClient', async () => {
    const mockConfig = {
    ...
    for (const errorInfo of tokenErrors) {
      ...
      try {
        await import('./glmOAuth2.js').then((module) =>
          module.getGLMOAuthClient(mockConfig),
        );
      } catch {
...
describe('Constants and Configuration', () => {
  it('should have correct OAuth endpoints', async () => {
    ...
    expect(url).toBe('https://chat.glm.ai/api/v1/oauth2/device/code');
  });
