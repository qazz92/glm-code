...
/**
 * Creates a mock GLMOAuth2Client for testing
 */
function createMockGLMClient(
  initialCredentials: Partial<GLMCredentials> = {},
): IGLMOAuth2Client {
  let credentials: GLMCredentials = {
  ...
  return {
    setCredentials: vi.fn((creds: GLMCredentials) => {
...
function createValidCredentials(
  overrides: Partial<GLMCredentials> = {},
): GLMCredentials {
  return {
...
describe('SharedTokenManager', () => {
  ...
  beforeEach(() => {
    ...
    mockPath.dirname.mockImplementation((filePath) => {
      ...
      if (!filePath || typeof filePath !== 'string') {
        return '/home/user/.glm'; // Return the expected directory path
      }
  ...
  describe('getValidCredentials', () => {
    it('should return valid cached credentials without refresh', async () => {
      const mockClient = createMockGLMClient();
    ...
    it('should refresh expired credentials', async () => {
      const mockClient = createMockGLMClient(createExpiredCredentials());
    ...
    it('should force refresh when forceRefresh is true', async () => {
      const mockClient = createMockGLMClient(createValidCredentials());
      const refreshResponse = createSuccessfulRefreshResponse();
  ...
  describe('CredentialsClearRequiredError handling', () => {
    it('should clear memory cache when CredentialsClearRequiredError is thrown during refresh', async () => {
      ...
      const refreshPromise =
        getPrivateProperty<Promise<GLMCredentials> | null>(
          tokenManager,
          'refreshPromise',
        );
    ...
    it('should properly clean up timeout when file operation completes before timeout', async () => {
      ...
      ) as (client?: IGLMOAuth2Client) => Promise<void>;
