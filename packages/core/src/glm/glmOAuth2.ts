...
const debugLogger = createDebugLogger('GLM_OAUTH');

// OAuth Endpoints
const GLM_OAUTH_BASE_URL = 'https://chat.glm.ai';

const GLM_OAUTH_DEVICE_CODE_ENDPOINT = `${GLM_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const GLM_OAUTH_TOKEN_ENDPOINT = `${GLM_OAUTH_BASE_URL}/api/v1/oauth2/token`;

// OAuth Client Configuration
const GLM_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';

const GLM_OAUTH_SCOPE = 'openid profile email model.completion';
const GLM_OAUTH_GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:device_code';

// File System Configuration
const GLM_CREDENTIAL_FILENAME = 'oauth_creds.json';
...
/**
 * GLM OAuth2 credentials interface
 */
export interface GLMCredentials {
  access_token?: string;
...
/**
 * GLM OAuth2 client interface
 */
export interface IGLMOAuth2Client {
  setCredentials(credentials: GLMCredentials): void;
  getCredentials(): GLMCredentials;
...
/**
 * GLM OAuth2 client implementation
 */
export class GLMOAuth2Client implements IGLMOAuth2Client {
  private credentials: GLMCredentials = {};
  ...
  async requestDeviceAuthorization(options: {
    ...
    const bodyData = {
      client_id: GLM_OAUTH_CLIENT_ID,
      scope: options.scope,
      code_challenge: options.code_challenge,
      code_challenge_method: options.code_challenge_method,
    };

    const response = await fetch(GLM_OAUTH_DEVICE_CODE_ENDPOINT, {
      method: 'POST',
      headers: {
  ...
  async pollDeviceToken(options: {
    ...
    const bodyData = {
      grant_type: GLM_OAUTH_GRANT_TYPE,
      client_id: GLM_OAUTH_CLIENT_ID,
      device_code: options.device_code,
      code_verifier: options.code_verifier,
    };

    const response = await fetch(GLM_OAUTH_TOKEN_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Accept: 'application/json',
  ...
  async refreshAccessToken(): Promise<TokenRefreshResponse> {
    ...
    try {
      ...
      throw new Error(
        `GLM OAuth refresh returned invalid JSON: ${responseText || '(empty response body)'}`,
      );
    }
...
/**
 * Global event emitter instance for GLMOAuth2 authentication events
 */
export const glmOAuth2Events = new EventEmitter();

export async function getGLMOAuthClient(
  config: Config,
  options?: { requireCachedCredentials?: boolean },
): Promise<GLMOAuth2Client> {
  const client = new GLMOAuth2Client();
  ...
  try {
    ...
    if (options?.requireCachedCredentials) {
      throw new Error(
        'GLM OAuth credentials expired. Please use /auth to re-authenticate with glm-oauth.',
      );
    }
    ...
    const result = await authWithGLMDeviceFlow(client, config);
    if (!result.success) {
      ...
      const errorMessage =
        ...
        (() => {
          switch (result.reason) {
            case 'timeout':
              return 'GLM OAuth authentication timed out';
            case 'cancelled':
              return 'GLM OAuth authentication was cancelled by user';
            case 'rate_limit':
              return 'Too many request for GLM OAuth authentication, please try again later.';
            case 'error':
            default:
              return 'GLM OAuth authentication failed';
          }
        })();
...
function showFallbackMessage(verificationUriComplete: string): void {
  const title = 'GLM OAuth Device Authorization';
...
async function authWithGLMDeviceFlow(
  client: GLMOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  glmOAuth2Events.once(GLMOAuth2Event.AuthCancel, cancelHandler);
  ...
  const checkCancellation = (): AuthResult | null => {
    ...
    debugLogger.debug('\n' + message);
    glmOAuth2Events.emit(GLMOAuth2Event.AuthProgress, 'error', message);
  ...
  const emitAuthProgress = (
    ...
    glmOAuth2Events.emit(GLMOAuth2Event.AuthProgress, status, message);
  };
  ...
  try {
    ...
    glmOAuth2Events.emit(GLMOAuth2Event.AuthUri, deviceAuth);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to GLMCredentials format
          const credentials: GLMCredentials = {
          ...
          await cacheGLMCredentials(credentials);

          // IMPORTANT:
          // SharedTokenManager maintains an in-memory cache and throttles file checks.
          // If we only write the creds file here, a subsequent `getGLMOAuthClient()`
    ...
    glmOAuth2Events.off(GLMOAuth2Event.AuthCancel, cancelHandler);
  }
}

async function cacheGLMCredentials(credentials: GLMCredentials) {
  const filePath = getGLMCachedCredentialPath();
  try {
...
/**
 * Clear cached GLM credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearGLMCredentials(): Promise<void> {
  try {
    const filePath = getGLMCachedCredentialPath();
    await fs.unlink(filePath);
    debugLogger.debug('Cached GLM credentials cleared successfully.');
  } catch (error: unknown) {
    ...
    debugLogger.warn(
      'Warning: Failed to clear cached GLM credentials:',
      error,
    );
  } finally {
...
function getGLMCachedCredentialPath(): string {
  return path.join(Storage.getGlobalGLMDir(), GLM_CREDENTIAL_FILENAME);
}

export const clearCachedCredentialFile = clearGLMCredentials;
