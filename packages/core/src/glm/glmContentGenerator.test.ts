...
import type { IGLMOAuth2Client } from './glmOAuth2.js';
import { type GLMCredentials, type ErrorData } from './glmOAuth2.js';
import type {
...
vi.mock('./sharedTokenManager.js', () => ({
  SharedTokenManager: class {
    ...
    private mockCredentials: GLMCredentials | null = null;
    ...
    async getValidCredentials(
      glmClient: IGLMOAuth2Client,
    ): Promise<GLMCredentials> {
      ...
      try {
        const { token } = await glmClient.getAccessToken();
        if (token) {
          const credentials = glmClient.getCredentials();
          return credentials;
        }
      } catch (error) {
        ...
        if (isAuthError) {
          ...
          try {
            const refreshResult = await glmClient.refreshAccessToken();
            if (refreshResult && !('error' in refreshResult)) {
              // Refresh succeeded, update client credentials and return them
              const updatedCredentials = glmClient.getCredentials();
              return updatedCredentials;
            } else {
              // Refresh failed, throw appropriate error
              throw new Error(
                'Failed to obtain valid GLM access token. Please re-authenticate.',
              );
            }
          } catch {
            throw new Error(
    ...
    getCurrentCredentials(): GLMCredentials | null {
      return this.mockCredentials;
    }
    ...
    setMockCredentials(credentials: GLMCredentials | null): void {
...
describe('GLMContentGenerator', () => {
  let mockGLMClient: IGLMOAuth2Client;
  let glmContentGenerator: GLMContentGenerator;
  let mockConfig: Config;

  const mockCredentials: GLMCredentials = {
  ...
  beforeEach(() => {
    ...
    // Create GLMContentGenerator instance
    ...
    glmContentGenerator = new GLMContentGenerator(
      mockGLMClient,
  ...
  describe('Core Content Generation Methods', () => {
    it('should generate content with valid token', async () => {
      vi.mocked(mockGLMClient.getAccessToken).mockResolvedValue({
        token: 'valid-token',
      });
      vi.mocked(mockGLMClient.getCredentials).mockReturnValue(mockCredentials);
      ...
      const result = await glmContentGenerator.generateContent(
        request,
        'test-prompt-id',
      );

      expect(result.text).toBe('Generated content');
      expect(mockGLMClient.getAccessToken).toHaveBeenCalled();
    });
    ...
    it('should generate content stream with valid token', async () => {
      ...
      const stream = await glmContentGenerator.generateContentStream(
        request,
        'test-prompt-id',
      );
      const chunks: string[] = [];
    ...
    it('should count tokens without requiring authentication', async () => {
      ...
      const result = await glmContentGenerator.countTokens(request);

      expect(result.totalTokens).toBe(15);
      // countTokens is a local operation and should not require OAuth credentials
      expect(mockGLMClient.getAccessToken).not.toHaveBeenCalled();
    });
    ...
    it('should embed content with valid token', async () => {
      ...
      const result = await glmContentGenerator.embedContent(request);
  ...
  describe('Token Management and Refresh Logic', () => {
    it('should refresh token on auth error and retry', async () => {
      ...
      vi.mocked(mockGLMClient.getAccessToken)
        .mockRejectedValueOnce(authError)
        .mockResolvedValueOnce({ token: 'refreshed-token' });

      // Refresh succeeds
      vi.mocked(mockGLMClient.refreshAccessToken).mockResolvedValue({
      ...
      vi.mocked(mockGLMClient.getCredentials).mockReturnValue({
        access_token: 'refreshed-token',
        token_type: 'Bearer',
      ...
      expect(result.text).toBe('Generated content');
      expect(mockGLMClient.refreshAccessToken).toHaveBeenCalled();
    });
    ...
    it('should handle token refresh failure', async () => {
      ...
      await expect(
        glmContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow(
        'Failed to obtain valid GLM access token. Please re-authenticate.',
      );
    ...
    it('should update endpoint when token is refreshed', async () => {
      ...
      await glmContentGenerator.generateContent(request, 'test-prompt-id');

      expect(mockGLMClient.getCredentials).toHaveBeenCalled();
    });
  });
  ...
  describe('Endpoint URL Normalization', () => {
    it('should use default endpoint when no custom endpoint provided', async () => {
      ...
      const parentPrototype = Object.getPrototypeOf(
        Object.getPrototypeOf(glmContentGenerator),
      );
  ...
  describe('Client State Management', () => {
    it('should set dynamic credentials during operations', async () => {
      const client = (
        glmContentGenerator as unknown as {
  ...
  describe('Error Handling and Retry Logic', () => {
    it('should retry once on authentication errors', async () => {
      ...
      vi.mocked(mockGLMClient.getAccessToken).mockImplementation(async () => {
    ...
    it('should not retry non-authentication errors', async () => {
      ...
      expect(mockGLMClient.refreshAccessToken).not.toHaveBeenCalled();
    ...
    it('should handle error response from token refresh', async () => {
      vi.mocked(mockGLMClient.getAccessToken).mockRejectedValue(
      ...
      await expect(
        glmContentGenerator.generateContent(request, 'test-prompt-id'),
      ).rejects.toThrow('Failed to obtain valid GLM access token');
    });
  });

  describe('Token State Management', () => {
    it('should cache and return current token', () => {
      expect(glmContentGenerator.getCurrentToken()).toBeNull();

      // Simulate setting a token internally
      (
        glmContentGenerator as unknown as { currentToken: string }
      ).currentToken = 'cached-token';

      expect(glmContentGenerator.getCurrentToken()).toBe('cached-token');
    });

    it('should clear token on clearToken()', () => {
      // Simulate having cached token value
      const glmInstance = glmContentGenerator as unknown as {
        currentToken: string;
      };
      glmInstance.currentToken = 'cached-token';

      glmContentGenerator.clearToken();
    ...
    it('should handle concurrent token refresh requests', async () => {
      ...
      vi.mocked(mockGLMClient.getAccessToken).mockRejectedValue(authError);
      vi.mocked(mockGLMClient.getCredentials).mockReturnValue(mockCredentials);

      vi.mocked(mockGLMClient.refreshAccessToken).mockImplementation(
        async () => {
          refreshCallCount++;
  ...
  describe('SharedTokenManager Integration', () => {
    it('should use SharedTokenManager to get valid credentials', async () => {
      ...
      const newGenerator = new GLMContentGenerator(
        mockGLMClient,
        { model: 'glm-turbo', authType: AuthType.GLM_OAUTH },
        mockConfig,
      );
  ...
  describe('getCurrentEndpoint Method', () => {
    it('should handle URLs with custom ports', () => {
      ...
      endpoints.forEach(({ input, expected }) => {
        ...
        const generator = glmContentGenerator as unknown as {
  ...
  describe('Stream Error Handling', () => {
    it('should set credentials when stream generation fails', async () => {
      ...
      try {
        await glmContentGenerator.generateContentStream(
          request,
          'test-prompt-id',
        );
      } catch (error) {
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should handle method types with token failure (except countTokens)', async () => {
      ...
      await expect(newGenerator.embedContent(embedRequest)).rejects.toThrow(
        'Failed to obtain valid GLM access token',
      );
