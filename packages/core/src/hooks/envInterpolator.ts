...
export function interpolateEnvVars(
  ...
  const interpolated = value.replace(
    ...
    (match, varName: string) => {
      ...
      // Auto-allow GLM_*, ZAI_*, and CLAUDE_* prefixed variables for GLM Code compatibility
      if (varName.startsWith('GLM_') || varName.startsWith('ZAI_') || varName.startsWith('CLAUDE_')) {
        return process.env[varName] || '';
      }
...
/**
 * Build GLM-specific environment variables for hook context.
 * Provides $GLM_HOME, $GLM_MODEL, $ZAI_API_KEY aliases,
 * plus $CLAUDE_* compatibility aliases for Claude Code hooks.
 */
export function buildGlmHookEnv(config: {
  configDir: string;
  model?: string;
  apiKeyEnvKey?: string;
  apiKey?: string;
  ...
  // GLM-native aliases
  env['GLM_HOME'] = config.configDir;
  if (config.model) env['GLM_MODEL'] = config.model;
  if (config.apiKey) env['ZAI_API_KEY'] = config.apiKey;
  if (config.apiKeyEnvKey) env['GLM_API_KEY_ENV'] = config.apiKeyEnvKey;
  // $CLAUDE_* compatibility aliases for hooks that reference Claude Code env
  if (config.cwd) {
    env['CLAUDE_CWD'] = config.cwd;
    env['CLAUDE_PROJECT_DIR'] = config.cwd;
  }
  if (config.filePath) env['CLAUDE_FILE'] = config.filePath;
  if (config.toolName) env['CLAUDE_TOOL'] = config.toolName;
  if (config.sessionId) env['CLAUDE_SESSION'] = config.sessionId;
