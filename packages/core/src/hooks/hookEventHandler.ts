...
import { detectWorkflowKeyword } from './keyword-detector.js';
import { checkDelegationNeed } from './delegation-enforcer.js';
...
export class HookEventHandler {
  ...
  /** Loop guard to prevent infinite hook triggering */
  ...
  async fireUserPromptSubmitEvent(
    ...
    // Reset loop guard counters for a new user turn
    this.loopGuard.beginTurn();

    // Detect workflow keywords in user prompt
    const keywordMatch = detectWorkflowKeyword(prompt);
    if (keywordMatch) {
      debugLogger.info(
        `Workflow keyword detected: "${keywordMatch.keyword}" → ${keywordMatch.workflow}`,
      );
    }
  ...
  async firePostToolUseEvent(
    ...
    // Check if the tool result suggests delegation
    const responseText =
  ...
  private async executeHooks(
    ...
    try {
      ...
      let allHookConfigs = [...registryHookConfigs, ...sessionHookConfigs];

      // Filter out hooks blocked by the loop guard (applies to PreToolUse only)
      if (eventName === HookEventName.PreToolUse) {
        allHookConfigs = allHookConfigs.filter((config) => {
          const hookName = this.getHookName(config);
          if (!this.loopGuard.tryAcquire(eventName, hookName)) {
            debugLogger.warn(
              `LoopGuard blocked hook ${hookName} for event ${eventName} (limit exceeded)`,
            );
            return false;
          }
          return true;
