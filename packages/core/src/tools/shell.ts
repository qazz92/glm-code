...
function findAttributableCommitSegment(
  ...
  for (const sub of splitCommands(command)) {
    ...
    if (start < 0) {
      ...
      // when GLM_DEBUG_LOG_FILE is set.
      debugLogger.warn(
...
> {
  ...
  async execute(
    ...
    // Add co-author to git commit commands and GLM Code attribution to
    ...
    // GLM_DEBUG_LOG_FILE entry the user has likely never set up.
  ...
  private async attachCommitAttribution(
    ...
    try {
      ...
      // Pass the actual model name (e.g. `glm3-coder-plus`) rather than the
   ...
   * PR body. Format: "🤖 Generated with GLM Code (N-shotted by GLM-Coder)"
   * when at least one user prompt has been recorded since the last commit;
   * otherwise just "🤖 Generated with GLM Code".
   *
  ...
  private addAttributionToPR(command: string): string {
    ...
    const generator = gitCoAuthorSettings.name ?? 'GLM-Coder';

    const attribution =
      shots > 0
        ? `\n\n🤖 Generated with GLM Code (${shots}-shotted by ${generator})`
        : `\n\n🤖 Generated with GLM Code`;
    ...
    // skip in `GLM_DEBUG_LOG_FILE`. Inline-body rewriting is the
