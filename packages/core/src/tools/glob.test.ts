...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .glmignore files by default', async () => {
      await fs.writeFile(
        path.join(tempRootDir, '.glmignore'),
        '*.glmignored.txt',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'a.glmignored.txt'),
        'ignored content',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'b.notignored.txt'),
      ...
      // Recreate the tool to pick up the new .glmignore file
      globTool = new GlobTool(mockConfig);
      ...
      expect(result.llmContent).toContain('Found 3 file(s)'); // fileA.txt, FileB.TXT, b.notignored.txt
      expect(result.llmContent).not.toContain('a.glmignored.txt');
    });
    ...
    it('should respect .glmignore when searching a subdirectory (path option)', async () => {
      await fs.writeFile(path.join(tempRootDir, '.glmignore'), '*.secret');
      await fs.writeFile(path.join(tempRootDir, 'sub', 'visible.txt'), 'ok');
      await fs.writeFile(
        path.join(tempRootDir, 'sub', 'hidden.secret'),
        'should be ignored',
