/**
 * GitHub Issue scheme handler (issue://).
 * Delegates to `gh` CLI for fetching issue content.
 */
...
function parseIssueRef(ref: string): { owner?: string; repo?: string; number: number } {
  // Forms: "123", "owner/repo/123", "owner/repo#123"
...
export const issueHandler: SchemeHandler = async (parsed: ParsedUrl): Promise<ToolResult<string>> => {
  try {
    const ref = parseIssueRef(parsed.path)
    const repoFlag = ref.owner && ref.repo ? [`-R`, `${ref.owner}/${ref.repo}`] : []
    const body = await execGh(['issue', 'view', String(ref.number), ...repoFlag, '--comments'])
    return { ok: true as const, data: body }
  } catch (e) {
    return toolErr('RUNTIME_ERROR', `gh issue failed: ${(e as Error).message}`)
  }
}
