...
import { isOverCap, DEFAULT_MEMORY_CAPS } from '../memory/caps.js';
import { findNearestAgentsMd } from '../memory/file-relative.js';
 import { isInternalUrl, resolveInternalUrl } from '../utils/internal-urls.js';
...
> {
  ...
  override async getDefaultPermission(): Promise<PermissionDecision> {
    ...
    if (
      ...
      // sensitive ~/.glm files such as settings.json or OAuth credentials.
  ...
  async execute(): Promise<ToolResult> {
    // Check for internal URL schemes before resolving as a file path
    if (isInternalUrl(this.params.file_path)) {
      const resolved = await resolveInternalUrl(this.params.file_path, {
        projectDir: this.config.getTargetDir(),
      });
      if (resolved?.content !== undefined) {
        return {
          llmContent: resolved.content,
          returnDisplay: `Resolved ${resolved.type}: ${resolved.path}`,
        };
      }
      return {
        llmContent: `Could not resolve internal URL: ${this.params.file_path}`,
        returnDisplay: 'Unresolved internal URL',
        error: { message: `Unresolved internal URL: ${this.params.file_path}`, type: ToolErrorType.FILE_NOT_FOUND },
      };
    }
    ...
    // Memory caps: check if file-read cache is over capacity.
    // If over limit, clear cache to stay under caps.
    if (cacheEnabled) {
      const overCap = isOverCap(
        { fileCount: cache.size(), totalBytes: 0 },
        DEFAULT_MEMORY_CAPS,
      );
      if (overCap) {
        debugLogger.debug('Memory caps eviction: clearing file-read cache', {
          entryCount: cache.size(),
          maxFiles: DEFAULT_MEMORY_CAPS.maxFiles,
        });
        cache.clear();
      }
    }
    ...
    // Attach nearest AGENTS.md context for deep file reads, giving the model
    // directory-local conventions without requiring a separate read.
    if (typeof llmContent === 'string') {
      const agentsPath = findNearestAgentsMd(absPath, projectRoot);
      if (agentsPath) {
        try {
          const agentsContent = await fs.readFile(agentsPath, 'utf-8');
          if (agentsContent.trim()) {
            llmContent +=
              `\n\n---\n[Context from ${path.relative(projectRoot, agentsPath)}]:\n${agentsContent}`;
          }
        } catch {
          // Best-effort — file may have been deleted between discovery and read.
        }
      }
    }
...
> {
  ...
  protected override validateToolParamValues(
    ...
    if (fileService.shouldGLMIgnoreFile(params.file_path)) {
      return `File path '${filePath}' is ignored by .glmignore pattern(s).`;
    }
