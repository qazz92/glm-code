...
/**
 * Per-process cache for `.glmignore` discovery. The same directories show
 ...
 * `glmIgnore`: dir → string | null (cached `.glmignore` path or null)
 *
 * **Known staleness window:** a `.glmignore` created mid-session, or a
...
const glmIgnoreCache = new Map<string, string | null>();
...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    // Add file exclusions from .gitignore and .glmignore
    const filteringOptions = this.getFileFilteringOptions();
    if (!filteringOptions.respectGitIgnore) {
      rgArgs.push('--no-ignore-vcs');
    }

    if (filteringOptions.respectGLMIgnore) {
      // Load .glmignore from each workspace directory, not just the primary one
      ...
      for (const searchPath of paths) {
        ...
        let glmIgnorePath = glmIgnoreCache.get(dir);
        if (glmIgnorePath === undefined) {
          const candidate = path.join(dir, '.glmignore');
          glmIgnorePath = fs.existsSync(candidate) ? candidate : null;
          glmIgnoreCache.set(dir, glmIgnorePath);
          trimCache(glmIgnoreCache);
        }
        if (glmIgnorePath && !seenIgnoreFiles.has(glmIgnorePath)) {
          rgArgs.push('--ignore-file', glmIgnorePath);
          seenIgnoreFiles.add(glmIgnorePath);
        }
      }
    }
  ...
  private getFileFilteringOptions(): FileFilteringOptions {
    ...
    return {
      ...
      respectGLMIgnore:
        options?.respectGLMIgnore ??
        DEFAULT_FILE_FILTERING_OPTIONS.respectGLMIgnore,
    };
  }
