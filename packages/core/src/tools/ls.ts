...
export interface LSToolParams {
  ...
  /**
   * Whether to respect .gitignore and .glmignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_glm_ignore?: boolean;
  };
}
...
class LSToolInvocation extends BaseToolInvocation<LSToolParams, ToolResult> {
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      const { filteredPaths, gitIgnoredCount, glmIgnoredCount } =
        fileDiscovery.filterFilesWithReport(relativePaths, {
          ...
          respectGLMIgnore:
            this.params.file_filtering_options?.respect_glm_ignore ??
            this.config.getFileFilteringOptions().respectGLMIgnore ??
      ...
      if (glmIgnoredCount > 0) {
        ignoredMessages.push(`${glmIgnoredCount} glm-ignored`);
      }
    ...
    super(
      ...
      Kind.Search,
      {
        properties: {
          ...
          file_filtering_options: {
            description:
              'Optional: Whether to respect ignore patterns from .gitignore or .glmignore',
            type: 'object',
            properties: {
              respect_git_ignore: {
                description:
              ...
              respect_glm_ignore: {
                description:
                  'Optional: Whether to respect .glmignore patterns when listing files. Defaults to true.',
                type: 'boolean',
              },
            },
          },
