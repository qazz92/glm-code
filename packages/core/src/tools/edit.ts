...
import { runPostEditDiagnostics, buildPostEditInstruction } from './post-edit-diagnostics.js';
const debugLogger = createDebugLogger('EDIT_PRIOR_READ');
import { containsAnchors, applyAnchorEdits, type AnchorEditOp } from './hashline-edit.js';
...
class EditToolInvocation implements ToolInvocation<EditToolParams, ToolResult> {
  ...
  private async calculateEdit(params: EditToolParams): Promise<CalculatedEdit> {
    ...
    // Hashline mode: if old_string contains LINE+HASH|TEXT anchors, route
    // through the anchor-based edit path which verifies hashes against the
    // current file content before applying changes.
    if (fileExists && currentContent !== null && containsAnchors(finalOldString)) {
      const oldLines = finalOldString.split('\n');
      const newLines = finalNewString.split('\n');
      const editOps: AnchorEditOp[] = [];
      const ANCHOR_RE = /^(\d+)([a-z][a-z0-9]{5})\|(.*)$/;

      for (const ol of oldLines) {
        const m = ANCHOR_RE.exec(ol);
        if (!m) continue;
        const lineNum = Number(m[1]);
        const hash = m[2];
        const oldText = m[3];
        // Find the matching line in newLines (same anchor line number)
        const newMatch = newLines.find((nl) => {
          const nm = ANCHOR_RE.exec(nl);
          return nm && Number(nm[1]) === lineNum;
        });
        const newText = newMatch ? ANCHOR_RE.exec(newMatch)![3] : oldText;
        if (newText !== oldText) {
          editOps.push({ type: 'replace', line: lineNum, hash, oldText, newText });
        }
      }
      ...
      if (editOps.length === 0) {
        error = {
          ...
          raw: `No changes to apply. No anchor mismatches found in file: ${params.file_path}`,
          type: ToolErrorType.EDIT_NO_CHANGE,
        };
      } else {
        const result = applyAnchorEdits(params.file_path, editOps);
        if (!result.ok) {
          error = {
            display: `Hashline edit failed: ${result.error}`,
            raw: `Hashline edit failed: ${result.error} in file: ${params.file_path}`,
            type: ToolErrorType.EDIT_NO_OCCURRENCE_FOUND,
          };
        } else {
          occurrences = editOps.length;
          return {
            currentContent,
  ...
  async execute(signal: AbortSignal): Promise<ToolResult> {
    ...
    try {
      ...
      // Run post-edit diagnostics after successful write
      const diagResult = runPostEditDiagnostics(
        [this.params.file_path],
        this.config.getProjectRoot(),
      );
      const diagInstruction = buildPostEditInstruction([this.params.file_path]);
      const diagSuffix = diagResult.hasErrors
        ? `\n\n${diagInstruction}`
        : '';

      return {
        llmContent: llmSuccessMessageParts.join(' ') + diagSuffix,
