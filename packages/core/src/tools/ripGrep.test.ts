...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should pass .glmignore to ripgrep when respected', async () => {
      await fs.writeFile(
    ...
    it('should include .glmignore matches when disabled in config', async () => {
      await fs.writeFile(path.join(tempRootDir, '.glmignore'), 'kept.txt\n');
  ...
  describe('multi-directory workspace', () => {
    ...
    it('should load .glmignore from each workspace directory', async () => {
      const secondDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'grep-tool-second-'),
      );
      await fs.writeFile(path.join(secondDir, '.glmignore'), 'ignored.txt\n');
      await fs.writeFile(
        path.join(tempRootDir, '.glmignore'),
        'other-ignored.txt\n',
      );
      ...
      // Verify both .glmignore files were passed
      ...
      expect(ignoreFileArgs).toContain(path.join(tempRootDir, '.glmignore'));
      expect(ignoreFileArgs).toContain(path.join(secondDir, '.glmignore'));
