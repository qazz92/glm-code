...
import { DEFAULT_GLM_EMBEDDING_MODEL } from './models.js';
  ...
  const pickBool = (v: unknown, fieldName: string): boolean => {
    ...
    if (typeof v === 'string') {
      ...
      if (!knownDisable.includes(lowered)) {
        ...
        // can see the actual coercion in GLM_DEBUG_LOG_FILE.
...
export type ExtensionOriginSource = 'GLMCode' | 'Claude' | 'Gemini';
...
export interface AgentsCollabSettings {
  ...
  arena?: {
    /** Custom base directory for Arena worktrees (default: ~/.glm/arena) */
...
export interface ConfigParameters {
   ...
   * the `GLM_DISABLED_SLASH_COMMANDS` environment variable.
   */
   ...
   * When set, GLM Code outputs structured JSON events to this fd while
   ...
   * Only meaningful in headless mode (`glm -p`).
   */
  ...
  /** Glob patterns to exclude from .glm/rules/ loading. */
...
/** GLM built-in MCP servers — auto-registered unless explicitly disabled. */
const GLM_BUILTIN_MCP_SERVERS: Record<string, MCPServerConfig & { disabled?: boolean }> = {
  'glm-vision': {
    command: 'npx',
  ...
  'glm-web-reader': {
    httpUrl: 'https://api.z.ai/api/mcp/web_reader/mcp',
    headers: { Authorization: 'Bearer ${ZAI_API_KEY}' },
  },
  'glm-zread': {
    httpUrl: 'https://api.z.ai/api/mcp/zread/mcp',
    headers: { Authorization: 'Bearer ${ZAI_API_KEY}' },
  },
};

export function getGlmBuiltinMcpServers(): Record<string, MCPServerConfig & { disabled?: boolean }> {
  return resolveEnvVarsInObject({ ...GLM_BUILTIN_MCP_SERVERS });
}
...
export class Config {
  ...
  constructor(params: ConfigParameters) {
    ...
    this.embeddingModel = params.embeddingModel ?? DEFAULT_GLM_EMBEDDING_MODEL;
    ...
    this.fileFiltering = {
      ...
      respectGLMIgnore: params.fileFiltering?.respectGLMIgnore ?? true,
  ...
  async initialize(options?: ConfigInitializeOptions): Promise<void> {
    ...
    // behavior with `GLM_CODE_LEGACY_MCP_BLOCKING=1` — kept ≥ 1 release as
    // an escape hatch.
   ...
   * - `GLM_CODE_LEGACY_MCP_BLOCKING=1` is set (MCP already discovered
  ...
  startNewSession(
    ...
    // glm --prompt-interactive) must not delete a sibling shell's
  ...
  private async handleModelChange(
    ...
    // Hot update path: only supported for glm-oauth.
    // For other auth types we always refresh to recreate the ContentGenerator.
    //
    // Rationale:
    // - Non-glm providers may need to re-validate credentials / baseUrl / envKey.
    ...
    if (authType === AuthType.GLM_OAUTH && !requiresRefresh) {
      const { config, sources } = resolveContentGeneratorConfigWithSources(
        this,
        authType,
        this.modelsConfig.getGenerationConfig(),
      ...
      // Hot-update fields (glm-oauth models share the same auth + client).
   ...
   * For glm-oauth model switches that are hot-update safe, this may update in place.
   *
  ...
  getMcpServers(): Record<string, MCPServerConfig> | undefined {
    // Start with GLM built-in servers (lowest priority — user/extension config overrides)
    const glmBuiltins = getGlmBuiltinMcpServers();
    let mcpServers: Record<string, MCPServerConfig> = {};

    // Add GLM built-ins first, unless disabled via user config
    const userServers = this.mcpServers || {};
    for (const [name, config] of Object.entries(glmBuiltins)) {
      const userEntry = userServers[name];
      // Skip if user explicitly disabled this built-in
      if (
        userEntry &&
        typeof userEntry === 'object' &&
        (userEntry as Record<string, unknown>)['disabled'] === true
      ) {
        continue;
      }
      // Skip if user excluded this server
      if (this.excludedMcpServers?.includes(name)) {
        continue;
      }
      // User config overrides built-in (don't add built-in if user has their own)
      if (!userEntry) {
        mcpServers[name] = config;
      }
    }

    // Merge user-configured servers on top
    Object.assign(mcpServers, userServers);

    // Add extension-provided servers (only if not already present)
  ...
  isCronEnabled(): boolean {
    ...
    if (process.env['GLM_CODE_ENABLE_CRON'] === '1') return true;
   ...
   *
   * Env overrides (either direction): `GLM_CODE_EMIT_TOOL_USE_SUMMARIES=0`
   * to force off, `=1` to force on.
   */
  getEmitToolUseSummaries(): boolean {
    const env = process.env['GLM_CODE_EMIT_TOOL_USE_SUMMARIES'];
  ...
  getFileFilteringRespectGLMIgnore(): boolean {
    return this.fileFiltering.respectGLMIgnore;
  }

  getFileFilteringOptions(): FileFilteringOptions {
    return {
      respectGitIgnore: this.fileFiltering.respectGitIgnore,
      respectGLMIgnore: this.fileFiltering.respectGLMIgnore,
    };
  }
  ...
  getBlockedMcpServers(): Array<{ name: string; extensionName: string }> {
    ...
    // Add GLM built-ins (respecting disabled/excluded)
    for (const [name, config] of Object.entries(glmBuiltins)) {
      const userEntry = userServers[name];
      if (
        userEntry &&
  ...
  async createToolRegistry(
    ...
    // `glm [--bare] --json-schema X -p "..."` to loop until
    ...
    // Register memory tools (retain, recall, reflect)
    await registerLazy(ToolNames.MEMORY_RETAIN, async () => {
      const { MemoryRetainTool } = await import('../tools/memory-tools.js');
      return new MemoryRetainTool();
    });
    await registerLazy(ToolNames.MEMORY_RECALL, async () => {
      const { MemoryRecallTool } = await import('../tools/memory-tools.js');
      return new MemoryRecallTool();
    });
    await registerLazy(ToolNames.MEMORY_REFLECT, async () => {
      const { MemoryReflectTool } = await import('../tools/memory-tools.js');
      return new MemoryReflectTool();
    });

    // Register notepad tool
    await registerLazy(ToolNames.NOTEPAD, async () => {
      const { NotepadTool } = await import('../tools/notepad-tool.js');
      return new NotepadTool();
    });

    // Register project-memory tool
    await registerLazy(ToolNames.PROJECT_MEMORY, async () => {
      const { ProjectMemoryTool } = await import(
        '../tools/project-memory-tool.js'
      );
      return new ProjectMemoryTool(this.getProjectRoot());
    });

    // Register shared-memory tool
    await registerLazy(ToolNames.SHARED_MEMORY, async () => {
      const { SharedMemoryTool } = await import(
        '../tools/shared-memory-tool.js'
      );
      return new SharedMemoryTool();
    });

    // Register commit tool
    await registerLazy(ToolNames.COMMIT, async () => {
      const { CommitTool } = await import('../tools/commit-tool.js');
      return new CommitTool();
    });

    // Register recipe tool
    await registerLazy(ToolNames.RECIPE, async () => {
      const { RecipeTool } = await import('../tools/recipe-tool.js');
      return new RecipeTool(this.getProjectRoot());
    });
