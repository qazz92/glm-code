 ...
 * out at runtime: `GLM_DISABLE_AUTO_TITLE=1` (or any truthy-ish value)
...
function autoTitleDisabledByEnv(): boolean {
  const v = process.env['GLM_DISABLE_AUTO_TITLE'];
  if (!v) return false;
 ...
 *
 * File location: ~/.glm/tmp/<project_id>/chats/
 *
...
export class ChatRecordingService {
  ...
  private maybeTriggerAutoTitle(): void {
    ...
    // Headless/one-shot CLI flows (`glm -p "…"`, cron, CI scripts) run a
