 ...
 */
function mockOk(title: string, modelUsed = 'glm-turbo'): void {
...
describe('ChatRecordingService - auto-title trigger', () => {
    ...
    mockConfig = {
      ...
      storage: {
        getProjectTempDir: vi
          .fn()
          .mockReturnValue('/test/project/root/.glm/tmp/hash'),
        getProjectDir: vi
          .fn()
          .mockReturnValue('/test/project/root/.glm/projects/test-project'),
      },
      getModel: vi.fn().mockReturnValue('glm-plus'),
  ...
  it('does not trigger when GLM_DISABLE_AUTO_TITLE is set', async () => {
    const prev = process.env['GLM_DISABLE_AUTO_TITLE'];
    process.env['GLM_DISABLE_AUTO_TITLE'] = '1';
    try {
      chatRecordingService.recordAssistantTurn({
        model: 'glm-plus',
        message: [{ text: 'reply' }],
      ...
      if (prev === undefined) delete process.env['GLM_DISABLE_AUTO_TITLE'];
      else process.env['GLM_DISABLE_AUTO_TITLE'] = prev;
    }
  });

  it('still triggers when GLM_DISABLE_AUTO_TITLE is falsy ("0")', async () => {
    mockOk('Fix login button');
    const prev = process.env['GLM_DISABLE_AUTO_TITLE'];
    process.env['GLM_DISABLE_AUTO_TITLE'] = '0';
    try {
      chatRecordingService.recordAssistantTurn({
        model: 'glm-plus',
        message: [{ text: 'reply' }],
  ...
  it('respects a late /rename that lands while the LLM call is in flight', async () => {
    ...
    resolveLlm({ ok: true, title: 'Auto Title', modelUsed: 'glm-turbo' });
    await flushMicrotasks();
