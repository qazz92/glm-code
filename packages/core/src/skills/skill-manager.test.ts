...
describe('SkillManager', () => {
  ...
  beforeEach(() => {
    ...
    // calls Storage.getGlobalGLMDir() which needs os.homedir()
    ...
    mockParseYaml.mockImplementation((yamlString: string) => {
      ...
      if (yamlString.includes('name: shared-skill')) {
        const desc = yamlString.includes('From glm dir')
          ? 'From glm dir'
  ...
  const validSkillConfig: SkillConfig = {
    ...
    description: 'A test skill',
    level: 'project',
    filePath: '/test/project/.glm/skills/test-skill/SKILL.md',
...
`;
    ...
    it('should determine level from file path', () => {
      const projectPath = '/test/project/.glm/skills/test-skill/SKILL.md';
      const userPath = '/home/user/.glm/skills/test-skill/SKILL.md';
  ...
  describe('listSkills', () => {
    beforeEach(() => {
      ...
      vi.mocked(fs.readdir).mockImplementation((dirPath: any) => {
        ...
        if (pathStr === projectGLMSkillsDir) {
...
Skill 3 content`);
    ...
    it('should deduplicate same-name skills across provider dirs within a level', async () => {
      // Override readdir to return the same skill name from both .glm and .agents dirs
      vi.mocked(fs.readdir).mockReset();
      const projectGLMDir = path.join('/test/project', '.glm', 'skills');
      const projectAgentDir = path.join('/test/project', '.agents', 'skills');

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      vi.mocked(fs.readdir).mockImplementation((dirPath: any) => {
        const pathStr = String(dirPath);
        if (pathStr === projectGLMDir) {
          return Promise.resolve([
            {
              name: 'shared-skill',
              isDirectory: () => true,
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.includes('.glm') && pathStr.includes('shared-skill')) {
          return Promise.resolve(
            `---\nname: shared-skill\ndescription: From glm dir\n---\nGLM content`,
          );
        }
      ...
      // Only one instance should remain, from .glm (first in PROVIDER_CONFIG_DIRS)
      expect(skills).toHaveLength(1);
      expect(skills[0].name).toBe('shared-skill');
      expect(skills[0].description).toBe('From glm dir');
    });
  ...
  describe('getSkillsBaseDirs', () => {
    it('should return all project-level base dirs', () => {
      ...
      expect(baseDirs).toContain(path.join('/test/project', '.glm', 'skills'));
    ...
    it('should return all user-level base dirs', () => {
      ...
      expect(baseDirs).toHaveLength(2);
      expect(baseDirs).toContain(path.join('/home/user', '.glm', 'skills'));
  ...
  describe('bundled skills', () => {
    ...
    const projectDirSegment = path.join('.glm', 'skills');
    const userDirSegment = path.join('.glm', 'skills');
...
`);
    ...
    it('does not activate a visible skill from a shadowed copy paths', async () => {
      ...
      vi.mocked(fs.readdir).mockImplementation((dirPath: any) => {
        const pathStr = String(dirPath);
        if (pathStr === projectGLMSkillsDir || pathStr === userGLMSkillsDir) {
          return Promise.resolve([
            {
              name: 'foo',
              isDirectory: () => true,
      ...
      vi.mocked(fs.readFile).mockImplementation((filePath) => {
        const pathStr = String(filePath);
        if (pathStr.startsWith(projectGLMSkillsDir)) {
          return Promise.resolve(`---
name: foo
description: A test skill
paths:
...
`);
        ...
        if (pathStr.startsWith(userGLMSkillsDir)) {
          return Promise.resolve(`---
name: foo
description: A test skill
paths:
...
Symlinked skill content`);
  ...
  describe('file watchers', () => {
    it('should pass ignored function and shallow depth to chokidar', async () => {
      const projectSkillsDir = path.join('/test/project', '.glm', 'skills');
