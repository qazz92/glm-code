...
/**
 * Converter for Claude Code plugins to GLM Code format.
 */
...
export function convertClaudeAgentConfig(
  ...
  if (claudeAgent.permissionMode) {
    const claudeToGLMMode: Record<string, string> = {
      default: 'default',
      plan: 'plan',
    ...
    const mapped =
      claudeToGLMMode[claudeAgent.permissionMode] ??
      claudeAgent.permissionMode;
    glmAgent['approvalMode'] = mapped;
  }
  if (claudeAgent.hooks) {
    glmAgent['hooks'] = claudeAgent.hooks;
  }
  if (claudeAgent.skills && claudeAgent.skills.length > 0) {
    glmAgent['skills'] = claudeAgent.skills;
  }
  if (claudeAgent.disallowedTools && claudeAgent.disallowedTools.length > 0) {
    glmAgent['disallowedTools'] = claudeAgent.disallowedTools;
  }
...
/**
 * Converts all agent files in a directory from Claude format to GLM format.
...
async function convertAgentFiles(agentsDir: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const glmAgent = convertClaudeAgentConfig(claudeAgent);

      // Build new frontmatter (excluding systemPrompt as it goes in body)
      const newFrontmatter: Record<string, unknown> = {};
      for (const [key, value] of Object.entries(glmAgent)) {
      ...
      const systemPrompt = (glmAgent['systemPrompt'] as string) || body.trim();
      const newContent = `---
${newYaml}
---
...
/**
 * Converts a Claude plugin config to GLM Code format.
 * @param claudeConfig Claude plugin configuration
 * @returns GLM ExtensionConfig
 */
export function convertClaudeToGLMConfig(
...
/**
 * Converts a complete Claude plugin package to GLM Code format.
 * Creates a new temporary directory with:
 * 1. Converted glm-extension.json
...
export async function convertClaudePluginPackage(
  ...
  try {
    ...
    // Step 9: Convert collected agent files from Claude format to GLM format
    const agentsDestDir = path.join(tmpDir, 'agents');
    await convertAgentFiles(agentsDestDir);

    // Step 10: Convert to GLM format config
    const glmConfig = convertClaudeToGLMConfig(mergedConfig);

    // Step 11: Write glm-extension.json
    const glmConfigPath = path.join(tmpDir, 'glm-extension.json');
    fs.writeFileSync(
      glmConfigPath,
      JSON.stringify(glmConfig, null, 2),
      'utf-8',
    );
