...
describe('substituteHookVariables', () => {
  ...
  it('should handle multiple event types with hooks', () => {
    const basePath = '/home/user/.glm/extensions/my-extension';
    ...
    expect(
      (result!['PreToolUse']![0].hooks![0] as { command: string }).command,
    ).toBe('/home/user/.glm/extensions/my-extension/scripts/pre-start.sh');
    expect(result!['UserPromptSubmit']).toHaveLength(1);
    expect(
      (result!['UserPromptSubmit']![0].hooks![0] as { command: string })
        .command,
    ).toBe('/home/user/.glm/extensions/my-extension/setup/install.py');
  });
...
describe('performVariableReplacement', () => {
  ...
  it('should replace .claude with .glm in markdown files', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('.glm/loop.local.md');
    expect(result).toContain('rm .glm/loop.local.md');
    expect(result).toContain('$HOME/.glm/cache');
    expect(result).toContain('./.glm/local');
  ...
  it('should replace .claude with .glm in shell scripts', () => {
    const extDir = path.join(testDir, 'ext');
    fs.mkdirSync(extDir, { recursive: true });
    ...
    expect(result).toContain('~/.glm/cache');
    expect(result).toContain('./.glm/local');
    expect(result).toContain('.glm/config');
    // Note: URLs are also being replaced in current implementation
    expect(result).toContain('https://example.com/.glm/page');
  });
