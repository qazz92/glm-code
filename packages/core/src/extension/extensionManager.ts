...
async function convertGeminiOrClaudeExtension(
  ...
  let originSource: ExtensionOriginSource = 'GLMCode';
...
export class ExtensionManager {
  ...
  async refreshCache(options?: { names?: string[] }): Promise<void> {
    ...
    if (requestedNames.length > 0) {
      ...
      // Default: load all extensions from GLM_HOME-aware user extensions dir.
    ...
    for (const extension of extensions) {
      try {
        const installMetadata: ExtensionInstallMetadata = {
          ...
          type: 'local',
          originSource: extension.installMetadata?.originSource || 'GLMCode',
        };
