...
/**
 * Converter for Gemini extensions to GLM Code format.
 */
...
/**
 * Converts a Gemini extension config to GLM Code format.
 * @param extensionDir Path to the Gemini extension directory
 * @returns GLM ExtensionConfig
 */
export function convertGeminiToGLMConfig(
  extensionDir: string,
): ExtensionConfig {
...
/**
 * Converts a complete Gemini extension package to GLM Code format.
...
export async function convertGeminiExtensionPackage(
  ...
  const geminiConfig = convertGeminiToGLMConfig(extensionDir);
  ...
  try {
    ...
    // Step 3: Create glm-extension.json with converted config
...
export function isGeminiExtensionConfig(extensionDir: string) {
  ...
  // If it has Gemini-specific fields but not GLM-specific fields, likely Gemini
  return true;
}
