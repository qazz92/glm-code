...
beforeEach(async () => {
  tmpDir = await mkdtemp(path.join(os.tmpdir(), 'glm-runtime-status-'));
});
...
describe('writeRuntimeStatus', () => {
  it('writes the expected fields', async () => {
    ...
    expect(data.glm_version).toBe('0.15.3');
  });

  it('defaults pid to process.pid and glm_version to null', async () => {
    ...
    expect(data.glm_version).toBeNull();
  });
...
describe('readRuntimeStatus', () => {
  it('round-trips a written record', async () => {
    ...
    expect(status!.glmVersion).toBe('0.15.3');
  });
