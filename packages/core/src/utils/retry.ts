...
import { isGLMQuotaExceededError } from './quotaErrorDetection.js';
 ...
 * Requires the user to opt in via GLM_CODE_UNATTENDED_RETRY — we intentionally
...
export function isUnattendedMode(): boolean {
  const val = process.env['GLM_CODE_UNATTENDED_RETRY'];
...
export async function retryWithBackoff<T>(
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      // Check for GLM OAuth quota exceeded error - throw immediately without retry
      if (authType === AuthType.GLM_OAUTH && isGLMQuotaExceededError(error)) {
        throw new Error(
          `GLM OAuth free tier has been discontinued as of 2026-04-15.\n\n` +
            `To continue using GLM Code, try one of these alternatives:\n` +
