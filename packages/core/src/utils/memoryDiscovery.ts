...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    const globalMemoryPath = path.join(globalGLMDir, geminiMdFilename);
    ...
    if (isHomeDirectory) {
      // For home directory, only check for GLM.md directly in the home directory
      ...
      while (currentDir && currentDir !== path.dirname(currentDir)) {
        if (
          currentDir === globalGLMDir ||
          currentDir === path.join(resolvedHome, GLM_DIR)
        ) {
          break;
        }
...
/**
 * Loads hierarchical GLM.md files and concatenates their content.
 * Also loads path-based context rules from `.glm/rules/` directories.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length > 0) {
    ...
    // Only count files that match configured memory filenames (e.g., GLM.md),
  ...
  // Load path-based context rules from .glm/rules/ directories
  ...
  if (!memoryContent && filePaths.length === 0 && ruleCount === 0) {
    logger.debug('No GLM.md files or rules found.');
  }
