...
//
// Discovers .glm/rules/ files (recursively) with optional YAML frontmatter.
...
/**
 * Discover and load rule files from a single `.glm/rules/` directory.
 ...
 * consistent with the `--- Context from: ... ---` format used for GLM.md.
 */
export function formatRules(rules: RuleFile[], projectRoot: string): string {
  return rules
    .map((rule) => {
      ...
      // display paths should match — otherwise Windows shows `.glm\rules\foo.md`
      // and Linux shows `.glm/rules/foo.md`, which is confusing in diffs/tests.
...
/**
 * Load rules from both global (`~/.glm/rules/`) and project-level
 * (`.glm/rules/`) directories.
 *
...
export async function loadRules(
  ...
  // 1. Global rules: <GLM_HOME or ~/.glm>/rules/
  const globalRulesDir = path.join(Storage.getGlobalGLMDir(), 'rules');
  ...
  // 2. Project-level rules: <projectRoot>/.glm/rules/  (trusted only)
  //    Skip if it resolves to the same directory as global rules.
  if (folderTrust) {
    const projectRulesDir = path.join(projectRoot, GLM_DIR, 'rules');
