...
import { GLM_DIR, unescapePath } from './paths.js';
...
`;
  ...
  describe('loadRules', () => {
    ...
    it('loads baseline rules into content', async () => {
      const rulesDir = path.join(projectRoot, GLM_DIR, 'rules');
...
Use hooks.`,
    ...
    it('skips project rules when folder is untrusted', async () => {
      await createTestFile(
        path.join(projectRoot, GLM_DIR, 'rules', 'r.md'),
        'Untrusted.',
      );
    ...
    it('loads global rules even when folder is untrusted', async () => {
      await createTestFile(
        path.join(homedir, GLM_DIR, 'rules', 'g.md'),
        'Global.',
      );
      const result = await loadRules(projectRoot, false);
      expect(result.ruleCount).toBe(1);
    ...
    it('does not duplicate rules when projectRoot equals homedir', async () => {
      await createTestFile(
        path.join(homedir, GLM_DIR, 'rules', 's.md'),
        'Shared.',
      );
    ...
    it('formats rules with source markers', async () => {
      await createTestFile(
        path.join(projectRoot, GLM_DIR, 'rules', 'test.md'),
        'Content.',
      );
      const result = await loadRules(projectRoot, true);
      expect(result.content).toContain(
        `--- Rule from: ${GLM_DIR}/rules/test.md ---`,
      );
    });

    it('reads global rules from GLM_HOME when set', async () => {
      const customGLMHome = path.join(testRootDir, 'custom-glm-home');
      const originalGLMHome = process.env['GLM_HOME'];
      process.env['GLM_HOME'] = customGLMHome;
      try {
        await createTestFile(
          path.join(customGLMHome, 'rules', 'fromCustomHome.md'),
          'CustomHome rule.',
        );
        // A stale rule in the legacy ~/.glm/rules location should NOT be
        // loaded once GLM_HOME points elsewhere.
        await createTestFile(
          path.join(homedir, GLM_DIR, 'rules', 'fromLegacyHome.md'),
          'LegacyHome rule.',
        );
