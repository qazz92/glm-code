...
import { GLMIgnoreParser } from './glmIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('GLMIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'glmignore-test-'));
  });
  ...
  describe('when .glmignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .glmignore', () => {
      const parser = new GLMIgnoreParser(projectRoot);
  ...
  describe('when .glmignore does not exist', () => {
