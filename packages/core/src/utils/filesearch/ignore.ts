...
export function loadIgnoreRules(options: LoadIgnoreRulesOptions): Ignore {
  ...
  if (options.useGLMignore) {
    const glmignorePath = path.join(options.projectRoot, '.glmignore');
    if (fs.existsSync(glmignorePath)) {
      ignorer.add(fs.readFileSync(glmignorePath, 'utf8'));
    }
  }
