...
describe('quotaErrorDetection', () => {
  describe('isGLMQuotaExceededError', () => {
    it('should detect the GLM insufficient_quota error', () => {
      const error = {
        status: 429,
        code: 'insufficient_quota',
        message: 'Free allocated quota exceeded.',
      };
      expect(isGLMQuotaExceededError(error)).toBe(true);
    });
    ...
    it('should not match when status is not 429', () => {
      ...
      expect(isGLMQuotaExceededError(error)).toBe(false);
    });
    ...
    it('should not match string errors', () => {
      expect(isGLMQuotaExceededError('insufficient_quota')).toBe(false);
    });

    it('should not match null or undefined', () => {
      expect(isGLMQuotaExceededError(null)).toBe(false);
      expect(isGLMQuotaExceededError(undefined)).toBe(false);
    });
  });
