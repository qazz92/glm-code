...
export class ArenaManager {
  ...
  constructor(config: Config, callbacks: ArenaCallbacks = {}) {
    ...
    // Use the user-configured base dir, or default to ~/.glm/arena.
    this.arenaBaseDir =
      arenaSettings?.worktreeBaseDir ??
      path.join(Storage.getGlobalGLMDir(), 'arena');
  ...
  private buildAgentSpawnConfig(agent: ArenaAgentState): AgentSpawnConfig {
    ...
    if (model.apiKey) {
      env['GLM_API_KEY'] = model.apiKey;
    }
    if (model.baseUrl) {
      env['GLM_BASE_URL'] = model.baseUrl;
    }
   ...
   * `~/.glm/worktrees/<sessionId>/`.  The directory contains:
