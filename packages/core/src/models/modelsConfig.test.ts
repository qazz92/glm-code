...
describe('ModelsConfig', () => {
  ...
  it('should always force GLM OAuth apiKey placeholder when applying model defaults', async () => {
    // Simulate a stale/explicit apiKey existing before switching models.
    const modelsConfig = new ModelsConfig({
      initialAuthType: AuthType.GLM_OAUTH,
      generationConfig: {
        apiKey: 'manual-key-should-not-leak',
      },
    });

    // Switching within glm-oauth triggers applyResolvedModelDefaults().
    await modelsConfig.switchModel(AuthType.GLM_OAUTH, 'coder-model');

    const gc = currentGenerationConfig(modelsConfig);
    expect(gc.apiKey).toBe('GLM_OAUTH_DYNAMIC_TOKEN');
  ...
  it('should apply GLM OAuth apiKey placeholder during syncAfterAuthRefresh for fresh users', () => {
    // Fresh user: authType not selected yet (currentAuthType undefined).
    const modelsConfig = new ModelsConfig();

    // Config.refreshAuth passes modelId from modelsConfig.getModel(), which falls back to DEFAULT_GLM_MODEL.
  ...
  it('should use default model for new authType when switching from different authType with env vars', () => {
    ...
    // User switches to glm-oauth via AuthDialog
    // refreshAuth calls syncAfterAuthRefresh with the current model (gpt-4o)
    // which doesn't exist in glm-oauth registry, so it should use default
    modelsConfig.syncAfterAuthRefresh(AuthType.GLM_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default glm-oauth model (coder-model), not the OPENAI model
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('GLM_OAUTH_DYNAMIC_TOKEN');
    expect(gc.apiKeyEnvKey).toBeUndefined();
  });

  it('should clear manual credentials when switching from USE_OPENAI to GLM_OAUTH', () => {
    ...
    // User switches to glm-oauth
    // Since authType is not USE_OPENAI, manual credentials should be cleared
    // and default glm-oauth model should be applied
    modelsConfig.syncAfterAuthRefresh(AuthType.GLM_OAUTH, 'gpt-4o');

    const gc = currentGenerationConfig(modelsConfig);
    // Should use default glm-oauth model, not preserve manual OpenAI credentials
    expect(gc.model).toBe('coder-model');
    expect(gc.apiKey).toBe('GLM_OAUTH_DYNAMIC_TOKEN');
    // baseUrl should be set to glm-oauth default, not preserved from manual OpenAI config
    expect(gc.baseUrl).toBe('DYNAMIC_GLM_OAUTH_BASE_URL');
    expect(gc.apiKeyEnvKey).toBeUndefined();
  });
  ...
  it('should fall back to settings-sourced apiKey when registry model envKey is not in process.env (restart scenario)', () => {
    ...
    modelsConfig.syncAfterAuthRefresh(AuthType.USE_OPENAI, 'glm3.5-plus');
    ...
    expect(gc.model).toBe('glm3.5-plus');
  ...
  it('should maintain consistency between currentModelId and _generationConfig.model after initialization', () => {
    ...
    expect(config3.getModel()).toBe('coder-model'); // Falls back to DEFAULT_GLM_MODEL
  ...
  describe('getAllConfiguredModels', () => {
    it('should return all models across all authTypes and put glm-oauth first', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // glm-oauth models should be ordered first
      const firstNonGLMIndex = allModels.findIndex(
        (m) => m.authType !== AuthType.GLM_OAUTH,
      );
      expect(firstNonGLMIndex).toBeGreaterThan(0);
      expect(
        allModels
          .slice(0, firstNonGLMIndex)
          .every((m) => m.authType === AuthType.GLM_OAUTH),
      ).toBe(true);
      expect(
        allModels
          .slice(firstNonGLMIndex)
          .every((m) => m.authType !== AuthType.GLM_OAUTH),
      ).toBe(true);

      // Should include glm-oauth models (hard-coded)
      const glmModels = allModels.filter(
        (m) => m.authType === AuthType.GLM_OAUTH,
      );
    ...
    it('should return empty array when no models are registered', () => {
      ...
      // Should still include glm-oauth models (hard-coded)
    ...
    it('should support filtering by authTypes and still put glm-oauth first when included', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        openai: [
          {
            id: 'openai-model-1',
      ...
      // Filter: OpenAI only (should not include glm-oauth)
      ...
      // Filter: include glm-oauth but request it later -> still ordered first
      const withGLM = modelsConfig.getAllConfiguredModels([
        AuthType.USE_OPENAI,
        AuthType.GLM_OAUTH,
        AuthType.USE_ANTHROPIC,
      ]);
      expect(withGLM.length).toBeGreaterThan(0);
      const firstNonGLMIndex = withGLM.findIndex(
        (m) => m.authType !== AuthType.GLM_OAUTH,
      );
      expect(firstNonGLMIndex).toBeGreaterThan(0);
      expect(
    ...
    it('should handle reload with empty config', async () => {
      ...
      expect(
        modelsConfig
          ...
          .filter((m) => m.authType !== 'glm-oauth').length,
      ).toBeGreaterThan(0);

      // Reload with empty config
      modelsConfig.reloadModelProvidersConfig({});

      // Only glm-oauth models should remain
      const models = modelsConfig.getAllConfiguredModels();
      expect(models.every((m) => m.authType === 'glm-oauth')).toBe(true);
    });

    it('should preserve glm-oauth models after reload', () => {
      const modelsConfig = new ModelsConfig({
        modelProvidersConfig: {
          openai: [{ id: 'gpt-4', name: 'GPT-4' }],
        },
      ...
      const initialGLMModels = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'glm-oauth');

      modelsConfig.reloadModelProvidersConfig({
        gemini: [{ id: 'gemini-pro', name: 'Gemini Pro' }],
      });

      // glm-oauth models should still exist
      const glmModelsAfterReload = modelsConfig
        .getAllConfiguredModels()
        .filter((m) => m.authType === 'glm-oauth');
      expect(glmModelsAfterReload.length).toBe(initialGLMModels.length);
    });
