...
import { GLM_OAUTH_MODELS } from './constants.js';
import { createDebugLogger } from '../utils/debugLogger.js';

const debugLogger = createDebugLogger('MODEL_REGISTRY');

export { GLM_OAUTH_MODELS } from './constants.js';
...
export class ModelRegistry {
  ...
  private getDefaultBaseUrl(authType: AuthType): string {
    switch (authType) {
      case AuthType.GLM_OAUTH:
        return 'DYNAMIC_GLM_OAUTH_BASE_URL';
  ...
  constructor(modelProvidersConfig?: ModelProvidersConfig) {
    ...
    // Always register glm-oauth models (hard-coded, cannot be overridden)
    this.registerAuthTypeModels(AuthType.GLM_OAUTH, GLM_OAUTH_MODELS);
    ...
    if (modelProvidersConfig) {
      for (const [rawKey, models] of Object.entries(modelProvidersConfig)) {
        ...
        // Skip glm-oauth as it uses hard-coded models
   ...
   * For glm-oauth, returns the coder model.
  ...
  getDefaultModelForAuthType(
    ...
    if (authType === AuthType.GLM_OAUTH) {
      return this.getModel(authType, DEFAULT_GLM_MODEL);
    }
    const models = this.modelsByAuthType.get(authType);
   ...
   * Preserves hard-coded glm-oauth models.
   */
  reloadModels(modelProvidersConfig?: ModelProvidersConfig): void {
    // Clear existing user-configured models (preserve glm-oauth)
