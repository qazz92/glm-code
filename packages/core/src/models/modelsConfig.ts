...
export class ModelsConfig {
  ...
  // One-shot flag for glm-oauth credential caching
  private requireCachedGLMCredentialsOnce: boolean = false;
  ...
  private createStateSnapshotForRollback(): {
    ...
    requireCachedGLMCredentialsOnce: boolean;
    ...
    return {
      ...
      requireCachedGLMCredentialsOnce: this.requireCachedGLMCredentialsOnce,
  ...
  private rollbackToStateSnapshot(
    ...
    this.requireCachedGLMCredentialsOnce =
      snapshot.requireCachedGLMCredentialsOnce;
   ...
   */
  getModel(): string {
    return this._generationConfig.model || DEFAULT_GLM_MODEL;
  }
   ...
   * - glm-oauth models are always ordered first.
  ...
  getAllConfiguredModels(authTypes?: AuthType[]): AvailableModel[] {
    ...
    // Order: openai (z.ai) first, then others.
    const orderedAuthTypes: AuthType[] = [];
    if (uniqueAuthTypes.includes(AuthType.USE_OPENAI)) {
      orderedAuthTypes.push(AuthType.USE_OPENAI);
    }
  ...
  async setModel(
    ...
    // Special case: glm-oauth model switch - hot update in place
    // coder-model supports vision capabilities and can be hot-updated
    if (
      this.currentAuthType === AuthType.GLM_OAUTH &&
      newModel === DEFAULT_GLM_MODEL
    ) {
      ...
      if (this.onModelChange) {
        await this.onModelChange(AuthType.GLM_OAUTH, false);
      }
      return;
    }
  ...
  async switchModel(
    ...
    if (authType === AuthType.GLM_OAUTH && options?.requireCachedCredentials) {
      this.requireCachedGLMCredentialsOnce = true;
    }
  ...
  consumeRequireCachedCredentialsFlag(): boolean {
    const value = this.requireCachedGLMCredentialsOnce;
    this.requireCachedGLMCredentialsOnce = false;
    return value;
  }
  ...
  private applyResolvedModelDefaults(model: ResolvedModelConfig): void {
    ...
    // For GLM OAuth, apiKey must always be a placeholder. It will be dynamically
    ...
    // replaced later.)
    if (this.currentAuthType === AuthType.GLM_OAUTH) {
      this._generationConfig.apiKey = 'GLM_OAUTH_DYNAMIC_TOKEN';
      this.generationConfigSources['apiKey'] = {
        kind: 'computed',
        detail: 'GLM OAuth placeholder token',
      };
      this._generationConfig.apiKeyEnvKey = undefined;
   ...
   *
   * Examples:
   * - GLM OAuth: coder-model switches (same authType, hot-update safe)
   ...
   * - OpenAI -> GLM OAuth: handled by switchModel(authType, modelId), always refreshes
   * - GLM OAuth -> OpenAI: handled by switchModel(authType, modelId), always refreshes
   */
  private checkRequiresRefresh(previousModelId: string): boolean {
    ...
    // For GLM OAuth, model switches within the same authType can always be hot-updated
  ...
  private getRuntimeModelOption(): AvailableModel | undefined {
    ...
    return {
      ...
      /**
       * `isVision` is for automatic switching of glm-oauth vision model.
