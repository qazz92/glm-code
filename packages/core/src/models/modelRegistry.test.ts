...
describe('ModelRegistry', () => {
  describe('initialization', () => {
    it('should always include hard-coded glm-oauth models', () => {
      const registry = new ModelRegistry();

      const glmModels = registry.getModelsForAuthType(AuthType.GLM_OAUTH);
      expect(glmModels.length).toBe(GLM_OAUTH_MODELS.length);
      expect(glmModels[0].id).toBe('coder-model');
    });

    it('should initialize with empty config', () => {
      const registry = new ModelRegistry();
      expect(registry.getModelsForAuthType(AuthType.GLM_OAUTH).length).toBe(
        GLM_OAUTH_MODELS.length,
      );
    ...
    it('should ignore glm-oauth models in config (hard-coded)', () => {
      const modelProvidersConfig: ModelProvidersConfig = {
        'glm-oauth': [
          {
            id: 'custom-glm',
      ...
      // Should still use hard-coded glm-oauth models
      const glmModels = registry.getModelsForAuthType(AuthType.GLM_OAUTH);
      expect(glmModels.length).toBe(GLM_OAUTH_MODELS.length);
      expect(glmModels.find((m) => m.id === 'custom-glm')).toBeUndefined();
    });
  });
  ...
  describe('modalities auto-fill', () => {
    ...
    it('returns text-only ({}) for models with no multimodal default', () => {
      ...
      const model = registry.getModel(AuthType.USE_OPENAI, 'glm3-coder-plus');
  ...
  describe('getDefaultModelForAuthType', () => {
    it('should return coder-model for glm-oauth', () => {
      const registry = new ModelRegistry();
  ...
  describe('default base URLs', () => {
    it('should apply default dashscope URL for glm-oauth', () => {
      const registry = new ModelRegistry();
      const model = registry.getModel(AuthType.GLM_OAUTH, 'coder-model');
      expect(model?.baseUrl).toBe('DYNAMIC_GLM_OAUTH_BASE_URL');
    });
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded glm-oauth models after reload', () => {
      const registry = new ModelRegistry({
        openai: [{ id: 'gpt-4', name: 'GPT-4' }],
      });
      ...
      expect(
        registry.getModel(AuthType.GLM_OAUTH, 'coder-model'),
      ).toBeDefined();
    });
    ...
    it('should ignore glm-oauth models in reload config', () => {
      const registry = new ModelRegistry();

      registry.reloadModels({
        'glm-oauth': [{ id: 'custom-glm', name: 'Custom GLM' }],
      });

      // glm-oauth should still use hard-coded models
      const glmModels = registry.getModelsForAuthType(AuthType.GLM_OAUTH);
      expect(glmModels.length).toBe(GLM_OAUTH_MODELS.length);
      expect(glmModels.find((m) => m.id === 'custom-glm')).toBeUndefined();
    });
