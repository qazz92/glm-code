...
/**
 * Applies GLM_CODE_API_TIMEOUT_MS env override if modelProvider has not set a timeout.
...
function applyTimeoutEnvOverride(
  ...
  const raw = env['GLM_CODE_API_TIMEOUT_MS'];
  ...
  if (Number.isFinite(parsed) && parsed > 0) {
    ...
    sources['timeout'] = {
      kind: 'env',
      envKey: 'GLM_CODE_API_TIMEOUT_MS',
    };
  }
}
...
export function resolveModelConfig(
  ...
  // Special handling for GLM OAuth
  if (authType === AuthType.GLM_OAUTH) {
    return resolveGLMOAuthConfig(input, warnings);
  }
  ...
  // ---- Env override: GLM_CODE_API_TIMEOUT_MS ----
...
/**
 * Special resolver for GLM OAuth authentication.
 * GLM OAuth has fixed model options and uses dynamic tokens.
 */
function resolveGLMOAuthConfig(
  input: ModelConfigSourcesInput,
  warnings: string[],
): ModelConfigResolutionResult {
  ...
  // GLM OAuth only allows specific models
  const allowedModels = new Set<string>(GLM_OAUTH_ALLOWED_MODELS);
  ...
  if (requestedModel && allowedModels.has(requestedModel)) {
    ...
    if (requestedModel) {
      ...
      const extraMessage = isVisionModel
        ...
        : '';
      warnings.push(
        `Warning: Unsupported GLM OAuth model '${requestedModel}', falling back to '${DEFAULT_GLM_MODEL}'.${extraMessage}`,
      );
    }
    resolvedModel = DEFAULT_GLM_MODEL;
    modelSource = defaultSource(`fallback to '${DEFAULT_GLM_MODEL}'`);
  }

  sources['model'] = modelSource;
  sources['apiKey'] = computedSource('GLM OAuth dynamic token');
  sources['authType'] = computedSource('provided by caller');
