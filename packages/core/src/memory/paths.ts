...
import { GLM_DIR, sanitizeCwd } from '../utils/paths.js';
 ...
 * Defaults to the global glm dir (`~/.glm` or `$GLM_HOME`);
 */
export function getMemoryBaseDir(): string {
  if (process.env['GLM_CODE_MEMORY_BASE_DIR']) {
    return process.env['GLM_CODE_MEMORY_BASE_DIR'];
  }
...
export function getAutoMemoryRoot(projectRoot: string): string {
  ...
  let result: string;
  if (process.env['GLM_CODE_MEMORY_LOCAL'] === '1') {
    result = path.join(projectRoot, GLM_DIR, AUTO_MEMORY_DIRNAME);
  } else {
    const canonicalRoot =
