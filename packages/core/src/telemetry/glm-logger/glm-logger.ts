...
export class GLMLogger {
  private static instance: GLMLogger;
  ...
  private constructor(config: Config) {
    this.config = config;
    this.debugLogger = createDebugLogger('GLM_LOGGER');
  ...
  static getInstance(config?: Config): GLMLogger | undefined {
    if (config === undefined || !config?.getUsageStatisticsEnabled())
      return undefined;
    if (!GLMLogger.instance) {
      GLMLogger.instance = new GLMLogger(config);
    }

    return GLMLogger.instance;
  }
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `GLMLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
      this.debugLogger.error('GLMLogger: Failed to enqueue log event.', error);
    }
  }
  ...
  readSourceInfo(): string {
    try {
      const globalDir = Storage.getGlobalGLMDir();
      const sourceJsonPath = path.join(globalDir, 'source.json');

      // Also check legacy ~/.glm/source.json when GLM_HOME is set,
      // since the installer writes to ~/.glm/ regardless of the env var.
      const legacyPath = path.join(os.homedir(), '.glm', 'source.json');
      const candidates =
  ...
  async flushToRum(): Promise<LogResponse> {
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'GLMLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `GLMLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
