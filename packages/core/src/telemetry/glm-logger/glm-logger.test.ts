...
import { GLMLogger, TEST_ONLY } from './glm-logger.js';
...
const makeFakeConfig = (overrides: Partial<Config> = {}): Config => {
  const defaults = {
    ...
    getAuthType: () => AuthType.GLM_OAUTH,
    getMcpServers: () => ({}),
    getModel: () => 'test-model',
...
describe('GLMLogger', () => {
  ...
  beforeEach(() => {
    ...
    (GLMLogger as any).instance = undefined;
  });
  ...
  describe('getInstance', () => {
    it('returns undefined when usage statistics are disabled', () => {
      ...
      const logger = GLMLogger.getInstance(config);
      expect(logger).toBeUndefined();
    });

    it('returns an instance when usage statistics are enabled', () => {
      const logger = GLMLogger.getInstance(mockConfig);
      expect(logger).toBeInstanceOf(GLMLogger);
    });

    it('is a singleton', () => {
      const logger1 = GLMLogger.getInstance(mockConfig);
      const logger2 = GLMLogger.getInstance(mockConfig);
  ...
  describe('logHookCallEvent', () => {
    ...
    it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
      ...
      const logger = GLMLogger.getInstance(configWithLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should not include error when telemetry log prompts disabled', () => {
      ...
      (GLMLogger as unknown as { instance: undefined }).instance = undefined;
      const logger = GLMLogger.getInstance(configWithoutLogPrompts)!;
      const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
    ...
    it('should sanitize hook name to remove sensitive information', () => {
      ...
      const event = new HookCallEvent(
        'PreToolUse',
        'command',
        '/home/user/.glm/hooks/check-secrets.sh --api-key=secret123',
        { tool_name: 'read_file' },
        100,
        true,
      );
