...
import { GLMLogger } from './glm-logger/glm-logger.js';
...
export function logStartSession(
  ...
  GLMLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config: Config, event: UserPromptEvent): void {
  GLMLogger.getInstance(config)?.logNewPromptEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserRetry(config: Config, event: UserRetryEvent): void {
  GLMLogger.getInstance(config)?.logRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolCall(config: Config, event: ToolCallEvent): void {
  ...
  GLMLogger.getInstance(config)?.logToolCallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logToolOutputTruncated(
  ...
  GLMLogger.getInstance(config)?.logToolOutputTruncatedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFileOperation(
  ...
  GLMLogger.getInstance(config)?.logFileOperationEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiRequest(config: Config, event: ApiRequestEvent): void {
  // GLMLogger.getInstance(config)?.logApiRequestEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logFlashFallback(
  ...
  GLMLogger.getInstance(config)?.logFlashFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logRipgrepFallback(
  ...
  GLMLogger.getInstance(config)?.logRipgrepFallbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiError(config: Config, event: ApiErrorEvent): void {
  ...
  GLMLogger.getInstance(config)?.logApiErrorEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiCancel(config: Config, event: ApiCancelEvent): void {
  ...
  uiTelemetryService.addEvent(uiEvent);
  GLMLogger.getInstance(config)?.logApiCancelEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logApiResponse(config: Config, event: ApiResponseEvent): void {
  ...
  GLMLogger.getInstance(config)?.logApiResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logLoopDetected(
  ...
  GLMLogger.getInstance(config)?.logLoopDetectedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logLoopDetectionDisabled(
  ...
  GLMLogger.getInstance(config)?.logLoopDetectionDisabledEvent();
}
...
export function logNextSpeakerCheck(
  ...
  GLMLogger.getInstance(config)?.logNextSpeakerCheck(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSlashCommand(
  ...
  GLMLogger.getInstance(config)?.logSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logIdeConnection(
  ...
  GLMLogger.getInstance(config)?.logIdeConnectionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logConversationFinishedEvent(
  ...
  GLMLogger.getInstance(config)?.logConversationFinishedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logChatCompression(
  ...
  GLMLogger.getInstance(config)?.logChatCompressionEvent(event);
...
export function logKittySequenceOverflow(
  ...
  GLMLogger.getInstance(config)?.logKittySequenceOverflowEvent(event);
  if (!isTelemetrySdkInitialized()) return;
  const attributes: LogAttributes = {
    ...getCommonAttributes(config),
    ...event,
...
export function logMalformedJsonResponse(
  ...
  GLMLogger.getInstance(config)?.logMalformedJsonResponseEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logInvalidChunk(
  ...
  GLMLogger.getInstance(config)?.logInvalidChunkEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetry(
  ...
  GLMLogger.getInstance(config)?.logContentRetryEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logContentRetryFailure(
  ...
  GLMLogger.getInstance(config)?.logContentRetryFailureEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logSubagentExecution(
  ...
  GLMLogger.getInstance(config)?.logSubagentExecutionEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logModelSlashCommand(
  ...
  GLMLogger.getInstance(config)?.logModelSlashCommandEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logHookCall(config: Config, event: HookCallEvent): void {
  // Log to GLMLogger for RUM telemetry only
  GLMLogger.getInstance(config)?.logHookCallEvent(event);
}
...
export function logExtensionInstallEvent(
  ...
  GLMLogger.getInstance(config)?.logExtensionInstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionUninstall(
  ...
  GLMLogger.getInstance(config)?.logExtensionUninstallEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export async function logExtensionUpdateEvent(
  ...
  GLMLogger.getInstance(config)?.logExtensionUpdateEvent(event);
...
export function logExtensionEnable(
  ...
  GLMLogger.getInstance(config)?.logExtensionEnableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logExtensionDisable(
  ...
  GLMLogger.getInstance(config)?.logExtensionDisableEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logAuth(config: Config, event: AuthEvent): void {
  GLMLogger.getInstance(config)?.logAuthEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logUserFeedback(
  ...
  uiTelemetryService.addEvent(uiEvent);
  config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
  GLMLogger.getInstance(config)?.logUserFeedbackEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionStarted(
  ...
  GLMLogger.getInstance(config)?.logArenaSessionStartedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaAgentCompleted(
  ...
  GLMLogger.getInstance(config)?.logArenaAgentCompletedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
...
export function logArenaSessionEnded(
  ...
  GLMLogger.getInstance(config)?.logArenaSessionEndedEvent(event);
  if (!isTelemetrySdkInitialized()) return;
