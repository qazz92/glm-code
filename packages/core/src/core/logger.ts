...
export class Logger {
  private glmDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.glmDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.glmDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.glmDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.glmDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.glmDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.glmDir) {
      this.debugLogger.error(
