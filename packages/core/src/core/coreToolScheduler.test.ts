    ...
    it('should return permission error for excluded tools instead of "not found" message', async () => {
      ...
      if (completedCall.status === 'error') {
        ...
        expect(errorMessage).toBe(
          'GLM Code requires permission to use write_file, but that permission was declined.',
        );
...
describe('Fire hook functions integration', () => {
  ...
  describe('Concurrent tool execution', () => {
    ...
    const origEnv = process.env['GLM_CODE_MAX_TOOL_CONCURRENCY'];
    beforeEach(() => {
      delete process.env['GLM_CODE_MAX_TOOL_CONCURRENCY'];
    });
    afterEach(() => {
      if (origEnv !== undefined) {
        process.env['GLM_CODE_MAX_TOOL_CONCURRENCY'] = origEnv;
      } else {
        delete process.env['GLM_CODE_MAX_TOOL_CONCURRENCY'];
      }
    });
