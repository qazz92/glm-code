...
import { assembleContext, createBlock, formatBudgetSummary } from '../memory/context-assembler.js';
...
export function getCoreSystemPrompt(
  ...
  // if GLM_SYSTEM_MD is set (and not 0|false), override system prompt from file
  // default path is .glm/system.md (project-level), can be overridden via GLM_SYSTEM_MD
  let systemMdEnabled = false;
  let systemMdPath = path.resolve(path.join(GLM_DIR, 'system.md'));
  // Resolve the environment variable to get either a path or a switch value.
  const systemMdResolution = resolvePathFromEnv(process.env['GLM_SYSTEM_MD']);
  ...
  const basePrompt = systemMdEnabled
    ...
    : `
You are GLM Code, an interactive CLI agent powered by z.ai, specializing in software engineering tasks. Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.
...
`.trim();
  ...
  // if GLM_WRITE_SYSTEM_MD is set (and not 0|false), write base system prompt to file
  const writeSystemMdResolution = resolvePathFromEnv(
    process.env['GLM_WRITE_SYSTEM_MD'],
  );
...
/**
 * Build a 6-block context structure with token budget tracking.
 * Returns the assembled context budget summary string for observability.
 */
export function buildContextBudget(params: {
  ...
  const blocks = [
    createBlock('system', params.systemPrompt),
    createBlock('skills', params.skillsText ?? ''),
    createBlock('agents_md', params.agentsMdText ?? ''),
    createBlock('summary', params.summaryText ?? ''),
    createBlock('history', params.historyText ?? ''),
    createBlock('user_turn', params.userTurnText ?? ''),
    // Legacy blocks for backward compat with callers that still pass these
    createBlock('memories', params.userMemory ?? ''),
    createBlock('tool_result', params.toolResultText ?? ''),
  ].filter((b) => b.content.length > 0);

  const budget = assembleContext(blocks, params.maxContextTokens);

  if (budget.needsCompaction) {
    debugLogger.warn(formatBudgetSummary(budget));
  }

  return formatBudgetSummary(budget);
}
...
Carefully consider the reversibility and blast radius of actions. Generally you can freely take local, reversible actions like editing files or running tests. But for actions that are hard to reverse, affect shared systems beyond your local environment, or could otherwise be risky or destructive, check with the user before proceeding. The cost of pausing to confirm is low, while the cost of an unwanted action (lost work, unintended messages sent, deleted branches) can be very high. For actions like these, consider the context, the action, and user instructions, and by default transparently communicate the action and ask for confirmation before proceeding. This default can be changed by user instructions - if explicitly asked to operate more autonomously, then you may proceed without confirmation, but still attend to the risks and consequences when taking actions. A user approving an action (like a git push) once does NOT mean that they approve it in all contexts, so unless actions are authorized in advance in durable instructions like GLM.md files, always confirm first. Authorization stands for the scope specified, not beyond. Match the scope of your actions to what was actually requested.
...
function getToolCallExamples(model?: string): string {
  ...
  const toolCallStyle = process.env['GLM_CODE_TOOL_CALL_STYLE'];
  if (toolCallStyle) {
    switch (toolCallStyle.toLowerCase()) {
      case 'glm':
        return glmToolCallExamples;
      case 'glm-vl':
        return glmVlToolCallExamples;
      case 'general':
        return generalToolCallExamples;
      default:
        debugLogger.warn(
          `Unknown GLM_CODE_TOOL_CALL_STYLE value: ${toolCallStyle}. Using model-based detection.`,
        );
        break;
    }
  }
  ...
  if (model && model.length < 100) {
    // Match GLM coder patterns (e.g., GLM-5.1, GLM-4.5, glm-coder)
    if (/glm/i.test(model) && !/vl/i.test(model)) {
      return glmToolCallExamples;
    }
    // Match GLM vision-language patterns (e.g., GLM-4V, glm-vl)
    if (/glm.*vl/i.test(model)) {
      return glmVlToolCallExamples;
    }
    // Match GLM-5.1 as default coder model
...
const INSIGHT_PROMPTS: Record<InsightPromptType, string> = {
  analysis: `Analyze this GLM Code session and extract structured facets.

CRITICAL GUIDELINES:

1. **goal_categories**: Count ONLY what the USER explicitly asked for.
   - DO NOT count GLM Code's autonomous codebase exploration
   - DO NOT count work GLM Code decided to do on its own
...
3. **friction_counts**: Be specific about what went wrong.
   - misunderstood_request: GLM Code interpreted incorrectly
...
4. If very short or just warmup, use warmup_minimal for goal_category`,
  ...
  impressive_workflows: `Analyze this GLM Code usage data and identify what's working well for this user. Use second person ("you").
...
Include 3 impressive workflows.`,
  ...
  project_areas: `Analyze this GLM Code usage data and identify project areas.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "areas": [
    {"name": "Area name", "session_count": N, "description": "2-3 sentences about what was worked on and how GLM Code was used."}
  ]
}

Include 4-5 areas. Skip internal QC operations.`,

  future_opportunities: `Analyze this GLM Code usage data and identify future opportunities.
...
Include 3 opportunities. Think BIG - autonomous workflows, parallel agents, iterating against tests.`,
  ...
  friction_points: `Analyze this GLM Code usage data and identify friction points for this user. Use second person ("you").
...
Include 3 friction categories with 2 examples each.`,
  ...
  memorable_moment: `Analyze this GLM Code usage data and find a memorable moment.
...
Find something genuinely interesting or amusing from the session summaries.`,
  ...
  improvements: `Analyze this GLM Code usage data and suggest improvements.

## QC FEATURES REFERENCE (pick from these for features_to_try):
1. **MCP Servers**: Connect GLM Code to external tools, databases, and APIs via Model Context Protocol.
   - How to use: Run \`glm mcp add --transport http <server-name> <http-url>\`
   - Good for: database queries, Slack integration, GitHub issue lookup, connecting to internal APIs
   - Example: "To connect to GitHub, run \`glm mcp add --header "Authorization: Bearer your_github_mcp_pat" --transport http github https://api.githubcopilot.com/mcp/\` and set the AUTHORIZATION header with your PAT. Then you can ask GLM Code to query issues, PRs, or repos."

2. **Custom Skills**: Reusable prompts you define as markdown files that run with a single /command.
   - How to use: Create \`.glm/skills/commit/SKILL.md\` with instructions. Then type \`/commit\` to run it.
...
3. **Headless Mode**: Run GLM Code non-interactively from scripts and CI/CD.
   - How to use: \`glm -p "fix lint errors"\`
   - Good for: CI/CD integration, batch code fixes, automated reviews

4. **Task Agents**: GLM Code spawns focused sub-agents for complex exploration or parallel work.
   - How to use: GLM Code auto-invokes when helpful, or ask "use an agent to explore X"
   - Good for: codebase exploration, understanding complex systems

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "GLM_md_additions": [
    {"addition": "A specific line or block to add to GLM.md based on workflow patterns. E.g., 'Always run tests after modifying auth-related files'", "why": "1 sentence explaining why this would help based on actual sessions", "prompt_scaffold": "Instructions for where to add this in GLM.md. E.g., 'Add under ## Testing section'"}
  ],
  "features_to_try": [
...
IMPORTANT for GLM_md_additions: PRIORITIZE instructions that appear MULTIPLE TIMES in the user data. If user told GLM the same thing in 2+ sessions (e.g., 'always run tests', 'use TypeScript'), that's a PRIME candidate - they shouldn't have to repeat themselves.

IMPORTANT for features_to_try: Pick 2-3 from the QC FEATURES REFERENCE above. Include 2-3 items for each category.`,

  interaction_style: `Analyze this GLM Code usage data and describe the user's interaction style.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "narrative": "2-3 paragraphs analyzing HOW the user interacts with GLM Code. Use second person 'you'. Describe patterns: iterate quickly vs detailed upfront specs? Interrupt often or let GLM Code run? Include specific examples. Use **bold** for key insights.",
  "key_pattern": "One sentence summary of most distinctive interaction style"
}
`,

  at_a_glance: `You're writing an "At a Glance" summary for a GLM Code usage insights report for GLM Code users. The goal is to help them understand their usage and improve how they can use GLM Code better, especially as models improve.

Use this 4-part structure:

1. **What's working** - What is the user's unique style of interacting with GLM Code and what are some impactful things they've done? You can include one or two details, but keep it high level since things might not be fresh in the user's memory. Don't be fluffy or overly complimentary. Also, don't focus on the tool calls they use.

2. **What's hindering you** - Split into (a) GLM Code's fault (misunderstandings, wrong approaches, bugs) and (b) user-side friction (not providing enough context, environment issues -- ideally more general than just one project). Be honest but constructive.

3. **Quick wins to try** - Specific GLM Code features they could try from the examples below, or a workflow technique if you think it's really compelling. (Avoid stuff like "Ask GLM Code to confirm before taking actions" or "Type out more context up front" which are less compelling.)
