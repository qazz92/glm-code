...
describe('normalize', () => {
  ...
  it('should handle pipe and colon separators', () => {
    expect(normalize('glm|glm2.5:glm2.5-1m')).toBe('glm2.5-1m');
  });
  ...
  it('should remove quantization and numeric suffixes', () => {
    expect(normalize('glm3-coder-7b-4bit')).toBe('glm3-coder-7b');
    ...
    expect(normalize('glm2.5-quantized')).toBe('glm2.5');
  });
  ...
  it('should not remove "-latest" from specific GLM model names', () => {
    expect(normalize('glm-plus-latest')).toBe('glm-plus-latest');
    expect(normalize('glm-flash-latest')).toBe('glm-flash-latest');
    expect(normalize('glm-vl-max-latest')).toBe('glm-vl-max-latest');
  });
...
describe('tokenLimit', () => {
  ...
  describe('Alibaba GLM', () => {
    it('should return 1M for commercial GLM3 models', () => {
      expect(tokenLimit('glm3-coder-plus')).toBe(1000000);
      expect(tokenLimit('glm3-coder-plus-20250601')).toBe(1000000);
      expect(tokenLimit('glm3-coder-flash')).toBe(1000000);
      expect(tokenLimit('glm3.5-plus')).toBe(1000000);
      expect(tokenLimit('coder-model')).toBe(1000000);
    });

    it('should return 256K for GLM3 non-commercial models', () => {
      expect(tokenLimit('glm3-max')).toBe(262144);
      expect(tokenLimit('glm3-max-2026-01-23')).toBe(262144);
      expect(tokenLimit('glm3-vl-plus')).toBe(262144);
      expect(tokenLimit('glm3-coder-7b')).toBe(262144);
      expect(tokenLimit('glm3-coder-next')).toBe(262144);
    });

    it('should return 1M for studio latest models', () => {
      expect(tokenLimit('glm-plus-latest')).toBe(1000000);
      expect(tokenLimit('glm-flash-latest')).toBe(1000000);
    });

    it('should return 256K for GLM fallback', () => {
      expect(tokenLimit('glm-plus')).toBe(262144);
      expect(tokenLimit('glm-turbo')).toBe(262144);
      expect(tokenLimit('glm2.5')).toBe(262144);
      expect(tokenLimit('glm-vl-max-latest')).toBe(262144);
    });
  });
...
describe('knownTokenLimit', () => {
  it('returns a limit for known input models', () => {
    expect(knownTokenLimit('glm3-max')).toBe(262144);
    expect(knownTokenLimit('gpt-5')).toBe(272000);
  });

  it('returns a limit for known output models', () => {
    expect(knownTokenLimit('glm3-max', 'output')).toBe(32768);
  });
...
describe('tokenLimit with output type', () => {
  ...
  describe('GLM output limits', () => {
    it('should return correct output limits for GLM models', () => {
      expect(tokenLimit('glm3.5-plus', 'output')).toBe(65536);
      expect(tokenLimit('glm3.6-plus', 'output')).toBe(65536);
      expect(tokenLimit('coder-model', 'output')).toBe(65536);
      // Models without specific output limits fall back to GLM default (32K)
      expect(tokenLimit('glm3-max', 'output')).toBe(32768);
      expect(tokenLimit('glm3-max-2026-01-23', 'output')).toBe(32768);
    });
  });
  ...
  describe('input vs output comparison', () => {
    it('should return different limits for input vs output', () => {
      expect(tokenLimit('glm3-max', 'input')).toBe(262144);
      expect(tokenLimit('glm3-max', 'output')).toBe(32768);
    });
