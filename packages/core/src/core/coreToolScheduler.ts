...
import { IdempotencyCache } from '../tools/idempotency-cache.js';
import {
...
export class CoreToolScheduler {
  ...
  private idempotencyCache = new IdempotencyCache();
      ...
      for (const reqInfo of requestsToProcess) {
        ...
        if (pm && !(await pm.isToolEnabled(reqInfo.name))) {
          ...
          const ruleInfo = matchingRule
            ...
            : '';
          const permissionErrorMessage = `GLM Code requires permission to use "${reqInfo.name}", but that permission was declined.${ruleInfo}`;
          newToolCalls.push({
            status: 'error',
            request: reqInfo,
        ...
        if (!pm) {
          ...
          if (excludeTools && excludeTools.length > 0) {
            ...
            if (excludedMatch) {
              const permissionErrorMessage = `GLM Code requires permission to use ${excludedMatch}, but that permission was declined.`;
              newToolCalls.push({
                status: 'error',
                request: reqInfo,
                response: createErrorResponse(
        ...
        try {
          ...
          if (
            ...
            if (isNonInteractiveDeny) {
              const errorMessage = `GLM Code requires permission to use "${reqInfo.name}", but that permission was declined (non-interactive mode cannot prompt for confirmation).`;
              this.setStatusInternal(
                reqInfo.callId,
                'error',
                createErrorResponse(
            ...
            if (hooksEnabled && messageBus) {
              fireNotificationHook(
                messageBus,
                `GLM Code needs your permission to use ${reqInfo.name}`,
  ...
  private async runConcurrently(
    ...
    const parsed = parseInt(
      process.env['GLM_CODE_MAX_TOOL_CONCURRENCY'] || '',
      10,
    );
    ...
    return withSpan(
      ...
      async (span: Span) => {
        ...
        if (cached !== null) {
          ...
          this.setStatusInternal(callId, 'success', cachedResponse);
          return;
        }
        ...
        try {
          ...
          if (toolResult.error === undefined) {
            ...
            if (typeof content === 'string') {
              this.idempotencyCache.set(cacheKey, content);
            }
          } else {
            // It is a failure
            // PostToolUseFailure Hook
