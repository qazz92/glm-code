 ...
 * `x-api-key` auth and a truthful `GLMCode` User-Agent (avoids identity
...
export class AnthropicContentGenerator implements ContentGenerator {
  ...
  private buildHeaders(useProxyIdentity: boolean): Record<string, string> {
    ...
    // truthful GLMCode User-Agent so usage isn't misattributed to Claude
    ...
    const userAgent = useProxyIdentity
      ...
      : `GLMCode/${version} (${process.platform}; ${process.arch})`;
   ...
   * `enableCacheControl` in-place on the glm-oauth path (without
   * recreating the ContentGenerator); non-glm-oauth providers refresh
  ...
  private async buildRequest(
    ...
    // on the glm-oauth path, where `Config.handleModelChange()`
    // hot-updates `enableCacheControl` in place without recreating the
    // ContentGenerator. (Non-glm-oauth providers refresh via generator
  ...
  private buildSamplingParameters(request: GenerateContentParameters): {
    ...
    if (userMaxTokens !== undefined && userMaxTokens !== null) {
      ...
      const envVal = process.env['GLM_CODE_MAX_OUTPUT_TOKENS'];
