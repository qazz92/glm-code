...
export function validateModelConfig(
  ...
  // GLM OAuth doesn't need validation - it uses dynamic tokens
  if (config.authType === AuthType.GLM_OAUTH) {
...
export async function createContentGenerator(
  ...
  } else if (authType === AuthType.GLM_OAUTH) {
    const { getGLMOAuthClient: getGLMOauthClient } = await import(
      '../glm/glmOAuth2.js'
    );
    const { GLMContentGenerator } = await import(
      '../glm/glmContentGenerator.js'
    );

    try {
      const glmClient = await getGLMOauthClient(
        config,
        isInitialAuth ? { requireCachedCredentials: true } : undefined,
      );
      baseGenerator = new GLMContentGenerator(
        glmClient,
        generatorConfig,
        config,
      );
