...
export class DashScopeOpenAICompatibleProvider extends DefaultOpenAICompatibleProvider {
  ...
  static isDashScopeProvider(
    ...
    if (authType === AuthType.GLM_OAUTH) return true;
  ...
  override buildHeaders(): Record<string, string | undefined> {
    ...
    const userAgent = `GLMCode/${version} (${process.platform}; ${process.arch})`;
  ...
  private static readonly VISION_MODEL_PREFIX_PATTERNS = [
    'glm-vl', // glm-vl-max, glm-vl-max-latest, etc.
    'glm3-vl-plus', // glm3-vl-plus variants
    'glm3.5-plus', // glm3.5-plus (has built-in vision capabilities)
  ];
