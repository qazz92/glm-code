...
export function normalize(model: string): string {
  ...
  // - GLM models: glm-plus-latest, glm-flash-latest, glm-vl-max-latest
  // - Kimi models: kimi-k2-0905, kimi-k2-0711, etc. (keep date for version distinction)
  if (
    !s.match(/^glm-(?:plus|flash|vl-max)-latest$/) &&
    !s.match(/^kimi-k2-\d{4}$/)
  ) {
    // Regex breakdown:
...
const PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  [/^glm3-coder-plus/, LIMITS['1m']],
  [/^glm3-coder-flash/, LIMITS['1m']],
  [/^glm3\.\d/, LIMITS['1m']],
  [/^glm-plus-latest$/, LIMITS['1m']],
  [/^glm-flash-latest$/, LIMITS['1m']],
  [/^coder-model$/, LIMITS['1m']],
  // Commercial API models (256K context)
  [/^glm3-max/, LIMITS['256k']],
  // Open-source GLM3 variants: 256K native
  [/^glm3-coder-/, LIMITS['256k']],
  // GLM fallback (VL, turbo, plus, 2.5, etc.): 128K
  [/^glm/, LIMITS['256k']],
...
const OUTPUT_PATTERNS: Array<[RegExp, TokenCount]> = [
  ...
  [/^glm/, LIMITS['32k']], // GLM fallback (VL, turbo, plus, etc.): 8K
