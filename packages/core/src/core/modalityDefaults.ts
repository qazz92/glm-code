...
const MODALITY_PATTERNS: Array<[RegExp, InputModalities]> = [
  ...
  // Alibaba / GLM
  // -------------------
  // GLM3.5-Plus, GLM3.6-Plus: image + video support
  [/^glm3\.5-plus/, { image: true, video: true }],
  [/^glm3\.6-plus/, { image: true, video: true }],
  [/^coder-model$/, { image: true, video: true }],

  // GLM VL (vision-language) models: image + video
  [/^glm-vl-/, { image: true, video: true }],
  [/^glm3-vl-/, { image: true, video: true }],

  // GLM Coder / text models: text-only
  [/^glm3-coder-/, {}],
  [/^glm/, {}],
