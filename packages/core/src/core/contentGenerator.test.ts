...
describe('createContentGeneratorConfig', () => {
  ...
  it('should preserve provided fields and set authType for GLM_OAUTH', () => {
    const cfg = createContentGeneratorConfig(mockConfig, AuthType.GLM_OAUTH, {
      model: 'coder-model',
      apiKey: 'GLM_OAUTH_DYNAMIC_TOKEN',
    });
    expect(cfg.authType).toBe(AuthType.GLM_OAUTH);
    expect(cfg.model).toBe('coder-model');
    expect(cfg.apiKey).toBe('GLM_OAUTH_DYNAMIC_TOKEN');
  });

  it('should not warn or fallback for GLM_OAUTH (resolution handled by ModelConfigResolver)', () => {
    const warnSpy = vi
      .spyOn(console, 'warn')
