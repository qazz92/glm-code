...
const TEST_HOME_DIR = path.join(os.tmpdir(), 'glm-core-logger-home');
...
describe('Logger', () => {
  ...
  describe('removeLastUserMessage', () => {
    ...
    it('only undoes USER entries (model_switch is left intact)', async () => {
      ...
      vi.advanceTimersByTime(1000);
      await logger.logMessage(MessageSenderType.MODEL_SWITCH, 'glm→glm-max');
      ...
      expect(removed).toBe(true);
      const onDisk = await readLogFile();
      expect(onDisk.map((e) => e.message)).toEqual(['glm→glm-max']);
    });
