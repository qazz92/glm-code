...
import ai.glm.code.cli.GLMCodeCli;
...
class SessionTest {
    ...
    void partialSendPromptSuccessfully() throws SessionControlException, SessionSendPromptException {
        Session session = GLMCodeCli.newSession(new TransportOptions().setIncludePartialMessages(true));
    ...
    void setPermissionModeSuccessfully() throws SessionControlException, SessionSendPromptException {
        Session session = GLMCodeCli.newSession(new TransportOptions());
    ...
    void sendPromptAndSetModelSuccessfully() throws SessionControlException, SessionSendPromptException {
        ...
        log.info(session.setModel("glm-4.5-air").map(s -> s ? "setModel 1 success" : "setModel 1 error").orElse("setModel 1 unknown"));
        writeSplitLine("setModel 1 end");

        session.sendPrompt("hello world", new SessionEventSimpleConsumers());
        writeSplitLine("prompt 1 end");

        log.info(session.setModel("glm-5.1").map(s -> s ? "setModel 2 success" : "setModel 2 error").orElse("setModel 2 unknown"));
        writeSplitLine("setModel 1 end");
        ...
        log.info(session.setModel("glm-5.1").map(s -> s ? "setModel 3 success" : "setModel 3 error").orElse("setModel 3 unknown"));
        writeSplitLine("setModel 1 end");
    ...
    void sendPromptAndInterruptContinueSuccessfully() throws SessionControlException, SessionSendPromptException {
        Session session = GLMCodeCli.newSession();
    ...
    void testJSON() {
        String json
                ...
                + " GLM Code SDK for Java today?\",\"type\":\"text\"}],\"id\":\"ed8374fe-a4eb-4fc0-9780-9bd2fd831cda\","
                + "\"model\":\"glm-5.1\",\"role\":\"assistant\",\"type\":\"message\",\"usage\":{\"cache_read_input_tokens\":12766,"
