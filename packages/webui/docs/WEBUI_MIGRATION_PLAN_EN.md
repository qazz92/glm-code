...
1. Extract components from `vscode-ide-companion/src/webview/` into an independent `@glm-code/webui` package
...
```typescript
// @glm-code/webui/src/context/PlatformContext.ts
...
```javascript
// @glm-code/webui/tailwind.preset.js
module.exports = {
  theme: {
    extend: {
      colors: {
...
module.exports = {
  presets: [require('@glm-code/webui/tailwind.preset')],
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@glm-code/webui/dist/**/*.js',
  ],
};
```
...
```
Developer ──► @glm-code/webui ──► vscode-ide-companion
  │              │                      │
...
// After: Update import
import { FileIcon } from '@glm-code/webui';
// or import { FileIcon } from '@glm-code/webui/icons';
```
...
1. **Tailwind Class Name Tree Shaking**
   ...
   - Solution: Consumer's `content` config needs to include `node_modules/@glm-code/webui`
...
│     └── Edit files in @glm-code/webui/src/                     │
│                                                                  │
...
│     └── import { Component } from '@glm-code/webui'            │
...
1. **Single Source of Truth**: All UI components live in `@glm-code/webui`
