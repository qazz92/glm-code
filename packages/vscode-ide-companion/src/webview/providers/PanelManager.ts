...
export class PanelManager {
  ...
  // the GLM webview. We update this when creating/revealing the panel and
  ...
  async createPanel(): Promise<boolean> {
    ...
    // First, check if there's an existing GLM Code group
    const existingGroup = this.findExistingGLMCodeGroup();

    if (existingGroup) {
      // If GLM Code webview already exists in a locked group, create the new panel in that same group
      console.log(
        '[PanelManager] Found existing GLM Code group, creating panel in same group',
      );
      ...
      // If no existing GLM Code group, create a new group to the right of the active editor group
      try {
  ...
  /**
   * Find the group and view column where the existing GLM Code webview is located
   * @returns The found group and view column, or undefined if not found
   */
  private findExistingGLMCodeGroup():
    ...
    for (const group of vscode.window.tabGroups.all) {
      for (const tab of group.tabs) {
        ...
        if (
          ...
          input.viewType === 'mainThreadWebview-glmCode.chat'
        ) {
          // Found an existing GLM Code tab
          console.log('[PanelManager] Found existing GLM Code group:', {
  ...
  captureTab(): void {
    ...
    setTimeout(() => {
      ...
      const match = allTabs.find((t) => {
        ...
        const sameViewType = isWebview && input.viewType === 'glmCode.chat';
  ...
  registerDisposeHandler(disposables: vscode.Disposable[]): void {
    ...
    this.panel.onDidDispose(
      () => {
        ...
        // empty (and typically locked for GLM). If so, close the group to
        // avoid leaving an empty locked column when the user closes GLM.
        if (targetColumn !== null) {
          ...
          setTimeout(async () => {
            try {
              ...
              // If the group that hosted GLM is now empty, close it to avoid
              ...
              if (group && group.tabs.length === 0) {
                ...
                try {
                  ...
                  try {
                    ...
                    console.warn(
                      '[PanelManager] Failed to close empty group after GLM panel disposed:',
                      err,
                    );
                  }
                }
              ...
              console.warn(
                '[PanelManager] Error while trying to close empty GLM group:',
                err,
              );
            }
          }, 50);
