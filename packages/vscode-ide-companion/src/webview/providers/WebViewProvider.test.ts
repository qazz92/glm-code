...
const {
  ...
  mockReadGLMSettingsForVSCode: vi.fn<
    () => {
  ...
  mockGLMAgentManagerInstances: [] as Array<{
...
vi.mock('../../services/settingsWriter.js', () => ({
  ...
  readGLMSettingsForVSCode: mockReadGLMSettingsForVSCode,
  clearPersistedAuth: mockClearPersistedAuth,
}));

vi.mock('../../services/glmAgentManager.js', () => ({
  GLMAgentManager: class {
    isConnected = false;
    ...
    disconnect = vi.fn();
    constructor() {
      mockGLMAgentManagerInstances.push(this);
    }
  },
}));
...
describe('WebViewProvider.attachToView', () => {
  beforeEach(() => {
    ...
    mockGLMAgentManagerInstances.length = 0;
  ...
  it('marks rejected switch_mode permission requests as failed without cancelling the session', async () => {
    ...
    const agentManager = mockGLMAgentManagerInstances.at(-1);
...
describe('WebViewProvider settings sync', () => {
  ...
  it('does not report success for api-key settings without interactive auth data', async () => {
    ...
    const synced = await (
      provider as unknown as {
        syncVSCodeSettingsToGLMConfig: () => Promise<boolean>;
      }
    ).syncVSCodeSettingsToGLMConfig();
  ...
  it('only syncs non-secret VS Code settings from ~/.glm/settings.json', async () => {
    mockReadGLMSettingsForVSCode.mockReturnValue({
    ...
    await (
      provider as unknown as {
        syncGLMConfigToVSCodeSettings: () => Promise<void>;
      }
    ).syncGLMConfigToVSCodeSettings();
  ...
  it('ignores non-auth glm-code setting changes', async () => {
    const provider = new WebViewProvider(
      { subscriptions: [] } as never,
      { fsPath: '/extension-root' } as never,
    );
    ...
    await configChangeHandler?.(createConfigChangeEvent('glm-code'));

    expect(syncSpy).not.toHaveBeenCalled();
  });

  it('reacts to auth-related glm-code setting changes', async () => {
    const provider = new WebViewProvider(
      { subscriptions: [] } as never,
      { fsPath: '/extension-root' } as never,
    );
    ...
    await configChangeHandler?.(
      createConfigChangeEvent('glm-code', 'glm-code.apiKey'),
    );
  ...
  it('clears persisted credentials and disconnects when apiKey is emptied', async () => {
    ...
    // syncVSCodeSettingsToGLMConfig returns false because apiKey is empty
    vi.spyOn(
      provider as unknown as {
        syncVSCodeSettingsToGLMConfig: () => Promise<boolean>;
      },
  ...
  it('does not de-auth when non-apiKey auth settings change on an api-key provider', async () => {
    ...
    // syncVSCodeSettingsToGLMConfig returns false — normal for api-key providers
    vi.spyOn(
      provider as unknown as {
        syncVSCodeSettingsToGLMConfig: () => Promise<boolean>;
      },
    ...
    await configChangeHandler?.(
      createConfigChangeEvent('glm-code', 'glm-code.codingPlanRegion'),
    );
...
describe('Notification & dot indicator', () => {
  ...
  it('shows orange dot and notification when a long task completes while panel is not active', async () => {
    ...
    expect(mockShowInformationMessage).toHaveBeenCalledWith(
      'GLM Code: Waiting for your input.',
      'Show',
    );
  });
  ...
  it('shows blue dot and notification for permission requests when panel is not active', async () => {
    ...
    expect(mockShowInformationMessage).toHaveBeenCalledWith(
      'GLM Code: Needs your permission to use Bash.',
      'Show',
    );
  });
