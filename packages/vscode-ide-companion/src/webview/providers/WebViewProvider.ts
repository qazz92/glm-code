...
import { GLMAgentManager } from '../../services/glmAgentManager.js';
...
import {
  ...
  readGLMSettingsForVSCode,
  clearPersistedAuth,
} from '../../services/settingsWriter.js';
import { parseInsightMessage } from '@glm-code/core';
...
const AUTH_RELATED_GLM_SETTINGS = [
  'glm-code.provider',
  'glm-code.apiKey',
  'glm-code.codingPlanRegion',
] as const;
...
export class WebViewProvider {
  ...
  private agentManager: GLMAgentManager;
  ...
  constructor(
    ...
    this.agentManager = new GLMAgentManager();
    ...
    const configChangeDisposable = vscode.workspace.onDidChangeConfiguration(
      async (e) => {
        const authSettingsChanged = AUTH_RELATED_GLM_SETTINGS.some((setting) =>
          e.affectsConfiguration(setting),
        );

        if (authSettingsChanged && !this.isSyncingToVSCode) {
          console.log(
            '[WebViewProvider] Auth-related glm-code settings changed by user, syncing...',
          );
          const synced = await this.syncVSCodeSettingsToGLMConfig();
          if (synced && this.agentInitialized) {
            ...
            e.affectsConfiguration('glm-code.apiKey')
          ) {
            // Only de-auth when glm-code.apiKey itself was cleared.
    ...
    // Note: Tool call updates are handled in handleSessionUpdate within GLMAgentManager
      ...
      async (request: RequestPermissionRequest) => {
        ...
        return new Promise((resolve) => {
          ...
          const handler = (message: PermissionResponseMessage) => {
            ...
            // Always close open glm-diff editors after any permission decision
            void vscode.commands.executeCommand('glm.diff.closeAll');
            ...
            if (isCancel) {
              ...
              void vscode.commands.executeCommand('glm.diff.suppressBriefly');
            }
          };
    ...
    newPanel.webview.onDidReceiveMessage(
      async (message: { type: string; data?: unknown }) => {
        ...
        if (message.type === 'updatePanelTitle') {
          ...
          if (panelRef) {
            panelRef.title = title ? truncatePanelTitle(title) : 'GLM Code';
          }
          return;
        }
        if (this.handleNewChatByContext(message)) {
  ...
  /**
   * Sync VSCode extension settings (glm-code.*) to ~/.glm/settings.json
   ...
   */
  private async syncVSCodeSettingsToGLMConfig(): Promise<boolean> {
    const config = vscode.workspace.getConfiguration('glm-code');
    ...
    try {
      ...
      console.log(
        `[WebViewProvider] Synced VSCode settings → ~/.glm/settings.json (provider=${provider})`,
      );
      return true;
    } catch (error) {
  ...
  /**
   * Sync ~/.glm/settings.json values back to VSCode Settings UI.
   ...
   */
  private async syncGLMConfigToVSCodeSettings(): Promise<void> {
    try {
      const glmSettings = readGLMSettingsForVSCode();
      if (!glmSettings) {
        return;
      }

      console.log(
        '[WebViewProvider] Syncing ~/.glm/settings.json → VSCode settings',
      );
      ...
      if (
        config.get<string>('provider', 'coding-plan') !== glmSettings.provider
      ) {
        updates.push(config.update('provider', glmSettings.provider, target));
      }
      if (
        config.get<'china' | 'global'>('codingPlanRegion', 'china') !==
        glmSettings.codingPlanRegion
      ) {
        updates.push(
          config.update(
            'codingPlanRegion',
      ...
      if (updates.length === 0) {
        console.log(
          '[WebViewProvider] VSCode settings already match ~/.glm/settings.json',
        );
        return;
      }
      ...
      console.error(
        '[WebViewProvider] Failed to sync glm config to VSCode settings:',
        error,
      );
    }
  }
  ...
  /**
   * Attempt to restore authentication state and initialize connection.
   * On startup, sync ~/.glm/settings.json → VSCode settings so the Settings UI
   * reflects existing non-secret CLI config, then attempt a connection.
   * Writing back to ~/.glm/settings.json happens through the auth flow and
  ...
  private async attemptAuthStateRestoration(): Promise<void> {
    ...
    this.initializationPromise = (async () => {
      try {
        await this.syncGLMConfigToVSCodeSettings();
  ...
  private async doInitializeAgentConnection(options?: {
    ...
    const run = async () => {
      ...
      try {
        ...
        // Load messages from the current GLM session
        ...
        vscode.window.showWarningMessage(
          `Failed to connect to GLM CLI: ${errorMsg}\nYou can still use the chat UI, but messages won't be sent to AI.`,
        );
   ...
   * Writes provider config to ~/.glm/settings.json and reconnects.
   * Mirrors the CLI's `glm auth coding-plan` / `glm auth` flow.
   */
  private async handleAuthInteractive(
    ...
    try {
      if (provider === 'coding-plan') {
        ...
        const ids = (modelIds || model || 'glm3.5-plus')
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean);
  ...
  private async attemptAutoReconnect(): Promise<void> {
    ...
    this.sendMessageToWebView({
      ...
      data: {
        message:
          'Lost connection to GLM agent and auto-reconnect failed. Please use the refresh button to try again.',
      },
    });
  }
  ...
  /**
   * Load messages from current GLM session
   ...
   *   routing to the newGLMSession handler (includes auth checks and UI clearing).
  ...
  private handleNewChatByContext(message: {
    ...
    void this.messageHandler.route({ type: 'newGLMSession', data: {} });
    return true;
  }
   ...
   * the GLM Code panel (or sidebar view) when clicked.
   */
  private notifyUser(message: string): void {
    void vscode.window
      .showInformationMessage(`GLM Code: ${message}`, 'Show')
      .then((action) => {
        if (action === 'Show') {
          const panel = this.panelManager.getPanel();
          if (panel) {
            ...
            void vscode.commands.executeCommand('glm-code.focusChat');
          }
        }
      });
  ...
  /**
   * Whether the user can currently see the GLM Code panel.
  ...
  /** Whether the glm-code.notifications setting is enabled. */
  ...
  async createNewSession(): Promise<void> {
    ...
    try {
      ...
      // Create new GLM session via agent manager
