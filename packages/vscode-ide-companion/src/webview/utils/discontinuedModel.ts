 ...
 * e.g. `glm3-coder-plus(glm-oauth)`. Runtime model snapshots are additionally
 * prefixed with `$runtime|${authType}|`, so the wrapped form becomes
 * `$runtime|glm-oauth|glm3-coder-plus(glm-oauth)`.
 *
...
/** Auth type marker for the (now-discontinued) GLM OAuth free tier. */
export const GLM_OAUTH_AUTH_TYPE = 'glm-oauth';
...
/**
 * Returns true when the model id refers to a non-runtime GLM OAuth registry
...
export function isDiscontinuedModel(modelId: string): boolean {
  ...
  return parsed.authType === GLM_OAUTH_AUTH_TYPE && !parsed.isRuntime;
}
