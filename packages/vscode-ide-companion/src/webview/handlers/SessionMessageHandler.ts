...
import type { ChatMessage } from '../../services/glmAgentManager.js';
...
import { stripZeroWidthSpaces } from '@glm-code/webui';
import {
  exportSessionToFile,
...
export class SessionMessageHandler extends BaseMessageHandler {
  ...
  async handle(message: { type: string; data?: unknown }): Promise<void> {
    ...
    switch (message.type) {
      ...
      case 'newGLMSession':
        await this.handleNewGLMSession();
        break;

      case 'switchGLMSession':
        await this.handleSwitchGLMSession((data?.sessionId as string) || '');
        break;

      case 'getGLMSessions':
        await this.handleGetGLMSessions(
      ...
      case 'deleteGLMSession':
        await this.handleDeleteGLMSession((data?.sessionId as string) || '');
        break;

      case 'renameGLMSession':
        await this.handleRenameGLMSession(
        ...
        try {
          const modelId =
            typeof data?.modelId === 'string' && data.modelId.trim().length > 0
              ? data.modelId.trim()
              : undefined;
          await vscode.commands.executeCommand('glmCode.openNewChatTab', {
  ...
  private async promptAuth(message: string): Promise<boolean> {
    ...
    if (result === 'Configure') {
      if (this.authHandler) {
        ...
        await vscode.commands.executeCommand('glm-code.auth');
      }
      return true;
    }
    return false;
    ...
    if (editTargetTurnIndex !== undefined) {
      ...
      if (!this.agentManager.isConnected) {
        await this.promptAuth(
          'You need to configure your provider to use GLM Code.',
        );
        return;
      }
      ...
      if (!this.agentManager.currentSessionId) {
        try {
          ...
          if (this.shouldPromptAuth(createErr)) {
            await this.promptAuth(
              'Your session has expired or is invalid. Please configure your provider to continue using GLM Code.',
            );
            return;
          }
  ...
  /**
   * Handle new GLM session request
   */
  private async handleNewGLMSession(): Promise<void> {
    try {
      console.log('[SessionMessageHandler] Creating new GLM session...');
  ...
  /**
   * Handle switch GLM session request
   */
  private async handleSwitchGLMSession(sessionId: string): Promise<void> {
    try {
  ...
  /**
   * Handle get GLM sessions request
   */
  private async handleGetGLMSessions(
    cursor?: number,
    size?: number,
  ): Promise<void> {
    try {
   ...
   */
  private async handleDeleteGLMSession(sessionId: string): Promise<void> {
    try {
      if (
        sessionId === this.currentConversationId ||
   ...
   */
  private async handleRenameGLMSession(
    sessionId: string,
    title: string,
  ): Promise<void> {
    try {
      ...
      // Matches SESSION_TITLE_MAX_LENGTH from @glm-code/core/sessionService
  ...
  private async handleSetModel(data?: { modelId?: string }): Promise<void> {
    try {
      ...
      // Defensive guard: refuse non-runtime GLM OAuth models in case the UI
