...
describe('SessionMessageHandler', () => {
  ...
  it('forwards the active model when opening a new chat tab', async () => {
    ...
    expect(mockExecuteCommand).toHaveBeenCalledWith('glmCode.openNewChatTab', {
  ...
  describe('handleSetModel — discontinued model defensive validation (Issue #3745)', () => {
    it('rejects a non-runtime GLM OAuth model and surfaces an error', async () => {
      ...
      await handler.handle({
        type: 'setModel',
        data: { modelId: 'glm3-coder-plus(glm-oauth)' },
      });

      expect(setModelFromUi).not.toHaveBeenCalled();
      expect(mockShowErrorMessage).toHaveBeenCalledWith(
        expect.stringContaining(
          'GLM OAuth free tier was discontinued on 2026-04-15',
        ),
      );
      expect(sendToWebView).toHaveBeenCalledWith({
        type: 'error',
    ...
    it('allows a runtime GLM OAuth snapshot to pass through', async () => {
      ...
      await handler.handle({
        type: 'setModel',
        data: {
          modelId: '$runtime|glm-oauth|glm3-coder-plus(glm-oauth)',
        },
      });
