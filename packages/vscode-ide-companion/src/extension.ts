...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@glm-code/core/src/ide/detect-ide.js';
  ...
  try {
    ...
    if (latestVersion && semver.gt(latestVersion, currentVersion)) {
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the GLM Code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('GLM Code Companion');
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer('glmCode.chat', {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('glm-code.copyMessage', () =>
      sendCopyToActive('copyMessage'),
    ),
    vscode.commands.registerCommand('glm-code.copyAllMessages', () =>
      sendCopyToActive('copyAllMessages'),
    ),
    vscode.commands.registerCommand('glm-code.copyLastReply', () =>
  ...
  context.subscriptions.push(
    ...
    (vscode.commands.registerCommand('glm.diff.accept', (uri?: vscode.Uri) => {
    ...
    vscode.commands.registerCommand('glm.diff.cancel', (uri?: vscode.Uri) => {
      const docUri = uri ?? vscode.window.activeTextEditor?.document.uri;
      if (docUri && docUri.scheme === DIFF_SCHEME) {
    ...
    vscode.commands.registerCommand('glm.diff.closeAll', async () => {
      try {
        await diffManager.closeAll();
      } catch (err) {
        console.warn('[Extension] glm.diff.closeAll failed:', err);
      }
    }),
    vscode.commands.registerCommand('glm.diff.suppressBriefly', async () => {
      try {
        diffManager.suppressFor(1200);
      } catch (err) {
        console.warn('[Extension] glm.diff.suppressBriefly failed:', err);
      }
    }),
  );
  ...
  if (!context.globalState.get(INFO_MESSAGE_SHOWN_KEY) && infoMessageEnabled) {
    void vscode.window.showInformationMessage(
      'GLM Code Companion extension successfully installed.',
    );
      ...
      async (
        ...
        if (!workspaceFolders || workspaceFolders.length === 0) {
          vscode.window.showInformationMessage(
            'No folder open. Please open a folder to run GLM Code.',
          );
          return;
        }
        ...
        if (workspaceFolders.length === 1) {
          ...
          selectedFolder = await vscode.window.showWorkspaceFolderPick({
            placeHolder: 'Select a folder to run GLM Code in',
          });
        }
        ...
        if (selectedFolder) {
          ...
          const terminalOptions: vscode.TerminalOptions = {
            name: `GLM Code (${selectedFolder.name})`,
          ...
          if (isWindows) {
            ...
            glmCmd = `ELECTRON_RUN_AS_NODE=1 ${baseCmd}`;
          }

          const terminal = vscode.window.createTerminal(terminalOptions);
          terminal.show();
          terminal.sendText(glmCmd);
        }
      },
    ),
    vscode.commands.registerCommand('glm-code.showNotices', async () => {
