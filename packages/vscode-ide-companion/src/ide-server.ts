...
} from '@glm-code/core/src/ide/types.js';
import { detectIdeFromEnv } from '@glm-code/core/src/ide/detect-ide.js';
...
import { getGlobalGLMDir } from './utils/paths.js';
...
const IDE_SERVER_PORT_ENV_VAR = 'GLM_CODE_IDE_SERVER_PORT';
const IDE_WORKSPACE_PATH_ENV_VAR = 'GLM_CODE_IDE_WORKSPACE_PATH';
const IDE_DIR = 'ide';

async function getGlobalIdeDir(): Promise<string> {
  // Anchored to the global GLM dir (not the runtime base dir) so the CLI's
  ...
  const ideDir = path.join(getGlobalGLMDir(), IDE_DIR);
...
const createMcpServer = (diffManager: DiffManager) => {
  const server = new McpServer(
    {
      name: 'glm-code-companion-mcp-server',
      version: '1.0.0',
    },
