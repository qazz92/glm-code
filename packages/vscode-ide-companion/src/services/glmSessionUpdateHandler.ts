...
/**
 * GLM Session Update Handler class
 * Processes various session update events and calls appropriate callbacks
 */
export class GLMSessionUpdateHandler {
  private callbacks: GLMAgentCallbacks;

  constructor(callbacks: GLMAgentCallbacks) {
   ...
   */
  updateCallbacks(callbacks: GLMAgentCallbacks): void {
    this.callbacks = callbacks;
  }
  ...
  handleSessionUpdate(data: SessionNotification): void {
    ...
    switch (sessionUpdate) {
      ...
      default:
        console.log('[GLMAgentManager] Unhandled session update type');
        break;
    }
  }
