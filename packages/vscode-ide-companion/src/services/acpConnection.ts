...
export class AcpConnection {
  ...
  async connect(
    ...
    if (!fs.existsSync(cliEntryPath)) {
      throw new Error(
        `Bundled GLM CLI entry not found at ${cliEntryPath}. The extension may not have been packaged correctly.`,
      );
    }
  ...
  private async setupChildProcessHandlers(): Promise<void> {
    ...
    this.child!.stderr?.on('data', (data: Buffer) => {
      ...
      if (
        ...
        console.error(`[ACP glm]:`, message);
      } else {
        console.log(`[ACP glm]:`, message);
      }
    });
    ...
    this.child!.on('exit', (code: number | null, signal: string | null) => {
      console.error(
        `[ACP glm] Process exited with code: ${code}, signal: ${signal}`,
      );
      ...
      rejectOnExit?.(
        new Error(
          `GLM ACP process exited unexpectedly (exit code: ${code}, signal: ${signal})${stderrSuffix}`,
        ),
      );
    ...
    if (!this.child || this.child.killed) {
      ...
      const stderrSuffix = stderrOutput
        ...
        : '';
      throw new Error(
        `GLM ACP process failed to start (exit code: ${code}, signal: ${signal})${stderrSuffix}`,
      );
    }
    ...
    this.sdkConnection = new ClientSideConnection(
      (_agent: Agent): Client => ({
        ...
        extNotification: async (
          ...
          } else if (method === '_glmcode/slash_command') {
