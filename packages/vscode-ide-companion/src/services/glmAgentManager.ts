...
import { GLMSessionReader, type GLMSession } from './glmSessionReader.js';
import { GLMSessionManager } from './glmSessionManager.js';
import type {
  ChatMessage,
  PlanEntry,
  ToolCallUpdateData,
...
import {
  GLMConnectionHandler,
  type GLMConnectionResult,
} from '../services/glmConnectionHandler.js';
import { GLMSessionUpdateHandler } from './glmSessionUpdateHandler.js';
...
export class GLMAgentManager {
  private connection: AcpConnection;
  private sessionReader: GLMSessionReader;
  private sessionManager: GLMSessionManager;
  private connectionHandler: GLMConnectionHandler;
  private sessionUpdateHandler: GLMSessionUpdateHandler;
  ...
  // Callback storage
  private callbacks: GLMAgentCallbacks = {};
  ...
  constructor() {
    ...
    this.sessionReader = new GLMSessionReader();
    this.sessionManager = new GLMSessionManager();
    this.connectionHandler = new GLMConnectionHandler();
    this.sessionUpdateHandler = new GLMSessionUpdateHandler({});
    ...
    this.connection.onSessionUpdate = (data: SessionNotification) => {
      ...
      try {
        ...
        console.warn('[GLMAgentManager] Rehydration routing failed:', err);
      }
    ...
    this.connection.onEndTurn = (reason?: string) => {
      try {
        ...
        console.warn('[GLMAgentManager] onEndTurn callback error:', err);
      }
    };
    ...
    this.connection.onAuthenticateUpdate = (
      ...
      try {
        ...
        console.warn(
          '[GLMAgentManager] onAuthenticateUpdate callback error:',
          err,
        );
      }
    };
    ...
    this.connection.onInitialized = (init: unknown) => {
      try {
        ...
        console.warn('[GLMAgentManager] onInitialized parse error:', err);
      }
    };
    ...
    this.connection.onDisconnected = (
      ...
      console.log(
        `[GLMAgentManager] Process disconnected (code: ${code}, signal: ${signal})`,
      );
  ...
  async connect(
    ...
    if (res.availableModels && res.availableModels.length > 0) {
      ...
      console.log(
        '[GLMAgentManager] Emitting availableModels from connect():',
  ...
  async reconnect(
    ...
    console.log('[GLMAgentManager] Attempting reconnection...');
    try {
  ...
  async setApprovalModeFromUi(
    ...
    try {
      ...
      console.error('[GLMAgentManager] Failed to set mode:', err);
      throw err;
    }
  }
  ...
  async setModelFromUi(modelId: string): Promise<ModelInfo | null> {
    try {
      ...
      console.error('[GLMAgentManager] Failed to set model:', err);
      throw err;
    }
  }
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      ...
      console.warn('[GLMAgentManager] Session validation failed:', error);
  ...
  async getSessionList(): Promise<Array<Record<string, unknown>>> {
    console.log(
      '[GLMAgentManager] Getting session list with version-aware strategy',
    );

    try {
      console.log(
        '[GLMAgentManager] Attempting to get session list via ACP method',
      );
      const response = await this.connection.listSessions();
      console.log('[GLMAgentManager] ACP session list response:', response);

      const res: unknown = response;
      const items = extractSessionListItems(res);

      console.log(
        '[GLMAgentManager] Sessions retrieved via ACP:',
        res,
        items.length,
      );
      if (items.length > 0) {
      ...
      console.warn(
        '[GLMAgentManager] ACP session list failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log('[GLMAgentManager] Getting session list from file system');
      const sessions = await this.sessionReader.getAllSessions(undefined, true);
      console.log(
        '[GLMAgentManager] Session list from file system (all projects):',
        sessions.length,
      );

      const result = sessions.map(
        (session: GLMSession): Record<string, unknown> => ({
      ...
      console.log(
        '[GLMAgentManager] Sessions retrieved from file system:',
        result.length,
      );
      return result;
    } catch (error) {
      console.error(
        '[GLMAgentManager] Failed to get session list from file system:',
        error,
      );
      return [];
    }
  ...
  async getSessionListPaged(params?: {
    ...
    try {
      ...
      console.warn('[GLMAgentManager] Paged ACP session list failed:', error);
    ...
    try {
      ...
      console.error('[GLMAgentManager] File system paged list failed:', error);
  ...
  async getSessionMessages(sessionId: string): Promise<ChatMessage[]> {
    try {
      try {
        ...
        console.log(
          '[GLMAgentManager] Session list item for filePath lookup:',
          item,
        );
        if (
          typeof item === 'object' &&
        ...
        console.warn('[GLMAgentManager] JSONL read path lookup failed:', e);
      }
      ...
      console.error(
        '[GLMAgentManager] Failed to get session messages:',
        error,
      );
      return [];
    }
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    try {
      ...
      console.error('[GLMAgentManager] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(sessionId: string, title: string): Promise<boolean> {
    try {
      ...
      console.error('[GLMAgentManager] Failed to rename session:', error);
      return false;
    }
  }
  ...
  private async readJsonlMessages(filePath: string): Promise<ChatMessage[]> {
    ...
    try {
      ...
      console.log(
        '[GLMAgentManager] JSONL records read:',
        records.length,
        filePath,
      );
      ...
      console.log(
        '[GLMAgentManager] JSONL messages reconstructed:',
        msgs.length,
      );
      return msgs;
    } catch (err) {
      console.warn('[GLMAgentManager] Failed to read JSONL messages:', err);
      return [];
    }
  }
  ...
  async loadSessionViaAcp(
    ...
    try {
      ...
      console.log(
        '[GLMAgentManager] Rehydration start for session:',
        sessionId,
      );
      console.log(
        '[GLMAgentManager] Attempting session/load via ACP for session:',
        sessionId,
      );
      ...
      console.log(
        '[GLMAgentManager] Session load succeeded. Response:',
      ...
      console.error(
        '[GLMAgentManager] Session load via ACP failed for session:',
        sessionId,
      );
      console.error('[GLMAgentManager] Error type:', error?.constructor?.name);
      console.error('[GLMAgentManager] Error message:', errorMessage);
      ...
      if (error && typeof error === 'object') {
        ...
        if ('error' in error) {
          ...
          if (acpError.error) {
            console.error(
              '[GLMAgentManager] ACP error code:',
              acpError.error.code,
            );
            console.error(
              '[GLMAgentManager] ACP error message:',
              acpError.error.message,
            );
          }
        } else {
          console.error('[GLMAgentManager] Non-ACPIf error details:', error);
        }
      }
      ...
      console.log('[GLMAgentManager] Rehydration end for session:', sessionId);
  ...
  async loadSession(sessionId: string): Promise<ChatMessage[] | null> {
    console.log(
      '[GLMAgentManager] Loading session with version-aware strategy:',
      sessionId,
    );

    try {
      console.log(
        '[GLMAgentManager] Attempting to load session via ACP method',
      );
      await this.loadSessionViaAcp(sessionId);
      console.log('[GLMAgentManager] Session loaded successfully via ACP');
      ...
      console.warn(
        '[GLMAgentManager] ACP session load failed, falling back to file system method:',
        error,
      );
    }
    ...
    try {
      console.log(
        '[GLMAgentManager] Loading session messages from file system',
      );
      const messages = await this.loadSessionMessagesFromFile(sessionId);
      console.log(
        '[GLMAgentManager] Session messages loaded successfully from file system',
      );
      return messages;
    } catch (error) {
      console.error(
        '[GLMAgentManager] Failed to load session messages from file system:',
        error,
      );
      return null;
    }
  ...
  private async loadSessionMessagesFromFile(
    ...
    try {
      console.log(
        '[GLMAgentManager] Loading session from file system:',
        sessionId,
      );
      ...
      if (!session) {
        console.log(
          '[GLMAgentManager] Session not found in file system:',
          sessionId,
        );
        return null;
      }
      ...
      console.error(
        '[GLMAgentManager] Session load from file system failed:',
        error,
      );
      throw error;
    }
  ...
  async createNewSession(
    ...
    if (!forceNew && this.connection.currentSessionId) {
      console.log(
        '[GLMAgentManager] createNewSession: reusing existing session',
    ...
    if (this.sessionCreateInFlight) {
      console.log(
        '[GLMAgentManager] createNewSession: session creation already in flight',
      );
      if (!forceNew) {
    ...
    console.log('[GLMAgentManager] Creating new session...');

    this.sessionCreateInFlight = (async () => {
      try {
        let newSessionResult: unknown;
        // Try to create a new ACP session. If GLM asks for auth, let it handle authentication.
        try {
          newSessionResult = await this.connection.newSession(workingDir);
          console.log(
            '[GLMAgentManager] newSession returned:',
          ...
          if (requiresAuth) {
            if (!autoAuthenticate) {
              console.warn(
                '[GLMAgentManager] session/new requires authentication but auto-auth is disabled. Deferring until user logs in.',
              );
              throw err;
            }
            console.warn(
              '[GLMAgentManager] session/new requires authentication. Retrying with authenticate...',
            );
            try {
              // Let CLI handle authentication - it's the single source of truth
              await this.connection.authenticate(authMethod);
              console.log(
                '[GLMAgentManager] createNewSession Authentication successful. Retrying session/new...',
              );
              ...
              console.error(
                '[GLMAgentManager] Re-authentication failed:',
                reauthErr,
              );
              throw reauthErr;
            }
        ...
        console.log(
          '[GLMAgentManager] New session created with ID:',
          newSessionId,
        );
        return newSessionId;
      } finally {
  ...
  async cancelCurrentPrompt(): Promise<void> {
    console.log('[GLMAgentManager] Cancelling current prompt');
