...
import { GLMConnectionHandler } from './glmConnectionHandler.js';
import type { AcpConnection } from './acpConnection.js';

describe('GLMConnectionHandler', () => {
  let handler: GLMConnectionHandler;
  ...
  beforeEach(async () => {
    ...
    handler = new GLMConnectionHandler();
    mockConnection = {
    ...
    it('retries connect on spawn failure and succeeds on second attempt', async () => {
      (mockConnection.connect as ReturnType<typeof vi.fn>)
        .mockRejectedValueOnce(
          new Error(
            'GLM ACP process failed to start (exit code: null, signal: SIGTERM)',
          ),
        )
