...
/**
 * GLM Connection Handler
 *
 * Handles GLM Agent connection establishment, authentication, and session creation
 */
...
/**
 * GLM Connection Handler class
 * Handles connection, authentication, and session initialization
 */
export class GLMConnectionHandler {
  /**
   * Connect to GLM service and establish session
   *
  ...
  async connect(
    ...
    console.log(`[GLMAgentManager] 🚀 CONNECT() CALLED - ID: ${connectId}`);
    ...
    if (proxyUrl) {
      ...
      console.log(
        '[GLMAgentManager] Using proxy from VSCode settings:',
        proxyUrl,
      );
    }
    ...
    for (let attempt = 1; attempt <= maxConnectAttempts; attempt++) {
      try {
        console.log(
          `[GLMAgentManager] Connecting to ACP process (attempt ${attempt}/${maxConnectAttempts})...`,
        );
        await connection.connect(cliEntryPath!, workingDir, extraArgs);
        console.log('[GLMAgentManager] ACP process connected successfully');
        break;
      } catch (connectError) {
        console.error(
          `[GLMAgentManager] Connect attempt ${attempt} failed:`,
        ...
        console.log(`[GLMAgentManager] Retrying connect in ${delay}ms...`);
    ...
    if (!sessionRestored) {
      console.log(
        '[GLMAgentManager] no sessionRestored, Creating new session...',
      );

      try {
        console.log(
          '[GLMAgentManager] Creating new session (letting CLI handle authentication)...',
        );
        ...
        if (
          ...
          console.log(
            '[GLMAgentManager] Extracted availableModels from session/new:',
        ...
        console.log('[GLMAgentManager] New session created successfully');
        sessionCreated = true;
      } catch (sessionError) {
        const needsAuth =
        ...
        if (needsAuth) {
          requiresAuth = true;
          console.log(
            '[GLMAgentManager] Session creation requires authentication; waiting for user-triggered login.',
          );
        } else {
          console.log(
            `\n⚠️ [SESSION FAILED] newSessionWithRetry threw error\n`,
          );
          console.log(`[GLMAgentManager] Error details:`, sessionError);
          throw sessionError;
        }
      }
    } else {
    ...
    console.log(`\n========================================`);
    console.log(`[GLMAgentManager] ✅ CONNECT() COMPLETED SUCCESSFULLY`);
    console.log(`========================================\n`);
    return {
      sessionCreated,
      requiresAuth,
  ...
  private async newSessionWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `[GLMAgentManager] Creating session (attempt ${attempt}/${maxRetries})...`,
        );
        const res = await connection.newSession(workingDir);
        console.log('[GLMAgentManager] Session created successfully');
        return res;
      } catch (error) {
        lastError = error;
        const errorMessage = getErrorMessage(error);
        console.error(
          `[GLMAgentManager] Session creation attempt ${attempt} failed:`,
          errorMessage,
        );

        // If GLM reports that authentication is required, try to
        // authenticate on-the-fly once and retry without waiting.
        const requiresAuth = isAuthenticationRequiredError(error);
        if (requiresAuth) {
          if (!autoAuthenticate) {
            console.log(
              '[GLMAgentManager] Authentication required but auto-authentication is disabled. Propagating error.',
            );
            throw error;
          }
          console.log(
            '[GLMAgentManager] GLM requires authentication. Authenticating and retrying session/new...',
          );
          try {
            ...
            console.log(
              '[GLMAgentManager] newSessionWithRetry Authentication successful',
            );
            // Retry immediately after successful auth
            const res = await connection.newSession(workingDir);
            console.log(
              '[GLMAgentManager] Session created successfully after auth',
            );
            return res;
          } catch (authErr) {
            console.error(
        ...
        console.log(`[GLMAgentManager] Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    }
