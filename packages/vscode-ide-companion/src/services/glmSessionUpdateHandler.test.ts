...
import type { GLMAgentCallbacks } from '../types/chatTypes.js';

describe('GLMSessionUpdateHandler', () => {
  let handler: GLMSessionUpdateHandler;
  let mockCallbacks: GLMAgentCallbacks;
  ...
  beforeEach(() => {
    ...
    handler = new GLMSessionUpdateHandler(mockCallbacks);
  });
  ...
  describe('plan handling', () => {
    ...
    it('falls back to stream chunk when onPlan is not set', () => {
      const handlerWithStream = new GLMSessionUpdateHandler({
        onStreamChunk: vi.fn(),
      });
  ...
  describe('available_commands_update handling', () => {
    ...
    it('does not call callback when onAvailableCommands is not set', () => {
      const handlerWithoutCallback = new GLMSessionUpdateHandler({});
