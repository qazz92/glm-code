...
import { getGitBranch } from '@glm-code/core/src/utils/gitUtils.js';
...
export class GLMSessionReader {
    ...
    try {
      ...
      if (!allProjects && workingDir) {
        ...
        if (!fs.existsSync(tmpDir)) {
          console.log('[GLMSessionReader] Tmp directory not found:', tmpDir);
          return [];
        }
      ...
      console.error('[GLMSessionReader] Failed to get sessions:', error);
      return [];
    }
  }
   ...
   */
  private async readSessionsFromDir(chatsDir: string): Promise<GLMSession[]> {
    const sessions: GLMSession[] = [];
    ...
    for (const file of jsonSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[GLMSessionReader] Failed to read session file:',
          filePath,
          error,
        );
      }
    ...
    for (const file of jsonlSessionFiles) {
      ...
      try {
        ...
        console.error(
          '[GLMSessionReader] Failed to read JSONL session file:',
          filePath,
          error,
        );
      }
   ...
   */
  getSessionTitle(session: GLMSession): string {
  ...
  private async readJsonlSession(
    ...
    try {
      ...
      console.error(
        '[GLMSessionReader] Failed to parse JSONL session:',
        error,
      );
      return null;
    }
  ...
  async deleteSession(sessionId: string, workingDir: string): Promise<boolean> {
    try {
      ...
      console.error('[GLMSessionReader] Failed to delete session:', error);
      return false;
    }
  }
  ...
  async renameSession(
    ...
    try {
      ...
      console.error('[GLMSessionReader] Failed to rename session:', error);
      return false;
    }
  }
}
